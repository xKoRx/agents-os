...
- **Build DEV efectivo:** `VITE_HASURA_ENDPOINT=http://192.168.31.75:8080/v1/graphql` (Hasura DEV según `v3/hasura/config.yaml`; probe 200 GraphQL con CORS reflejado) y `VITE_GATEWAY_ENDPOINT=http://192.168.31.161:8090` (Gateway DEV §5.5).
- **Servicio:** unidad `systemd --user` `echo-front-dev.service` (`~/.config/systemd/user/`), `vite preview --host --port 4173 --strictPort` con el binario node absoluto de mise, enabled en `default.target.wants`, RSS ~119 MB. Rebuild del front ⇒ `systemctl --user restart echo-front-dev`. `npm run dev` (vite `:3000`) queda como modo manual para editar el front (HMR), no como servicio.
- **CORS Gateway DEV:** nueva key ETCD `/echo/development/gateway/cors_allowed_origins = http://192.168.31.161:4173,http://localhost:4173,http://192.168.31.161:3000,http://localhost:3000` (rev 58912; antes ausente ⇒ `CORS_ORIGIN_DENIED`). Restart controlado de `echo-gateway-dev` (MainPID 2543059→3056377): health 200, `forge_ingest_misconfigured=false` en journal, automation handler y job scheduler arriba. Preflight OPTIONS 204 reflejando el origen del front.
- **Smoke:** `:4173/` y `/watchtower` (fallback SPA) 200 por LAN y localhost; shell del backoffice renderiza en browser (navegación completa, reloj UTC vivo, system log).
- **Gap declarado (data plane):** Hasura DEV `.75` **no tiene auth webhook** — request con o sin Bearer responde `x-hasura-admin-secret required`; el front envía `Bearer` de token ingresado por operador (`session_tokens.js`, sessionStorage) y el auth hook existe en el Gateway (`/api/v1/auth/hasura`). Datos vivos requieren cablear `HASURA_GRAPHQL_AUTH_HOOK` del Hasura DEV hacia `http://192.168.31.161:8090/api/v1/auth/hasura` (con su token de servicio) y proveer el token READ al operador. Sin ello el front muestra shell con `ECHO: ERROR` y métricas en cero.
- **Ownership:** esta sesión es dueña de `echo-front-dev.service` y de la key `gateway/cors_allowed_origins` DEV; cualquier otra sesión reconcilia contra esta sección antes de mutar.
...
| Front DEV (echo-backoffice) en Daedalus | **AS-BUILT 2026-09-21 en §5.7** — `echo-front-dev` RUNNING (`vite preview :4173`), build con endpoints DEV, allowlist CORS DEV agregada, Gateway reiniciado sin pérdida de ingest | Owner: cablear auth webhook del Hasura DEV (`.75`) hacia `/api/v1/auth/hasura` y proveer token READ para datos vivos. Rebuild ⇒ `vite build` con env override + restart de la unidad. |
