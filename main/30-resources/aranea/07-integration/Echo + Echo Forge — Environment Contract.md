...
- **Ownership:** esta sesión es dueña de `echo-front-dev.service`, del commit local `269fefc3` (sin push) y de la key `gateway/cors_allowed_origins` DEV; cualquier otra sesión reconcilia contra esta sección antes de mutar.
...
| Front DEV (echo-backoffice) en Daedalus | **AS-BUILT 2026-09-22 en §5.7** — `echo-front-dev` RUNNING (`vite preview :4173`), datos vivos verificados vía `VITE_HASURA_ADMIN_SECRET` (commit local `269fefc3` sin push), CORS DEV en allow-all (`*` + `GATEWAY_ENV=develop`), Gateway reiniciado sin pérdida de ingest | Owner: revisar/decidir destino del commit local `269fefc3` (push o revert); cablear auth webhook del Hasura DEV para endurecer a Bearer; rotar la secret DEV si su exposición por LAN no se acepta. Rebuild ⇒ `vite build` con env override + restart de la unidad. |
