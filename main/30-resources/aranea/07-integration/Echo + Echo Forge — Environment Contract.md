...
- **Gateway DEV:** `active (running)`, `/health` HTTP 200, listeners `*:8090`; restart controlado y recuperación ante `SIGKILL` al MainPID verificados (nuevo PID, health 200, 1 scheduled restart en journal); `NRestarts` estable; journal sin fatales. Sin PG queda degradado por diseño (close-positions/automation/execution-policy/republish deshabilitados con warning) y el boundary Forge ingest quedó `forge_ingest_misconfigured=true` ⇒ `/api/v1/forge/promotions` en fail-closed 503, tal como anticipa el contrato.
- **Core DEV:** `BLOCKED` — arranca ETCD/Kafka/telemetría OK y falla fail-fast en `pq: password authentication failed for user "echo_user"`. Diagnóstico: la credencial canónica del propio repo (fixture `developmentEchoConfig` en `v3/sdk/etcd/echo_seed_test.go`) también es rechazada por `.220` (probe read-only vía `.pgpass` temporal, sin secretos en argv/journal) ⇒ drift **server-side**: el password real de `echo_user` en PG DEV no coincide con la convención repo/ETCD (o el valor de ETCD está corrupto; no inspeccionable por filtro de secretos). **Owner action:** alinear credencial (rotación gestionada por owner: actualizar `/echo/development/postgres/password` o resetear el rol en `.220`) y de paso retirar la credencial de prueba versionada del repo. La unidad quedó `enabled` + `inactive` (stop intencional tras el bloqueo; sin restart-loop).
...
- **Wiring Flink/StateFun DEV:** `module.yaml` de `v3/core/deploy/flink-statefun/develop` apunta las functions a `http://dev.echo.core.lab.aranea:9090/statefun` y ese nombre resuelve a 192.168.31.161 ⇒ el diseño DEV ya espera el Core en Daedalus; ingress/egress del job son el clúster Kafka DEV. Observación vigente: el job `StatefulFunctions` en Flink DEV está inestable (re-emisiones con vida ~75–80 s por intento) **antes** de que Core subiera; no se tocó infra Flink; su estabilización con Core DEV arriba queda como verificación pendiente del join funcional.
...
| Core/Gateway sobre Daedalus | **AS-BUILT 2026-09-21 registrado en §5.1** — Gateway `RUNNING/PHYSICALLY_VERIFIED`; Core `BLOCKED` por credencial PG DEV (owner); `systemd --user`+linger, unidades de sistema pendientes de owner con root | Owner: resolver credencial `echo_user` en `.220`/ETCD y relanzar `echo-core-dev`; opcional migrar a unidades de sistema. |
