...
7. **Consumer smoke.** Desde un consumer real (Daedalus via `daedalus-ops`): resolver la entry real de `mcp.json` con su ref `${env:VAR}`, bearer por stdin, initialize/tools/list/tools/call. PASS sólo con el MCP client shape real del consumer. Valida primero las llamadas del probe contra la superficie real (schemas/inputSchema vivos del backend) antes de congelar aserciones: un fallo del probe es del probe, no de la capability, y corregirlo no cuenta como mutación. Para el gate de persistencia del chain el smoke stdin NO basta: certificar source-ando el chain real con el `HOME` del consumer resolviendo la config publicada.
