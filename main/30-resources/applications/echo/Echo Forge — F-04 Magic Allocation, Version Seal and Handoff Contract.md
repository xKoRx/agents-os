...
- C5.1–C5.6 implementation evidence: Symphony `b57bfb2c3d2c4e0a96d2b3fa654cea41e1a64f43` (parent `bba833d7b57c767d6ce5ebfeae7a7b71b5785782`, fast-forward on `feature/f04-magic-version-handoff`). Dedicated `StrategyManifestIdentityReader` uses the exact StrategyRef query above; durable row is manifest authority, carrier canonical is exact consistency only, and `StrategyIdentityView` is unchanged. Durable `L|LONG→contracts.SideLong`, `S|SHORT→contracts.SideShort`, `B|BOTH|empty|unknown→CONTRACT_CONFLICT` before manifest effects; requested/observed instrument/timeframe come from the durable row after exact case-sensitive TrimSpace gate against WorkflowSpec (direction transported from `spec.Direction`). Legacy parser and helper are removed. C5 tests cover opaque/decoy canonical IDs, LONG/SHORT/BOTH/unknown/empty, missing/incomplete row, carrier mismatch, instrument/timeframe/direction gates, G22, G24 preservation, and byte-exact `manifest.Encode()`. Focused `-race`, build and vet pass; worker full red set equals baseline (16); registry full HEAD was limited by a 10-minute embedded-postgres timeout, with the historical subset showing no new failure. `MIGRATION: NONE`; migrations 015/016 untouched and 017 absent. T2.11/T2.12/T2.13 remain OPEN; F-04 remains NOT CLOSED and not physical-ready.
