...
`ENGINEERING_STAGE_CLOSED_RESEARCH_REPRODUCIBLE` sobre HEAD `85e27ff` (código `56e8fac`, M4 v07 @ `c38f6c4`). **Cero código nuevo en integración; M4 v07 no recertificado.** U-02 **`V2_CASH_CONFIRMED`**: BUY fee en pUSD, shares completas; `TAKER_PROCEEDS` @ `d5ce263` es V1 y no se integra. `REAL_FEE_READY=NO`. Discovery PE-001: 41/73/7/0/0. Weather inventariado (KLGA/RJTT) sin vintages. Catalog `first_known_at` ≠ `createdAt` demostrado; cohorte O/B real ausente. `LIVE_DISABLED`. Review/publicación owner abiertas (histórico; supersedido por master 2026-09-21). Informes: `hardening-20260921/REPORT.md`, `u02-protocol-authority-20260921/REPORT.md`. Change logs [[2026-09-21-polymarket-final-readiness]], [[2026-09-21-u02-protocol-authority]].
...
`DESCRIPTIVE_ONLY`. Publicado `09e8c76` (código `66486ac`). M4 no-live recertificado en ese código, 27/0/5 diferidos live. SCREEN `cuts=30` completa con 0 oportunidades; el tick `0.01` sólo está acreditado por un `old_tick_size` posterior a la ventana y no se aplica hacia atrás. Kickoff post hoc. `REAL_FEE_READY=NO`. `LIVE_DISABLED`. No declarar backtest, OOS ni hipótesis validada. La semana Sports sigue en el binario `bfa6eaf`; el servicio no se reinició y la campaña entró en captura por su propio reloj. Change log [[2026-09-21-historical-backtest-readiness]].
