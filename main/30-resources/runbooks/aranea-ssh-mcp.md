...
1. **Fijar perfil y autoridad.** Default: perfil `viewer` y `read-command`. No usar `run-command` contra perfiles viewer. Usar operator sólo cuando la tarea exija escritura/ejecución. No invocar operaciones privileged/admin ni alterar ACLs salvo autorización explícita de esa acción administrativa exacta.
...
| `docker-echo-dev-operator` | Linux `docker-echo-dev` / `192.168.31.75` | operator / writable / root | config/filesystem, Docker/Compose, logs, exec y lifecycle DEV |
...
Un intento de `run-command` contra el perfil viewer `sqx-zeus` devolvió `POLICY_DENIED: Profile "sqx-zeus" is read-only, so "safe" commands are refused.`; este rechazo es comportamiento esperado y confirma el boundary, no un fallo del MCP.
...
Reportar profile, target host, operación ejecutada, output/file path relevante, si hubo mutación y cualquier boundary de policy/ACL. Para operators root-equivalent, incluir rollback/post-condición cuando exista mutación.
