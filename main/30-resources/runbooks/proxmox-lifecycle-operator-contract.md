...
3. Power (start/stop/shutdown/reboot): target proof + estado actual leído en el mismo minuto; soft shutdown con timeout antes de hard stop; el LXC `mcps` (VMID 113 @ hades, 192.168.31.219 — sostiene el plano MCP) y la VM `agent`/hermes-vm (VMID 118 @ kronos — el propio runtime de Ariadna) son críticos: su manipulación es materia de [[HERMES — Agent Access Operations]], gate propio o recuperación out-of-band (si el clúster completo cae, la recuperación debe originarse fuera de hermes-vm).
