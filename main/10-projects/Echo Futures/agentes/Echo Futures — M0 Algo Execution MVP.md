...
## Architecture Correction — Futures-first, market-extensible Core — 2026-09-25

This section is binding for M0 design and supersedes any interpretation that turns Echo Futures into a provider-specific copier.

### Principle

Build the runtime **for futures now**, but keep generic domain boundaries where doing so costs little and prevents a future rewrite for another centralized/streaming market such as US small-cap equities.

Do NOT implement equities, LULD, short locates, NBBO, corporate actions or equity-specific execution now.

Do NOT genericize every futures concept into an abstract framework. Futures-specific logic is allowed behind market capabilities.

### Echo Core owns the decision plane

Existing Echo already centralizes execution-policy resolution, risk/lot sizing, execution planning, account/position state and automation in Core. Echo Futures extends that model so Core can also originate virtual Reference signals rather than requiring every Reference to come from an external trading account.
...
MarketDataHub
  ...
  -> Capital/RiskPolicy
  -> ExecutionPlanner
  -> Account/ProviderPolicy
  -> ExecutionCommand
```

A `ReferenceSignal` is a virtual reference generated by strategy code inside Core.

It is not an executed trade and contains no account sizing.

### Signal contract

Strategy owns the trade thesis, not capital allocation.

Minimum `ReferenceSignal`:

- signal_id / strategy_id / strategy_version;
- market + instrument_id;
- side;
- entry intent (market/limit/stop semantics where supported);
...
The initial SL/TP belong to the strategy thesis. A separately versioned `PositionManagementPolicy` may later modify them after fills (hardscalping/add/recovery), but those actions are not signal-generation logic.

### Capital/risk split

Separate pluggable policies:

1. `CapitalPolicy` — how much real/account risk budget is available for this signal/account/stage.
2. `SizingModel` — convert risk budget + stop distance + instrument specification into quantity.
3. `InterTradeRiskPolicy` — progression/reduction based on prior outcomes and prop lifecycle.
4. `PositionManagementPolicy` — intra-trade adds, reductions, stop/target moves.
5. `ProviderRuleSet` — what the account/provider permits.

All are versioned and independently swappable.

### Market-agnostic low-cost domain

Generic primitives now:
- `MarketID`;
- `VenueID`;
- `InstrumentID`;
- `InstrumentSpec`;
...
Futures extension now:
- root/contract/expiry;
- tick size/value;
- contract multiplier;
- roll mapping;
- CME session templates.

Future equity-specific capabilities are deferred.

## Market Data Architecture — hot path cannot query a database

Latency rule:

> A strategy decision triggered by a tick/bar MUST NOT perform a network/database query for candles or indicators.

### Hot state — in Core memory

For every active `instrument × timeframe` maintain a single-writer `MarketStateShard`:

- latest canonical quote/trade;
- current forming bar;
- bounded ring buffer of closed bars;
- incremental indicator state;
- feed sequence/health;
- session/calendar state.

Strategies access a read-only snapshot/view of this state.

Typical configured hot window:
- enough bars for the largest strategy lookback + safety margin;
- e.g. 2k–10k closed bars/timeframe, configured rather than hard-coded.

No Ristretto/general cache is required in the tick path: use purpose-built fixed-capacity ring buffers / arrays with predictable allocation and O(1) latest-N access.

### Per-tick behavior

```text
canonical tick
 -> MarketStateShard.Update()
 -> update current bars for subscribed timeframes
 -> update incremental indicators
 -> emit TickUpdated / BarClosed
 -> execute only subscribed strategies
```

Never recalculate an indicator from the complete candle history on every tick.

Never deserialize candles from PostgreSQL/Mongo/MinIO on every tick.

### Warm state / startup

Before a strategy becomes ACTIVE:

1. load its required recent closed bars from the historical store;
2. build ring buffers + indicator state;
3. connect reference feed;
4. reconcile any gap;
5. mark `MarketStateReady=true`.
...
Use different storage by purpose.

**Operational metadata/state**
- existing PostgreSQL/etcd patterns remain appropriate.

**Hot market state**
- memory only + checkpoint/reconstruct contract.

**Canonical market-data archive**
- append-only immutable segments; partition by market/instrument/date/source.
- initial implementation may use compressed files/Parquet and MinIO/local disk.
- this archive is the authority for replay/backtest data owned by Echo.

**Queryable historical bars**
- write closed canonical bars asynchronously.
- persistence is NOT in the decision path.
- keep storage behind `HistoricalMarketStore`; choose/upgrade physical engine only when measured query volume requires it.

Do not introduce ClickHouse/Timescale/etc. solely for M0. The contract must permit one later.

### Recovery

Persist/checkpoint:
- last accepted feed sequence/time;
- current partial bars if needed;
- latest closed-bar key;
- indicator checkpoint optional.

Restart:
- warm from durable closed bars;
- replay recorded events from checkpoint;
- reconcile live-feed gap;
- activate strategies only after deterministic state is reconstructed.

### Feed authority

One feed is authoritative at a time.

Primary + backups are monitored in parallel but never mixed into the same canonical event sequence.

Failover:
`FREEZE_NEW_RISK -> validate candidate feed/gap/contract -> FEED_FAILOVER event -> warm/reconcile -> RESUME`.

Execution-platform quotes/fills remain execution evidence; they do not silently replace the Reference feed.

## Multi-strategy runtime

Multiple StrategyDefinitions may run concurrently.
...
- parameters;
- PositionManagementPolicy;
- CapitalPolicy/RiskPolicy;
- target account group.

Strategy instances are deterministic state machines over the canonical event stream.

A slow strategy MUST NOT block the market-data shard. Strategy evaluation has explicit latency budgets and instrumentation.

## Future small-cap compatibility boundary

To avoid a future rewrite:
- do not name generic domain fields `futures_symbol`, `contract_qty`, etc.;
- use market/instrument capabilities;
- keep feed normalization and execution adapters outside strategy code;
- keep calendar/session logic injectable;
- keep storage partitioning market-aware.

Do NOT implement small-cap-only concerns in M0.

Acceptance test for extensibility is architectural only:
a fake second market with a different InstrumentSpec/Calendar can flow through MarketDataHub -> Strategy -> ReferenceSignal without modifying strategy runtime infrastructure.

## Performance budget

Initial futures decision-plane objective:
- feed-normalization + bar update: p99 << 10ms locally;
- strategy evaluation: instrumented, target p99 <100ms for ordinary bar/tick strategies;
- signal -> fan-out decisions: target <100ms intra-core;
- no DB/network read on decision hot path;
- end-to-end remains compatible with Echo's existing <1s objective.

These are budgets, not HFT targets.
