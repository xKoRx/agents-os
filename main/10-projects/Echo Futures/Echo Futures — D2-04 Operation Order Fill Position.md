...
> Diseño técnico V1 del modelo `Operation / Order / Fill / Position` y su runtime dentro de Echo V3. Respeta sin reabrir las decisiones owner [[Echo Futures]] D2-01 (snapshot + contract pinning), D2-02 (fan-out + single Operation) y D2-03 (Signal + Strategy/MM boundary), y el lifecycle A2 aceptado en [[Echo Futures — D1 Analysis Pack]]. Responde Q2 (Position attribution) y Q3 (Order lifecycle). Baseline físico verificada: `xKoRx/echo origin/master = 372af59a7b83604781346613da01e3d510ea1360` (sin delta). No implementa código productivo y no cierra D2. Tras el primer manager review (`D2_04_MANAGER_REVIEW = CORRECTION_REQUIRED`) incorpora las correcciones R1–R9, y tras el segundo (`D2_04_MANAGER_REVIEW_2 = CORRECTION_REQUIRED`) las R10–R13 — ambas registradas en la sección "Manager Repair — 2026-09-26".
...
- **Duplicado antiguo** (fill ya aplicado a A, reaparece con A terminada): guard de identidad → late-event path → intento de inserción en PG con identidad única `(account_id, provider_execution_id)` → PK conflict → métrica de duplicado, sin fila nueva, sin breach falso, sin tocar B.
- **Fill genuino nuevo de una Operation TERMINAL** (execution id no visto antes, para Order de A): es un hecho físico real. Manejo V1: se conserva como hecho vinculado a A (fila en `echo.fills` vía late-event path, insert idempotente), se marca/emite `POST_TERMINAL_EXECUTION_BREACH` (telemetría + alerta safety/reconciliación) y queda visible en `POSITION_MISMATCH` (§7.3). Prohibido: tratarlo como duplicado automático, descartarlo, aplicarlo a B o revivir A. A permanece TERMINAL (el hecho no recomputa decisiones de un aggregate terminal); la política de reacción (p. ej. bloquear nueva exposición de la cuenta) es del safety plane — sin auto-repair, `DT-EF-POSITION-RECONCILIATION-05` sigue diferida.
- **Estado mínimo que sobrevive — justificación KISS:** el guard sólo necesita `current_operation_id` (ya existe en el keyed state) + la identidad que porta el evento; la pregunta "¿este execution id ya se aplicó a una operación anterior?" se resuelve fuera del aggregate con la identidad única de PG (best-effort, no correctness — la correctness del aggregate vive en el guard) y la verdad física en la reconciliación del venue. No se requiere archivo de tombstones en StateFun: la membresía histórica es consultable en `echo.fills` y no hay hot path que la necesite.
...
- **PG = proyección eventual/query-only (R11):** `echo.operations` / `echo.orders` / `echo.fills` / `contract_positions` se escriben best-effort con el writer async embebido (upserts idempotentes y **stale-safe**: sólo aplican si `operation_event_seq` avanza). PG adelantada al checkpoint (caso B: PG seq 50, checkpoint 45) es inofensiva: el runtime restaura 45 y el replay re-aplica 46..50 con contenido idéntico; PG atrasada (caso C: checkpoint 50, PG 45) converge eventualmente. PG failure/lag jamás cambia decisiones live ni bloquea Operation/MM (métrica de lag + retry del writer; sin backpressure hacia el aggregate). PG sirve a queries, UI, reporting, observabilidad y auditoría — no a la recreación de `mm_state` ni a decidir qué Orders físicas existen.
...
- **Persistencia autoritativa (R11):** el estado operativo (identidad/estado de Operation y Orders, exposición, dedup sets R7, `mm_state`, `operation_event_seq`) vive en el keyed state checkpointeado por Flink/StateFun. PG es proyección auxiliar (queries/reporting/auditoría) de esas mismas entidades más todos los Fills inmutables; nunca fuente de recovery. Cold disaster ⇒ `COLD_RECOVERY_REQUIRED` (§8.5).
...
- **R9 — Cold recovery procedure:** el procedimiento completo `COLD_RECOVERY_REQUIRED` (re-anclaje de `mm_state`, bootstrap controlado, DR) queda para implementación/DR; V1 congela sólo el comportamiento fail-closed (reconciliar venue + bloquear nuevo riesgo), no la reconstrucción exacta.
