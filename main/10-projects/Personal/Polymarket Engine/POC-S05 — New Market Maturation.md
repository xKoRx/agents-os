...
- **Shared foundations integradas (2026-09-20):** [[Polymarket Engine — POC Shared Unblocker]] entregó INTEGRATION_SHA `9d0512a912fcce4b9aefc152c7a89b090ff8df1d` (branch `feature/shared-poc-unblocker`, rebase sobre `feature/research-strategies-v01@f070496`, sin push, pendiente Review del owner). Estado esta POC: cohorte **O/B `READY_WITH_RESTRICTIONS`** — SFG-05/01/03/07 re-validados PASS sobre ese SHA (`FrameObserver` + observación durable incluso 0 oportunidades + `RunStrategyReplay` probados); cohorte **W `BLOCKED_BY_SFG06`** — decisión G3 Caso A de esta SPEC (WP C1 condicional): el residual Catalog reducer→UniverseChanged→replay queda tarea explícita separada, fuera de la ruta crítica. Baseline de arranque = `9d0512a`; prohibido reimplementar FrameObserver/observaciones, replay o dataset guard; handoff completo en la nota shared.
