...
- **Estado de la iteración:** Fase A **PASS** (`RS-V01-A_PASS`, checkpoint engine `7eb3532` en `feature/research-strategies-v01`, sin push): screen runner extraído a pipeline compartido (journal+store+books+dispatcher), registro de factories de estrategia, comando `screen-consolidated` que entrega el mismo frame (copias inmutables) a N instancias aisladas y consolida un manifest JSON contando decisiones por instancia — sin sumar PnL ni dedupear oportunidades entre estrategias sin identidad compartida demostrada. Evidencia del gate en `cmd/engine/screen_consolidated_test.go`: dos instancias del fixture con parámetros distintos → manifests distintos y métricas distinguibles, instancias ≡ run solo (sin estado compartido), determinismo probado, rechazos honestos; regresión completa verde.
