...
- **`DESCRIPTIVE_ONLY` (2026-09-21), readiness histórica PE-005-R1:** `master`/`main` @ `09e8c76` (código `66486ac`). M4 no-live recertificado en `66486ac` (27/0/5 diferidos live). El tick `0.0100` de la cohorte 22:40Z queda acreditado por el primer `tick_size_change` posterior (`old_tick_size`), con continuidad desde el `new_market` del 2026-08-26 y cero cambios intermedios; el reducer congelado no lo aplica a los libros de la ventana. El kickoff sigue `SCHEDULE_CORROBORATED_POSTHOC` (Wayback 503). SCREEN `cuts=30` ahora entrega 30/30 con 0 oportunidades; replay determinista; shadow `INCONCLUSIVE`, fee no resuelta. No es `BACKTEST_PASS`, `OOS_PASS` ni `HYPOTHESIS_VALIDATED`. `REAL_FEE_READY=false`. `LIVE_DISABLED`. El binario de Sports Week sigue en `bfa6eaf`; el servicio no se reinició. La campaña entró sola en `CAPTURING` a las 2026-09-21T21:05:20Z. Detalle: continuidad §16 y [[2026-09-21-historical-backtest-readiness]].
- **`HISTORICAL_DATA_PARTIAL` (2026-09-21), campaña Sports intacta:** `master`/`main` @ `de33d7d` (código `56195c9`). Adaptador `histimport` admite NDJSON histórico al journal de Capture sin inventar tick ni fee. M4 recertificado no-live en `56195c9` (27/0/5 diferidos live); el binario de la semana Sports sigue siendo `bfa6eaf` y el servicio no se reinició (`started_at` null). Piloto PE-005-R1 sobre cuatro moneylines MLB del 2026-09-01 22:40Z: replay determinista, calidad restringida `SYNCING` (cero `tick_size_change` en las horas), describe con 0 ensanchamientos al umbral congelado de 50 bps, SCREEN congelado `cuts=30` bloqueado por inbox, shadow `INCONCLUSIVE` con 0 oportunidades y fee no resuelta. No es `BACKTEST_PASS`, `OOS_PASS` ni `HYPOTHESIS_VALIDATED`. `REAL_FEE_READY=false`. `LIVE_DISABLED`. Detalle: continuidad §15 y [[2026-09-21-historical-research-m0]].
- **`ARMED_WAITING_NEXT_WINDOW` (2026-09-21):** `master`/`main` @ `bfa6eaf` (código `c3b1aa1`). Fee de mercado y fee de trade son identidades distintas: un trade dentro del cap deja de ir a `fee_source_discrepancy`. M4 recertificado no-live en `c3b1aa1` (27/0/5 diferidos live). Canary 4584879 PASS (`OBSERVED_USABLE`, trade `"0"` conservado, suspect false). Campaña de 7 días **armada, no iniciada** (`started_at` null). Próxima ventana T−90: 2026-09-21T21:05:00Z. `REAL_FEE_READY=false`. `LIVE_DISABLED`. U-02 `d5ce263` sigue excluido. Detalle: continuidad §14 y [[2026-09-21-pe005-r1-sports-week]].
- **`MASTER_CANONICAL` (2026-09-21, histórico):** repo `xKoRx/polymarket-engine`, default GitHub `master` @ `a770da6648f93127411cac9a844baaacb7a53397` (= `origin/main`, fast-forward desde `9ae5dde`). Código Five-POC `56e8fac` / evidencia v07 `c38f6c4` / HEAD de integración `85e27ff` absorbidos. M4 recertificado no-live **en `85e27ff`** (27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 live diferidos); recibo `testdata/research-master/certificate.json` pineado entonces en `a770da6`. Worktree canónico: `~/go/src/github.com/xKoRx/polymarket-engine-master`. `LIVE_DISABLED`. U-02 `d5ce263` **excluido** (`PATCH_NOT_ACCEPTED_FOR_V2`); unidad BUY = `V2_CASH_CONFIRMED`; `REAL_FEE_READY=false`. Sports E2 es research-only (`NO_SIGNALS_IN_SAMPLE`). Supersedido en publicación por `bfa6eaf`. Detalle: continuidad §13 y [[2026-09-21-polymarket-master-consolidation]].
...
- **`M4_CERTIFIED_NON_LIVE` — MANDATO S05–S13+M4 COMPLETADO (2026-09-18):** S05 Regimes/Resolution (`5cc6476`), S06 Market WS/Books con captura real (`0159eb5`), S07 Frame Builder (`b9e81e7`), S08 Replay + **M3-B** (`4631f3b`), S09 Strategy/SCREEN (`238ada9`), S10 Economics/Simulator (`1d19254`), S11 Account/Risk (`37b8146`), barrera S10×S11 (`1bbe445`), S12 Supervisor/Backup-Restore + **M3-C** (`edc0a89`), S13 Experiments/SHADOW + **M3-D** (`d2e73e8`), certificación M4 (`ab33405`): 23 gates no-live PASS, 0 FAIL, 5 diferidos live (G-16…G-19, G-14b) NOT_RUN por diseño. Pre-flight S04 ratificado (`e28c7eb`). Verificaciones reales read-only: Gamma sync (S04), CLOB fee-rate/market-details, Data v2 resolutions, Market WS record + replay (S06/S08). Detectos y corregidos upstream por wire vivo: discriminador `event_type` del WS, frames array-wrapped, extras del book, keying XOR de resolutions, presencia de flags Gamma. LIVE_DISABLED durante todo el mandato; sin órdenes reales; sin push. Detalle completo por slice en la bitácora. **[SUPERSEDIDO por auditoría manager → `M4_ACCEPTANCE_PENDING`, ver bullet siguiente y bitácora.]**
- **`M4_ACCEPTANCE_PENDING` — AUDITORÍA M4 MANAGER (2026-09-18):** la auditoría física sobre `ab33405` no ratifica el cierre: (1) `go test ./...` NO reproducía verde — defecto real primitivo outbox (S03) vs migración 0035 (S11): `namespace NOT NULL` + `INSERT OR IGNORE` tragaba la violación y reportaba `inserted=false` en el primer enqueue (el test del baseline lo detectaba); (2) el denominador del harness omitía 4 identificadores in-scope de M2.5 (G-07/G-11/G-12/G-15): las 23 filas no son 26 gates — G-01 aparece en 3 filas y G-08+G-09 comparte fila; (3) el harness certificaba un bundle intercambiado entre gates (sin verificación de identidad) y sus filas suite-backed son PASS incondicionales; (4) coverage módulo 84.1% < piso 95% sin excepciones aprobadas (supervisor 64.7%, experiment 68.4%, frames 82.4%, cmd 50.4%). Correcciones aplicadas por ownership en checkpoint `M4-AUDIT-C1` (`879886f`): primitivo outbox con namespace + regresión contra la tabla migrada real; fixture E2E record con books periódicos como el wire vivo (flake bajo race 3/3 eliminado); harness con identidad de gate, denominador restaurado (G-07/G-12/G-15 PASS respaldados por suites físicas; **G-11 NOT_RUN in-scope** por fixtures de reconcile inexistentes) y regresión del denominador; outcome `certify` ahora `M4_PARTIAL` con evidencia completa. Fix upstream WS `event_type` **RATIFICADO** (variante demostrada con fixture live sanitizado + provenance, `type` documentado first-class, fail-closed sin tipo, unknown crítico con raw preservado, sin parser permisivo global). E2E físicos verdes: `journal verify` 4621 records sin fallos, replay digest idéntico en 2 schedules, shadow sobre journal vivo (920 frames, 920 oportunidades, 0 delivery errors, INCONCLUSIVE honesto), backup→restore→verify (8/8 archivos, journal restaurado sin fallos), boot LIVE_DISABLED/NO_LEASE/readiness_live false, tidy sin diff, diff-check limpio. Quality final en `879886f`: build/vet/test/race verdes (24 paquetes). Pendiente decisión owner (tabla en bitácora): fixtures G-11 o scoping explícito, excepciones de coverage por paquete, política de verificación de suites dentro de `certify`.
- **`M4_MANAGER_ACCEPTANCE_READY_PENDING_OWNER` — CORRECTIVO M4-AUDIT-C2 (2026-09-19, baseline final `1c4f123`):** sobre el baseline ratificado `879886f` se ejecutó el correctivo integral: **(1) G-11 cerrado** con fixtures propias dentro del ownership Account (`reconcile.go` + migración 0036 `account_reconcile_cases`): ejes de orden/settlement monótonos con contradicciones → casos durables idempotentes (nunca terminalidad inventada), settlement FAILED sin liberar reserva/crear saldo/confirmar y escalando a HUMAN_REVIEW_REQUIRED con fills atribuidos, reorg rechazado tras CONFIRMED, transferencias exactly-once con UNATTRIBUTED bloqueante y SELL cross-instancia sólo vía INSTANCE_MOVE explícito, scan REST paginado con re-lectura de estabilidad (mutación o fallo de página ⇒ UNKNOWN + caso, jamás ausencia concluyente); fixtures versionadas con manifest sha256 + provenance y evidencia `testdata/account/evidence/g11.json` (15 escenarios); regresión G-02/G-02b/G-10c/G-11b/G-12/G-12b verde con race. **(2) G-07 con evidencia formal** `testdata/replay/evidence/g07.json`: tres schedules {32}/{1}/{7,3,1} explícitos, digests idénticos, delivery verdicts. **(3) Cobertura endurecida por prioridad** con tests de falla útiles (sin borrar código ni excluir paquetes): supervisor 64.7→79.5 (corrigiendo **defecto real**: Boot no copiaba dataDir a cfg ⇒ watermarks de disco nunca salían de OK), frames 82.4→87.6, account 77.7→80.5, risk 82.9→91.4, replay 79.7→80.7, strategy 75.9→78.5, simulator 79.8→85.4, cmd 50.3→68.0; módulo ~87% — **el piso 95 por paquete sigue sin cumplirse**: única decisión owner pendiente (elevar o excepciones por líneas con la clasificación entregada). **(4) Harness reconstruido (D3/D4):** 32 filas (27 no-live sobre 26 identificadores + 5 live diferidos), identidad completa por fila (evidence path+SHA-256, verify command, timestamp, baseline), receipts reales ejecutados al certificar (recibo vacío = FAIL; sin suites = NOT_RUN jamás PASS; PARTIAL fail-closed), `M4_STALE_CERTIFICATION` ante baseline incorrecto, matriz negativa pineada + test de integración sobre el árbol real. **(5) Certificado final en `1c4f123`:** `M4_CERTIFIED_NON_LIVE` con pin de baseline — 27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 diferidos; regresión build/vet/test/race/tidy/diff-check verde; E2E sobre el journal vivo: verify sin fallos (frontier 6464), replay digest `7f24c081…` idéntico en {1}/{32} (coincide con la auditoría previa — sin recaptura), SHADOW INCONCLUSIVE honesto, backup→restore 16/16 + verify restaurado, boot LIVE_DISABLED/NO_LEASE/readiness_live false. Checkpoints `17cc207` (G-11), `5fbeec5` (G-07), `4381884` (coverage), `1c4f123` (harness) en `main` sin push. Agent run: [[2026-09-19-zcode-glm-polymarket-m4-audit-c2]].
- **`M4_MANAGER_ACCEPTANCE_READY_PENDING_OWNER` — MANDATO M4-COVERAGE-FINAL (2026-09-19, baseline final `9ae5dde`):** el piso Agents-OS de 95% de cobertura por paquete quedó CUMPLIDO en los 24 paquetes con statements (denominador estricto tests-propio-paquete, `covermode=atomic`, el mismo de la auditoría C2; sin `-coverpkg`, sin exclusiones, sin manipulación de denominador): cmd/engine 68.1→95.2, account 80.5→95.3, strategy 78.5→99.4, strategy/neutral 78.3→97.6, supervisor 79.5→95.8, replay 80.7→97.4, frames 87.6→100, capture 89.5→98.9, books 90.2→99.6, economics 78.1→97.1, simulator 85.4→100, regimes 87.0→97.5, persist 88.9→97.5, clob/datav2/marketws 86.5/87.8/85.7→100×3, experiment 88.0→95.8, risk 91.4→100, foundation 94.0→99.2, protocol 94.7→96.4 (módulo 87.0→97.15%; residuo total 280 statements clasificados: PROVEN_UNREACHABLE con demostración o NONCRITICAL wiring sobre caminos probados en su capa dueña — cero CRITICAL_TESTABLE alcanzable sin cubrir y cero UNRESOLVED). La auditoría detectó y corrigió por ownership DOS defectos reales con regresión: `books.applyBook` fusionaba mapas de niveles en snapshots del mismo epoch violando la semántica frozen replace-all-levels (liquidez fantasma, cruces enmascarados; el test enmascarado se reparó al camino genuino de cruce por delta) y `replay.RunObservation` colgaba en bucle infinito con schedule no-positivo por la API exportada (el CLI validaba, la librería no). Verificación completa verde en `9ae5dde`: build/vet/test/race/tidy/diff-check; certificado re-emitido con pin de baseline: `M4_CERTIFIED_NON_LIVE` 27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 diferidos; negativos del harness comprobados (baseline incorrecto → `M4_STALE_CERTIFICATION`; `--skip-suites` → `M4_PARTIAL` 14 in-scope NOT_RUN). Único pendiente de aceptación: resolución del manager `M4_MANAGER_ACCEPTED_NON_LIVE`. Agent run: [[2026-09-19-zcode-glm-polymarket-m4-coverage-final]].
...
## 🚀 Research Strategies v0.1 — Sports + NegRisk

Iteración de investigación activa (mandato owner 2026-09-19) sobre el baseline certificado `9ae5dde` (`M4_CERTIFIED_NON_LIVE`): implementar dos estrategias experimentales como consumidores del engine, ejecutar SCREEN/REPLAY/SHADOW y producir scorecards para decidir qué hipótesis merece investigación posterior. Branch local `feature/research-strategies-v01` (sin push); LIVE_DISABLED intacto; outcomes `RS_V01_COMPLETE | RS_V01_PARTIAL_DATA | RS_V01_BLOCKED`.

**Nota de identidad:** el mandato citó `POC-S01: Sports / POC-S02: NegRisk`, pero la nomenclatura canónica vigente de este proyecto es `POC-S01 — NegRisk` y `POC-S02 — Sports Combinatorial` (D-010). Como el propio mandato prohíbe reasignar un ID existente a otra tesis, se conserva el mapeo canónico: **POC-S01 = NegRisk**, **POC-S02 = Sports**.

- **Estado de la iteración — `RS_V01_PARTIAL_DATA` (completada 2026-09-19):** las tres fases pasaron sus gates (A `7eb3532`, B `9d737ae`, C `0d6a635` en `feature/research-strategies-v01`, sin push) y el SHA final quedó **recertificado `M4_CERTIFIED_NON_LIVE`** (27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 diferidos). Ambas POCs implementan el contrato Strategy real, corren SCREEN consolidado aislado, y su SHADOW E2E llena por profundidad observada con paridad determinista de scorecards; los outcomes son honestamente `INCONCLUSIVE`. La capa de datos reales ejecutó su ciclo completo (sync Gamma + 45 s de Market WS del evento NegRisk 30829, 57–224 niveles reales) pero es `DATA_BLOCKED` con causa exacta: los constraints de régimen de esos mercados quedan fuera del techo offset≤2000 de Gamma y la exhaustividad no es verificable hoy — las estrategias se negaron a fabricar premisas y reportaron cero candidatos con razones. `PARTIAL_DATA`: implementación + certificación completas; validación sobre datos reales aptos queda para la siguiente iteración.
- **Fase A — Specs + Screen Composition (gate `RS-V01-A_PASS`):** congelar las dos SPEC de experimento (hipótesis, mecanismo, claim falsable, UniverseSpec, DataRequirements, parámetros versionados, detección, payoff, elegibilidad, exclusiones, controles negativos, métricas, unidad estadística, criterios de parada, INCONCLUSIVE y criterios ex ante); componer el screen runner existente para N instancias aisladas (factories registradas, params por instancia, mismo frame consumido sin mutación) y un agregador/manifest liviano de resultados consolidados; demostrar con fixtures dos instancias, manifests distintos, estado aislado, métricas distinguibles, cero acceso prohibido (gate G-15b) y suite verde.
- **Fase B — Dos estrategias (gate `RS-V01-B_PASS`):** implementar `POC-S01` (NegRisk complete-set) y `POC-S02` (pre-match reversion) sobre el contrato Strategy real (M1.8), con lineage completo por oportunidad, Economics para cotización, reasons tipadas, controles negativos y tests obligatorios por POC; suite global + race verdes.
- **Fase C — Replay, Shadow y certificación (gate `RS-V01-C_PASS` o `RS-V01_PARTIAL` con causa):** SCREEN sobre fixtures + datos reales si existen y son elegibles; REPLAY determinista desde manifest cerrado (paridad de hashes, `NOT_REPRODUCIBLE` sin evidencia); SHADOW E2E con Simulator + Account virtual en modo `INDEPENDENT` (al menos una POC, ambas si hay datos aptos); scorecards independientes por POC; regresión completa (`build/vet/test/race/tidy/diff-check`), piso de cobertura ≥95% por paquete también en los paquetes nuevos, y certificación con el harness existente pineando el SHA final.

### SPEC POC-S01 — NegRisk Complete-Set (frozen v0.1)

- **Hypothesis:** `PE-002` (familia MVP del Edge Research Consolidado 2026-09-16: full basket y conversión permiten monetizar desigualdades entre outcomes; esta revisión investiga la estructura económica del complete-set SIN conversión habilitada).
- **Mechanism:** en eventos NegRisk con outcomes exhaustivos y relaciones verificadas, pueden existir instantes en que el coste ejecutable de comprar YES de todos los outcomes es inferior al payout conjunto ($1 por conjunto bajo conversión), después de fees y riesgos de ejecución.
- **Falsifiable claim:** existen eventos elegibles con `Σ sweep_ask(YES_i, size) + fees < $1 − min_edge` con profundidad observable compatible; se falsa si tras costes completos el net edge ≤ 0 en todos los eventos elegibles observados, o si la elegibilidad exige premisas no verificables.
- **UniverseSpec:** eventos NegRisk declarados por el experimento (`event_id` + lista canónica de assets); DataRequirements: books `OBSERVED_USABLE` por cada leg, fechas de metadata acotadas, ≥2 assets.
- **Parámetros versionados v1:** `event_id`, `assets` (coma, orden canónico), `membership` (VERIFIED|UNKNOWN), `exhaustiveness` (VERIFIED|UNKNOWN), `other_present` (YES|NO|UNKNOWN), `fee_bps` o `fee_low_bps`/`fee_high_bps` o `fee_unresolved`, `min_edge_bps`, `max_size_per_leg`, `protocol` (CTF|V2|UNKNOWN). Procedencia exigida: los estados de relación provienen de `catalog inspect` (provenance del sync); en fixtures son entradas controladas del experimento. Cablear el consumo directo del catálogo en el frame queda como siguiente experimento (no como deuda silenciosa).
- **Elegibilidad (todo requerido):** protocol conocido ≠ UNKNOWN; membership VERIFIED; exhaustiveness VERIFIED (sin ella no hay demostración de payoff del conjunto); `other_present` = NO (YES → inelegible por semántica mutable; UNKNOWN → UNSUPPORTED); fee POINT o INTERVAL defendible (UNRESOLVED → INCONCLUSIVE); todos los legs presentes con book usable; sizes compatibles con profundidad.
- **Detección:** condición preliminar del basket con best asks por leg (suma de asks < 1); la profundidad jamás se asume desde best asks aislados — la evaluación usa niveles observados.
- **Payoff/Evaluación:** sweep `economics.WalkSide` BUY por leg sobre los niveles observados del frame + fee vía `economics.BuildQuote` (interval respetado); net edge = 1 − Σ(cost+fee) por unidad de conjunto; size grid acotado por `max_size_per_leg` y por la profundidad mínima común; `CONDITIONAL_UNEXECUTABLE` como reason en todo ACCEPT (la monetización exige conversión NegRisk deshabilitada — jamás se simula como operación disponible).
- **Exclusiones/controles negativos:** relación UNKNOWN, exhaustividad UNKNOWN, Other presente, fee unresolved, profundidad insuficiente, una leg parcialmente ejecutable, cambio de régimen (book suspect/stale), payoff incompatible (suma de asks ≥ 1 tras costes), orden alterado de outcomes (orden canónico obligatorio), dos instancias independientes con params distintos.
- **Métricas:** `sets_observed`, `eligible_sets`, `candidates`, `accepted`, `rejected`, `inconclusive`, `net_edge_bps` (por candidato), `fee_mode`, `depth_limited`. Unidad estadística: el conjunto (evento) — jamás legs sueltas.
- **Parada/INCONCLUSIVE:** parada ex ante al alcanzar `min_samples` conjuntos elegibles observados o al agotar el dataset; INCONCLUSIVE si fee no resoluble, coverage parcial, o falta cualquier premisa de payoff.
- **Criterio ex ante de avance:** net edge positivo sostenido con fee resuelta y depth observable en ≥ `min_samples` conjuntos → siguiente gate de investigación; nunca permiso live (conversión deshabilitada).
- **Resultado v0.1 (`RS_V01_PARTIAL_DATA`):** implementada en `internal/strategy/pocs/negrisk`, screen/REPLAY/SHADOW certificados sobre fixtures con fills y paridad determinista; en datos reales (evento 30829) `DATA_BLOCKED` con causa exacta: constraints de régimen fuera del techo offset≤2000 de Gamma y exhaustividad no verificable hoy. Siguiente acción: iteración de datos (sync por ids de mercado o keyset + verificación de exhaustividad del evento) y re-ejecución del experimento preregistrado sobre journal real apto.

### SPEC POC-S02 — Sports Pre-Match Reversion (frozen v0.1)

- **Hypothesis:** `PE-005-R1` (revisión identificada de la familia `PE-005` "shocks de liquidez sin noticia revierten tras recuperar depth", registry de experimentos, con trazabilidad al parent; la tesis canónica original de POC-S02 `PE-001` combinatorial permanece registrada para una iteración posterior).
- **Mechanism:** en mercados deportivos pre-match, ampliaciones de spread frente a una referencia histórica reciente revierten parcialmente dentro de una ventana definida; la reversión sólo es explotable si el net edge sobrevive costes taker completos.
- **Falsifiable claim:** la fracción de señales con reversión ≥ breakeven dentro de la ventana supera la requerida por `min_net_edge_bps` con ejecución taker plausible; se falsa si la reversión desaparece al descontar costes o si no es distinguishable del bid-ask bounce instrumental (PE-019/PE-020 como gates de calidad, no como señal).
- **UniverseSpec:** subconjunto acotado de mercados deportivos declarado por el experimento (identidad verificable, rules documentadas, kickoff verificable, lifecycle compatible, books elegibles); no se mezclan deportes/tipos de mercado con reglas distintas en una población sin declararlo. DataRequirements: 1 asset usable, serie histórica mínima (`ref_frames` observaciones previas).
- **Parámetros versionados v1:** `kickoff_ms` (instante de inicio verificable; vacío → señales bloqueadas WAITING_DATA — jamás se sustituye por endDate editorial o hora actual), `window_ms` (ventana de reversión), `ref_frames` (lookback de referencia), `widen_min_bps` (ampliación mínima vs referencia), `entry_budget` (notional máximo por entrada), `min_net_edge_bps`, `taker=true` (v0.1 conservador; maker → UNCALIBRATED sin evidencia de calibración).
- **Detección:** referencia = mediana de los spreads observados en las `ref_frames` observaciones previas del asset (sólo datos anteriores al corte actual — sin look-ahead); señal si spread actual ≥ referencia + `widen_min_bps`, book usable, kickoff conocido y `signal_time < kickoff`. Estado por instancia conserva ventana histórica, instante de señal, spread, referencia y calidad; los datos posteriores a la señal jamás entran a la detección.
- **Payoff/Evaluación:** coste de entrada taker = sweep del lado ask sobre niveles observados + fee (unresolved → INCONCLUSIVE); breakeven_bps = estrechamiento mínimo del spread que cubre costes de round trip; ACCEPT (elegible para medición) sólo si `breakeven ≤ widen_min_bps` objetivo declarado y net edge ≥ `min_net_edge_bps`; REJECT con `REVERT_INSUFFICIENT_AFTER_COSTS` cuando el coste mata el spread — control negativo explícito.
- **Medición de reversión (sin fills inventados):** la instancia acumula señales pendientes; con frames posteriores (sólo después de la señal, dentro de `window_ms` desde `signal_time`) registra `reverted` si spread ≤ breakeven_bps, `not_reverted` si la ventana expira sin reversión, `window_open` mientras pendiente. En SCREEN no existe fill ni PnL: los contadores son medición, la reversion observada no equivale a fill demostrado.
- **Exclusiones/controles negativos:** timestamp futuro (frame posterior a kickoff → sin nuevas señales), kickoff inválido/desconocido (bloqueado), join ambiguo (asset duplicado en universo → config error), book stale/suspect, frame incompleto, cambio de kickoff (nueva revisión del experimento), señal posterior al corte (imposible por construcción de la instancia), reversión que desaparece por costes (REJECT), reversion anterior a llegada simulada (dominio del Simulator en SHADOW), mercados sin resolución (exclusión declarada), duplicados del mismo evento (dedup por asset). Partición estadística por evento y tiempo; el mismo evento jamás es entrenamiento y holdout simultáneos.
- **Métricas:** `signals_total`, `signals_independent`, `reverted`, `not_reverted`, `window_open`, `rejected_by_costs`, `entry_vwap`, `breakeven_bps`, `spread_bps`, `ref_bps`. Unidad estadística: la señal por (asset, evento, tiempo).
- **Parada/INCONCLUSIVE:** parada ex ante al alcanzar `min_samples` señales resueltas o al agotar el dataset; INCONCLUSIVE si kickoff no verificable, serie insuficiente, fee no resoluble o coverage parcial.
- **Criterio ex ante de avance:** tasa de reversión ≥ breakeven sostenida OOS-particionada por evento con costes completos → siguiente gate de investigación; nunca permiso live.
- **Resultado v0.1 (`RS_V01_PARTIAL_DATA`):** implementada en `internal/strategy/pocs/sports`, screen/REPLAY/SHADOW certificados sobre fixtures con fills y paridad determinista; en datos reales `DATA_BLOCKED` por contrato: sin kickoff verificable (Gamma startDate es editorial sin contrato de equivalencia) y sin constraints de régimen alcanzables, la estrategia bloquea toda señal en vez de adivinar. Siguiente acción: contrato de horario de inicio verificable por mercado deportivo (join versionado evento↔mercado) y dataset deportivo con constraints alcanzables; el roll-out maker requiere evidencia de calibración de cola.

## 🚀 Research v0.3 — NegRisk Validation + Sports Continuous Capture

Iteración de validación (mandato owner 2026-09-19) sobre baseline `7bd264d` (branch `feature/research-strategies-v01`): resolver los bloqueos restantes de datos de las dos POCs sin modificar hipótesis, con lectura on-chain pública autorizada (sólo lectura) y captura continua deportiva. Sin nuevas estrategias, sin rediseño, LIVE_DISABLED, sin push. Gates: A `ea1e3c8` (V03_PREFLIGHT_PASS), B `cc57e8d`, C `4189a9b`, D higiene+cert `eaf2d94`; árbol final `f070496`. Estado final `RS_V03_PARTIAL_DATA`: POC-S01 NegRisk línea COMPLETA (validada sobre evento estándar con membresía on-chain demostrada), POC-S02 Sports PARCIAL (fase 1 de captura completa; ventana pre-match completa preregistrada y pendiente de ejecución).

- **Fase A — Preflight y calidad (gate `V03_PREFLIGHT_PASS`):** flake de catalog reproducido (2/4 rondas del suite completo con race) y corregido por causa raíz — el test cancelaba el contexto en carrera contra `RunScan`; el fixture ahora notifica cada request servido y la cancelación es causalmente posterior al inicio del walk (3/3 rondas verdes). Flake adicional descubierto y corregido por causa raíz en supervisor (`TestShutdownDrainDeadlineExceeded`, TOCTOU con el writer libre): seam aditivo `supervisor.Config.CaptureCfg` (nil-preserving) + coreografía determinista con fault hook (10/10 con race). Cobertura `cmd/engine` 94.4%→**98.24%**: 57 statements cubiertas con tests de fallo deterministas y útiles (esquemas hostiles, contenido proyectado corrupto, bundles manipulados, agotamiento de presupuesto de frames); las 26 restantes clasificadas con inaccesibilidad demostrada por bloque (`testdata/research-v03/COVERAGE.md`) — la excepción `DEFENSIVE_FAULT_GUARD` de v0.2 queda resuelta sin autoaprobación (la narración v0.2 subestimó: el residuo real eran 83 statements). Erratum `manifest_id` de v0.1 verificado: evidencia histórica intacta, erratum vigente en los bundles. Journal real v0.2 preservado fuera de rutas temporales en `polymarket-engine-datasets/rs-v02/` (47 archivos, SHA-256 por archivo, frontier 1429, replay desde el destino con digest `b24982b0…` idéntico en {1} y {32,7,1}, originales intactos).
- **Fase B — NegRisk estándar (POC-S01, PE-002 congelado, evento ancla 159954 "What will the Fed rate be at the end of 2026?", 15 miembros):** B1: walk de 2000 eventos activos → 788 `negRisk=true`, **685 aumentados** (un mercado Other dentro del conjunto — patrón 30829 generalizado: 87% de los NegRisk vivos), 103 estándar, **82 elegibles** tras el tamiz automático; hallazgo de wire: `negRiskOther` NO existe a nivel evento (el tamiz válido es por miembro). B2: lectura pública block-pinned — chainId `0x89` en dos RPCs independientes, bloque `0x59bede5` con hash cruzado, direcciones desde `docs.polymarket.com/resources/contracts`, adaptador legado `0xd91E80cF…` sigue siendo la autoridad de membresía del evento → `getQuestionCount(ctx)=15` == 15 miembros (**ONCHAIN_MEMBERSHIP_MATCH VERIFIED**), CTF `getOutcomeSlotCount=2` ×15, `payoutDenominator=0` ×15; re-check post-captura (`0x59bf457`) mantiene 15. B3: `PAYOUT_EXHAUSTIVENESS=UNKNOWN` honesto (resolución futura + semántica mutable de preguntas), separado explícitamente del match; la estrategia permanece con `exhaustiveness=UNKNOWN`. B4: captura real 1800 s (3764 frames in, 3585 admitidos, 0 rechazados; frontier 3591) → SCREEN 30 cortes: **21 oportunidades, 21 INCONCLUSIVE, 0 rechazos estructurales** (el bloqueo `EXHAUSTIVENESS_UNSUPPORTED_STRUCTURE` de v0.2 desaparece sobre evento estándar); REPLAY determinista {1}=={32,7,1} digest `33cbaffe…` 3647 records `not_reproducible=0`; SHADOW 923 frames, **663 conjuntos observados, 663 evaluaciones, 0 rejected, 663 INCONCLUSIVE**, 0 delivery errors, 0 fills, outcome INCONCLUSIVE por muestra preregistrada no alcanzada.
- **Fase C — Sports continuo (POC-S02, PE-005-R1 congelado, partido ancla 494880 Rays vs Yankees, moneyline 2284198):** kickoff verificado `gameStartTime == event.startTime = 2026-09-22T17:05:00Z` (contrato v0.2; el wire difiere sólo en formato); libro vivo 0.42/0.43 con ~$112k en el mejor bid. Captura fase 1 (ventana preregistrada 21600 s, T-64.9h→T-58.9h): 8844 frames in, 6685 admitidos, 0 rechazados, ambos lados **OBSERVED_USABLE** (46 niveles), frontier 6748 contiguo, sin gaps. Pipeline congelado: SCREEN 56 ventanas → **0 señales** (`NO_SIGNAL_OBSERVED_IN_WINDOW`, fase temprana tranquila); REPLAY determinista {1}=={32,7,1} digest `110381d7…` `not_reproducible=0`; SHADOW 923 frames, 0 oportunidades, outcome INCONCLUSIVE (min_samples 100 no alcanzado). Ningún parámetro ajustado. La ventana pre-match completa (T-8.5h→kickoff, 30600 s) está preregistrada como runbook mecánico (`testdata/research-v03/SPORTS-CAPTURE-RUNBOOK.md`) pendiente de ejecución.
- **Addendum 2026-09-21 — E2 (otros MLB, no 2284198):** muestra preregistrada por kickoff (4584879, 4584889, 4584903, 4613496), captura 1800 s en HEAD `85e27ff`, journal congelado antes de SHADOW. Exit **`NO_SIGNALS_IN_SAMPLE`**: 0 señales en baseline y variante `widen_min_bps=25`. Tres mercados de esta noche `SUSPECT` por `fee_source_discrepancy`; 4613496 `OBSERVED_USABLE` 1-tick. No es NO_GO global. Detalle `pe005-r1-e2-20260921/REPORT.md`.
- **Fase D — Integración y certificación:** regresión completa verde (build, vet, gofmt, **suite completa 28/28 con `-race`** — incluidos los dos paquetes con flakes corregidos); cobertura de paquetes modificados ≥95% (cmd 98.2, catalog 95.1, supervisor 95.9); certificado **`M4_CERTIFIED_NON_LIVE`** (27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 diferidos live por diseño) emitido con `--baseline eaf2d94` y re-pineado en el árbol final `f070496`; prueba negativa del harness verificada (baseline ajeno ⇒ `M4_STALE_CERTIFICATION`); prueba física de `vcs.revision`: binario construido reporta `commit=f070496…` == HEAD. Journals v0.3 preservados en `polymarket-engine-datasets/rs-v03/` (41 archivos, verificación desde el destino, digests reproducidos, originales intactos); los directorios de trabajo `.rs-v03-*` quedaron no-trackeados según la convención v0.1/v0.2.
- **Observación de entorno:** durante la captura, un actor externo del checkout compartido (sesión paralela/autosync) creó y pusheó al remoto el commit `25f578a` (checkpoint mid-capture que arrastraba archivos de trabajo de esta iteración); esta ejecución **no hizo push**, el árbol certificado vive sólo local (ahead 5 del remoto) y los binarios de trabajo fueron des-trackeados en `eaf2d94`. Higiene gofmt aplicada a 2 archivos preexistentes ya desalineados en el baseline `7bd264d`.

## 🚀 Research Data Unblock v0.2 — NegRisk + Sports

Iteración de datos (mandato owner 2026-09-19) sobre baseline `0d6a635` (branch `feature/research-strategies-v01`, recertificado `M4_CERTIFIED_NON_LIVE`): preservar la evidencia real de v0.1, desbloquear los datos que dejaron la capa real en `DATA_BLOCKED` (constraints de régimen fuera del techo offset≤2000 de Gamma, exhaustividad NegRisk no verificable, kickoff Sports sin contrato) y reejecutar las dos POCs preregistradas sin modificar hipótesis ni parámetros estadísticos. Sin rediseñar el engine, sin nuevas estrategias, LIVE_DISABLED. Decisiones manager aprobadas: POC-S01=NegRisk (PE-002), POC-S02=Sports (PE-005-R1), PE-001 conserva identidad histórica, Gamma ampliable sólo con consultas públicas read-only verificadas, kickoff Sports autorizado; erratum de nomenclatura de v0.1 ya registrado en la nota de identidad de arriba. Outcomes `RS_V02_DATA_READY | RS_V02_PARTIAL_DATA | BLOCKED`; secuencia Fase A → checkpoint → Fase B → checkpoint → Fase C → checkpoint → reporte; sin autorizaciones intermedias salvo conflicto con contratos frozen, fuente nueva que requiera autorización o seguridad.

- **Fase A — Evidencia e identidades (gate `DATA_A_PASS`):** preflight (SHA, árbol limpio, writers, suite); bundle persistente del journal real `.rs-v01-real/` mediante copia íntegra fuera de rutas temporales con manifiesto (rutas explícitas, SHA-256 por archivo, clasificación RESEARCH_EVIDENCE, retención, referencias a `testdata/research-v01/` y esta nota), frontera durable verificada con `journal verify` sobre la COPIA y replay físico comparando digests contra los comprometidos; sin mover ni eliminar originales hasta verificar el bundle. Matriz de identidades: NegRisk 30829 (event/market/condition/asset IDs, revisiones de relaciones, membresía, evidencia o ausencia de exhaustividad, constraints faltantes, cobertura y provenance) y Sports (evento/mercado, deporte, game ID, kickoff disponible, fuente y semántica, estado del join, revisiones y cobertura), clasificando `NOT_OBSERVED | UNKNOWN | CONFLICT | UNSUPPORTED`; sin consultas masivas nuevas.
- **Fase B — Capacidades mínimas de datos (gate `DATA_B_PASS` | `DATA_B_PARTIAL` con contraejemplo):** B1 Gamma dirigido GET-por-ID para recuperar sólo las entidades requeridas reutilizando adapter/parsers/Capture/Catalog (`HTTP → Protocol → Capture durable → Catalog/Regimes`, sin catálogo paralelo; verificar que las respuestas por ID entregan las entidades y relaciones correctas). B2 contrato keyset (`/events/keyset`, `/markets/keyset`, cursor, envelope, terminación, filtros, errores) contrastado OpenAPI + wire real read-only; adapter keyset mínimo dentro del ownership Gamma/Protocol/Catalog con fixtures (multipágina, cursor repetido, página repetida, cursor inválido, cambio de cohort, timeout, respuesta parcial, recovery, dedup, terminación) sólo si queda demostrado; si no `KEYSET_UNVERIFIED` sin bloquear B1. B3 prueba explícita de exhaustividad NegRisk combinando estructura estándar/aumentada, rules y payout, conjunto de resultados, identidad y relaciones, placeholders y Other; jamás VERIFIED por `negRisk=true` ni por número de mercados; `EXHAUSTIVENESS_UNKNOWN` si no hay evidencia (conversión DISABLED). B4 kickoff Sports: auditar `event.startTime`, `market.gameStartTime`, identificadores de partido y endpoints públicos de games; sólo campos documentados o demostrados empíricamente; join exacto o con relación explícita verificable; sin convertir `startDate` editorial en kickoff; `KICKOFF_UNVERIFIED` si el contrato no se puede verificar.
- **Fase C — Experimentos reales:** conectar estrategias al Catalog/Universe por interfaces públicas eliminando la dependencia de params manuales derivados de `catalog inspect` cuando exista canal canónico suficiente; hipótesis y parámetros estadísticos preregistrados intactos; por POC `Universe → SCREEN → REPLAY → SHADOW → Scorecard` sobre journal real apto: NegRisk exige exhaustividad y constraints verificables antes de evaluar payout (si no `INCONCLUSIVE` con disponibilidad medida sólo dentro del universo cubierto, jamás `NO_SIGNAL` sobre universo incompleto, conversión nunca simulada como habilitada); Sports exige kickoff y join válidos antes de admitir señales (reversión medida sólo con observaciones posteriores al instante de señal para outcome y jamás para Detect; breakeven/costes/llegada simulada; sin edge con muestra insuficiente). Evidencia: manifests, hashes, datos y ventanas, coverage, revisiones, resultados SCREEN, digests REPLAY, fills virtuales, scorecards, razones de inconclusión y controles negativos; si los datos no alcanzan, documentar el requisito de muestra exacto pendiente.

- **Estado de la iteración — `RS_V02_PARTIAL_DATA` (completada 2026-09-19):** las tres fases pasaron sus gates (A `54b68b7`, B `d9fab99`, C `e1134f1`; certificado re-emitido `M4_CERTIFIED_NON_LIVE` en el árbol final `7bd264d`, 27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 diferidos, LIVE_DISABLED intacto). Capa de datos desbloqueada y demostrada extremo a extremo sobre journal real `.rs-v02-real/` (frontier 1074): sync dirigido por IDs del evento 30829 (129 entidades, membresía ×128 VERIFIED, constraints de régimen observados), contrato keyset demostrado + adapter mínimo, kickoff Sports verificado y proyectado en catálogo, canal canónico `catalog emit-params`. Resultados honestos: NegRisk `INCONCLUSIVE` con causa estructural exacta (el canal descubrió que el mercado 559779 "another person" declara `negRiskOther=true` — conjunto con Other, inelegible por semántica mutable preregistrada; y la exhaustividad no es demostrable pre-resolución sin `getQuestionCount` on-chain, maquinaria demostrada sobre el hermano resuelto 903193); Sports `INCONCLUSIVE` por muestra insuficiente con requisito de captura documentado. Defecto real corregido: books multi-sesión (cursor por asset testifica max, regresión test). Evidencia completa en `testdata/research-v02/` (IDENTITIES, DATA-CAPABILITIES, EXPERIMENTS, certificado).
...
- [ ] FASE B4: kickoff Sports verificable (o `KICKOFF_UNVERIFIED` con causa exacta) #owner/me #type/research #area/personal
- [x] FASE C: canal canónico `catalog emit-params` + `Universe→SCREEN→REPLAY→SHADOW→Scorecard` por POC sobre journal real apto (`e1134f1`) #owner/me #type/dev #area/personal
- [x] Quality completo + certificado `M4_CERTIFIED_NON_LIVE` re-emitido en el árbol final (`7bd264d`); `LIVE_DISABLED` intacto; excepción de cobertura cmd/engine (14 statements DEFENSIVE_FAULT_GUARD) declarada pendiente de owner #owner/me #type/dev #area/personal
...
### Five-POC Golden Baseline — integración y cierre (2026-09-20, manager)

- **Estado: `FIVE_POC_GOLDEN_BASELINE_READY_PENDING_OWNER_REVIEW`** — branch local `feature/five-poc-integration` HEAD `19ce64b` (baseline certificada `037c15d`, receipt pineado en el commit siguiente), descendiente de `feature/shared-poc-unblocker@9d0512a` ← `f070496`; sin push; `LIVE_DISABLED` intacto; resultados económicos históricos `INCONCLUSIVE` de S01/S02 preservados sin reescritura.
- **S01 NegRisk + S02 Sports Reversion:** regresión PASS sobre la baseline (31→34 paquetes); verticales engine-surface del harness F5 verdes (SCREEN conditional-accept CONDITIONAL_UNEXECUTABLE / bid shock, SHADOW `DECLARED_L2` con fills de depth observada, replay RESOLVED, determinismo por hash); `HYPOTHESIS_VALIDATED=NO`.
- **S03 Sports Combinatorial (PE-001):** implementación integrada @ `9917109` (paquete `internal/strategy/pocs/sportscombinatorial`, 9 archivos): proof `Cover(A,-h)⇒Win(A)`, F01–F22 con oráculos O1–O7, fee `SYNTHETIC_FEE_EXPLICIT` (`REAL_UNVERIFIED` rechazada por la factory; U-02 vigente), cobertura 95.2%; vertical F5-G09 verde (B0 pin ACCEPT con PROOF_VERIFIED+FEE_SYNTHETIC_EXPLICIT → shadow 2 legs → replay RESOLVED). `IMPLEMENTATION_PASS=YES / PIPELINE_PASS=YES (offline) / HYPOTHESIS_VALIDATED=NO`.
- **S04 Weather (PE-030):** implementación integrada @ `0630805` (paquete `internal/strategy/pocs/weather`, 19 archivos): F01–F22 con razones `WX_*`, adapter vía `external.AdmitSync`/`Load`/`Project` (SFG-04 consumido), modelo `wx-uniform-v1` UNCALIBRATED, replay MATCH, SHADOW `DECLARED_L2` con rechazos honestos y fills `FeeInconclusive`, cobertura 96.1%, race limpio; verticales WX-CASE ×5 en paquete + instancia en coexistencia F5-G12. `IMPLEMENTATION_PASS=YES / PIPELINE_PASS=YES (offline) / HYPOTHESIS_VALIDATED=NO`.
- **S05 Maturation O/B (PE-004):** implementación integrada @ `fa69adc` (paquete `internal/strategy/pocs/maturation`, 7 archivos): 22/22 fixtures, claves `pe004.*`, FrameObserver con observaciones durables incluidas las de cero oportunidades, `RunStrategyReplay` ×2 digest MATCH + schedule-invariance, SHADOW `DECLARED_L2` observation-only con 0/0/0 verificado, cobertura 99.7%; instancia en coexistencia F5-G12. `IMPLEMENTATION_PASS=YES / PIPELINE_PASS=YES (offline) / HYPOTHESIS_VALIDATED=NO` — descriptiva: cero candidates/orders/fills ES el diseño. Track **W sigue `BLOCKED_BY_SFG06`** (fuera de ruta crítica de este ciclo).
- **Cross-gates:** F5-G01 registry único (6 consumidores distinguibles: neutral, poc-negrisk, poc-sports, poc-sports-combinatorial, poc-weather, poc-maturation); F5-G02/G12 coexistencia de las cinco instancias sobre un mismo journal sin contaminación; F5-G03 causalidad (negativos future/leak/stale/revision en las tres POCs + SFG-04/01); F5-G04 determinismo (replay ×2 digest MATCH en las cinco + verticales con hash idéntico); F5-G05 economics parity (oracle `big.Rat` SFG-02 + fills DECLARED_L2 por depth observada); F5-G06 sin liquidez sintética (fallback BBO×10 ausente; ACCEPT sin legs ⇒ `ExecutionBlocked`); F5-G07 dataset guard (`ENGINE_PROTECTED_DATA_DIRS` refusado en G07 + paquetes, `t.TempDir` siempre); F5-G08 LIVE disabled (seam rechaza frozen/duplicados, outcome nunca GO, archtest + gates live `NOT_RUN` explícitos); F5-G09 cinco verticales demostradas (S01/S02 engine verticals, S03 F5-G09, S04 WX-CASE ×5, S05 receipts observation-only).
- **Quality:** `go build ./...` OK · `go vet ./...` OK · `go test ./...` 34/34 paquetes ok · `go test -race ./...` 34/34 ok · cobertura paquete: PE001 95.2%, PE030 96.1%, PE004 99.7% (residuos defensivos documentados en receipts) · **`M4 RECERTIFICADO`** sobre baseline `037c15d` con receipt propio: **27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 live diferidos** — `testdata/research-v04/certificate-v04.json`; wiring manager-owned (`screenFactories` + `experiment.RegisterShadowStrategy` + seam de verificación), cero módulos shared modificados.
- **Deferred:** `PE004_W` (SFG-06 residual: Catalog reducer→UniverseChanged→replay); fee venue `REAL_UNVERIFIED` (sólo `SYNTHETIC_FIXTURE`); datos reales `REAL_DATA_BLOCKED` (bundle RS v0.3 sin certificar cierre); LIVE/órdenes/wallet; merge a `feature/research-strategies-v01` y push (decisión owner).
- **REFACTOR_CANDIDATES (registrados, NO ejecutados):** gofmt drift pre-existente en ~14 archivos de la línea shared (commits SFG sin formatear); doble superficie de registro de estrategias (`screenFactories` vs resolver de experiment); serialización testdata A2 de PE-004 pendiente; adapter Catalog O (A3) de PE-004 pendiente.
- **OWNER_ACTION_REQUIRED:** Review humana del golden baseline (no autoaceptable por el manager); decisión de merge/push; aceptación de receipts de las tres POCs y del harness F5 de gates (co-desarrollado entre la sesión paralela del owner y el manager: verticales y journal fixtures por aquélla, wiring de registro y seams por éste).
- **NEXT:** auditoría post-implementación de arquitectura five-POC + refactor (mandato separado).

### Five-POC Research-Ready — hardening de investigación (2026-09-20, manager)

- **Estado: `FIVE_POC_RESEARCH_READY`** — misma branch `feature/five-poc-integration`, baseline final `c977447` (HEAD `cb549c7`, receipt M4 pineado en el commit siguiente); sobre `FIVE_POC_GOLDEN_BASELINE_READY` (`037c15d`, que sigue siendo la baseline funcional certificada v04). Sin push; `LIVE_DISABLED` intacto; deuda previa sin cambios (PE004 W/SFG-06, fee real U-02, weather real, calibración, live).
- **P0 clock seam (defecto demostrado corregido):** el dispatcher derivaba `VirtualTime` del reloj de pared → fixtures históricos `STALE_BOOK` (fallo real de la integración). Los STAMPS (`VirtualTime`/`RequestedAt`/`CompletedAt`) ahora viajan por un seam explícito `frames.ReceiveClock` sobre la receive timeline del journal (`WithStampClock`/`SetStampClock`); los deadlines de corte siguen en reloj real. Fixtures ahora genuinamente históricos (receive wall == source ms del reloj de fixture, sin `time.Now()`). Canary: `TestClockSeamHistoricalFixtureStaysFresh`; determinismo de stamps entre runs: `TestClockSeamStampDeterminismAcrossRuns`.
- **P1 factory resolution única:** `cmd/engine/strategies.go::strategyCapabilities` es la ÚNICA fuente — SCREEN, screen-consolidated, SHADOW y strategy replay derivan de una tabla (`New`+`Defaults`+`DefaultRun`+`Shadow`); una POC nueva = una entrada. (Consume el REFACTOR_CANDIDATE "doble superficie de registro".)
- **P2 mutation drills:** 5/5 POCs con BASE/VARIANT vía sólo `--param` (`core_files_changed: 0`): S01 `min_edge_bps` 50→5000 (accepted 919→0), S02 `widen_min_bps` 150→100000 (1→0), S03 `min_worst_net` 0→100000 (5→0), S04 `fee_rate` 0.05→0.6 (1827→0, fee_bps 395→4740), S05 Q-grid+filtro (920 observaciones con digests distintos; `windows_s` sigue restringido a {60,300,3600} por validación de factory — documentado). Evidencia durable + comparaciones: `testdata/research-v05/experiment-drills/` (DRILLS.md + base/variant/compare.json por POC).
- **P3 operabilidad:** nuevo `engine fixture fivepoc --kind vertical|s03|combined` materializa los journals canónicos (deterministas, SFG-07 guarded, nunca sobrescribe); flujo de operador nuevo verificado de punta a punta con la guía corregida (fixture→screen→shadow×2→compare→manifest→replay 75/75 RESOLVED). `NEW_AGENT_CAN_RUN_WITHOUT_REDISCOVERY = YES`.
- **P4 artifact contract:** scorecards y reportes screen llevan `engine_sha`, `mode`, `dataset_digest` (digest de routing order compartido screen/shadow; `content_hash` intacto — paridad G-07b preservada); `engine experiment compare <a> <b>` responde "¿qué cambió?" desde artifacts (parámetros, contadores, métricas, hashes). Guía operativa actualizada (fixture, compare, superficie de configuración SAFE/USUALLY/DO_NOT/REQUIRES por POC).
- **Gates nuevos:** F5-G13 RESEARCH MUTABILITY · F5-G14 OPERATIONAL DISCOVERABILITY · F5-G15 ARTIFACT COMPARABILITY (`go test ./cmd/engine/ -run 'TestF5G13|TestF5G14|TestF5G15'`) — verdes.
- **Quality:** build OK · vet OK · `go test ./...` 34/34 · `go test -race ./...` 34/34 · archtest incluido (34/34) · **`M4 RECERTIFICADO` @ `c977447`: 27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 live diferidos** — `testdata/research-v05/certificate-v05.json`.
- **OWNER_ACTION_REQUIRED:** misma revisión humana pendiente del golden baseline, ahora sobre el rango `19ce64b..cb549c7` (delta research-ready); decisión de merge/push.

### Five-POC Final Acceptance — aceptación técnica y handoff de investigación (2026-09-20, manager)

- **Estado: `FIVE_POC_FINAL_ACCEPTANCE_READY`** — branch local `feature/five-poc-integration`, código final `1bcae43` (HEAD `c915c11` = receipt `certificate-v06.json` pineado a `1bcae43`, sin código); delta sobre `cb549c7` = 4 commits (adapter A3 PE-004 `5d82312`, regresiones semántica S01/S04 + DRILLS `7b83ea6`, scope de determinismo en DRILLS `1bcae43`, certificado `c915c11`); sin push; `LIVE_DISABLED` intacto.
- **Gate A — integridad:** `cb549c7` limpio; `c977447` ancestro; delta `c977447→cb549c7` exclusivamente `certificate-v05.json` (cero código); el certificado v05 correspondía al código real — tras los correctivos se RECERTIFICÓ en el SHA final (ninguna herencia de certificación entre árboles distintos).
- **PE-004 O/B (Gate B) — `READY_WITH_CORRECTIONS`:** cohortes O y B IMPLEMENTADAS y demostradas de punta a punta: O exige `known_at_ms` (ancla `CATALOG_FIRST_OBSERVED`, jamás adivinada) y B lo PROHÍBE (ancla `FIRST_USABLE_BOOK_OBSERVED`); observaciones durables `pe004.*` con digest por frame, 0 opportunities/0 orders por diseño, replay ×2 schedules digest idéntico, compare O↔B nombra `{cohort, known_at_ms}`. **A2 = `OPTIONAL_IMPROVEMENT`** (los 22 fixtures existen y corren 22/22 como tests con expectativas y digests; falta sólo la serialización `testdata/pe004/` — no impide O/B). **A3 = `MISSING_REQUIRED_FUNCTIONALITY` → CORREGIDO:** adapter `CatalogFirstKnownAnchor` (`internal/strategy/pocs/maturation/catalog_anchor.go`) read-only as-of sobre `catalog.Service.InspectEntity`: ancla = `first_known_at` durable, revisión tardía NO la mueve, `createdAt` declarado NUNCA antedata, rechazos fail-closed; tests contra `InspectEntity` real + double declarado (38/38 en el paquete). Track **W sigue `BLOCKED_BY_SFG06`**. `REAL_DATA_READY=NO`.
- **Semántica de resultados (Gate C) — sin doble conteo ni bug:** NegRisk: `accepted`=evaluaciones aceptadas (≤1 candidato/frame), `baskets_planned`=planes 1:1, `simulated_fills`=fills a NIVEL DE PATA (1838 intentos → 16 fills = 8×2 = `baskets_completed`; 911 canastas `RESIDUAL_HELD` con cero exposición consumida — verificado en store: 8 COMPLETED + 911 RESIDUAL_HELD, 16 legs NOT_SENT + 1822 PLANNED). Weather: el MISMO gate económico (`net>0`) opera en BASE y variante; el bloque `strategy_metrics` (`fee_bps`/`net`/`fee_total`) son hechos del ÚLTIMO bucket evaluado del último frame (merge last-wins), no agregados del run — por eso `accepted 0` con `fee_bps 4740 / net -1.0954` es legítimo (fee 12× escala lineal; notional dependiente del libro idéntico). Regresiones pinadas en ambos paquetes + DRILLS.md §"Semántica de los contadores reportados".
- **Smoke operacional (Gate D) 5/5:** flujo de agente nuevo con la guía en directorios limpios (fixture → screen → shadow BASE + repetición → manifest/replay `RESOLVED` → VARIANT → compare): las cinco mutaciones nombradas exactas por `parameters_changed`, S05 con cohortes B y O, determinismo same-run-id verificado por `content_hash` idéntico, S01 `CONDITIONAL_UNEXECUTABLE` en screen (documentado), S02 ACCEPT `MEASUREMENT_ELIGIBLE` en cut 5.
- **Artifacts (Gate E):** identidad del run durable (`experiment_runs`: hypothesis/params/seed/outcome) + métricas (`experiment_scorecards`: content_hash, observation_digests) + provenance (`engine_sha`/`mode`/`dataset_digest` en scorecard y screen); `dataset_digest` estable entre regeneraciones del fixture; `compare` expone `parameters_changed`/`counters_differ`/`metrics_changed`. Determinismo aclarado: identidad de investigación determinista; contenedor journal lleva `capture_id`/`boot_id` aleatorios (sin efecto en experimentos).
- **Mutabilidad (Gate F):** `CORE_CHANGES_REQUIRED_FOR_NEXT_EXPERIMENT = NONE` en las cinco (drills y smokes con sólo `--param`; `windows_s` sigue restringido a {60,300,3600} — ampliarlo es research surface del paquete maturation, no core).
- **Quality (Gate G):** build/vet OK · `go test ./...` 34/34 · `-race` 34/34 · archtest verde · F5-G13/G14/G15 + SFG-07 verdes · negativo `M4_STALE_CERTIFICATION` re-verificado · **`M4 RECERTIFICADO @ 1bcae43`: 27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 live diferidos** — `testdata/research-v06/certificate-v06.json`.
- **Defectos de core REPORTADOS, NO aplicados (decisión owner):** (a) `notional_by_scenario` vacío en la ruta de patas (acumulador arranca `""` y `ParseDecimal("")` errora ⇒ la suma se salta; el notional movido queda auditable en `account_reserves`/`account_ledger`); (b) `reserve_held` siempre `""` (lee `Balances["held"]` pero las reservas viven en `HeldReserves`). Aplicarlos cambiaría `content_hash` de scorecards (inválida digests de drills v05) por ganancia cosmética: quedan como hallazgo con diffs propuestos en DRILLS.md.
- **OWNER_ACTION_REQUIRED:** revisión humana del rango `cb549c7..c915c11` (delta aceptación final); decisión de merge/push; destino de los dos defectos de core reportados.
- **NEXT_EXPERIMENT_PER_POC:** S01 barrido `min_edge_bps`/`max_size_per_leg` sobre el drill; S02 serie `widen_min_bps`/`window_ms` con fixture de más cortes; S03 pares con handicaps distintos + barrido `min_worst_net`; S04 contratos vía `contract_json` (buckets distintos) + dataset con vintages encadenadas; S05 ventanas alternativas (exige ampliar dominio en paquete maturation) y grid Q sobre capturas reales futuras.

### Five-POC Cierre Definitivo — correcciones económicas, Catalog O y baseline certificada (2026-09-20, owner)

- **Estado: `FIVE_POC_FINAL_CERTIFIED_BASELINE_READY`** — branch local `feature/five-poc-integration`: código final `56e8fac` (merges `fix/accounting-correctness` `1f924d3`+`bd7a5ed` y `feat/pe004-catalog-o-wiring` `eaa8154` sobre `c915c11`), evidencia v07 `c38f6c4`, HEAD `85e27ff` = receipt `certificate-v07.json` pineado a `c38f6c4`; delta `56e8fac..85e27ff` SIN cambios de código (sólo `testdata/research-v07/`); sin push; `LIVE_DISABLED` intacto; los dos defectos de core reportados en la aceptación quedan CORREGIDOS.
- **P0 `notional_by_scenario` CORREGIDO (`1f924d3`):** causa raíz = acumulador iniciado `""` (lexema no numérico): `ParseDecimal("")` errora y la rama de patas saltaba la acumulación silenciosamente (runs DECLARED_L2 quedaban con notional vacío) mientras la ruta legacy BBO ignoraba el error — contabilidad dependiente del orden de fills. Ahora arranca en la identidad aditiva `"0"` y toda corrupción del acumulador es `KindInvariantViolation` en ambas rutas. Efecto demostrado: el drill S01 publica `notional=170`, exactamente el notional que la auditoría v05 declaró "auditable en account_reserves/account_ledger"; S02 `24.9998`, S03 `18`, S04 `23.8`.
- **P1 `reserve_held` CORREGIDO (`1f924d3`):** causa raíz = leía `View.Balances["held"]`, clave inexistente (balances son `asset -> disponible`); el valor publicado fue siempre `""`. Definición semántica fijada: dinero reservado actualmente por el namespace (capital locks vivos, `account_reserves` status `HELD`/`PARTIALLY_CONSUMED`), suma decimal de `View.HeldReserves`, `"0"` sin reservas. Impacto Risk: reporte-only verificado (`riskPolicy`/`riskSnapshot` descartados en `RunShadow`; consumers = `compare.go` reporting). Forense v07: reservas se liberan 1:1 en la observación `FinalFill` (16 `RELEASED` + 0 `HELD` en S01); reserva viva sólo ante re-run del mismo run-id sobre el mismo dataset (dedup de fill keys, comportamiento documentado).
...
- **2026-09-21 — `HISTORICAL_DATA_ACQUISITION_RESULT` (addendum one-shot, LIVE_DISABLED, Sports Week intacto, OOS sellado):** adquisición verificada end-to-end de la cohorte MLB 2026-09-01 22:40Z desde `archive.pendulumflow.com/v3`: SHA-256 locales recalculados = hashes del publicador para las dos horas completas (`ee4c5add…` 866.445.402 B; `5cd4e6b1…` 595.930.005 B) cerrando `NOT_RECOMPUTED`; extracción independiente con DuckDB = multiset idéntico al explore previo (15.487/13.204/13.027/9.166 líneas, ventana congelada 21:10Z–22:40Z); import fresco + replay en binario `master 66486ac` = digests `COHORT` 4/4 con schedules `{1}` y `{32,7,1}` idénticos. Autoridad kickoff: CLOB `game_start_time` 22:40:00Z ×4 + statsapi 22:40:00Z ×4 Final (`KICKOFF_VENUE_DECLARED_POSTHOC`). Fee: `fee_rate_bps=0` en las 256 fills v3 del cohort (fee efectiva observada; economía sigue sin certificar). Hallazgo certificador nuevo: la reconstrucción L2 por deltas NO es confiable — 66–85% de los intervalos snapshot→snapshot con mismatch de BBA (142/216, 117/138, 82/98, 63/84); sólo los snapshots (~2–3 min) son estado L2 admisible (`L2_SNAPSHOT_ONLY`); el dato `SYNCING` del informe previo queda ratificado con métrica. Benchmark de persistencia sobre el mismo dataset (Parquet+DuckDB vs ClickHouse efímero; fases scan/reconstrucción/frames/sweep): digests canónicos fase-a-fase idénticos entre motores; DuckDB gana en todos los tiempos (0,03–0,34 s cohorte; 0,34–0,53 s en 80M filas de hora llena vs 1,35–1,69 s CH) con ingesta cero (attach) y menor RSS → **decisión: A Parquet+DuckDB** como persistencia de backtesting; Parquet raw durable como fuente hash-vinculada, derivados regenerables; TimescaleDB NO medible aquí (sin sudo/docker — exacto en addendum §benchmark). Proveedores: PendulumFlow verificado físicamente (CC BY 4.0; v3 desde 2026-08-18T06; espejo AG6 sin licencia — no redistribuir); PMData auditado documentalmente (Free 600→50/mes; Plus $49 uso personal; Enterprise PIT = afirmación no demostrada como dato adicional); polymarketdata.co no verificado físicamente. Sin compra necesaria hoy. Dataset durable: `xKoRx/polymarket-engine-datasets/hist-acq-20260921/` (MANIFEST + SHA256SUMS; 2,1 GB). Detalle y tabla correctiva: [[Research — Historical Data Acquisition ADDENDUM 2026-09-21]]. Pendientes owner: key PMData Free para comparar proveedores, pregunta PIT al vendor, congelar fee 0 bps, habilitar docker/sudo si se quiere medir TimescaleDB.

- **2026-09-21 — `HISTORICAL_DATA_PARTIAL` — piloto histórico PE-005-R1 (`56195c9` → recibo `de33d7d`, publicado, LIVE_DISABLED, campaña Sports no tocada):** cuatro moneylines MLB con kickoff 2026-09-01T22:40:00Z, leídas por predicado remoto desde PendulumFlow v3 (horas 21 y 22) sin guardar los parquet completos. El adapter escribe un journal Capture; replay `{1}` y `{32,7,1}` coinciden en cada mercado; la calidad con régimen es `SYNCING` porque esas horas no traen `tick_size_change` y no se fabricó el tick de Gamma consultado hoy. Describe (no es la estrategia) cuenta 0 ensanchamientos frente a la mediana propia con `widen_min_bps=50`. SCREEN `cuts=30` no corre: el lote supera `MaxInboxDepth` 128. Una grilla no congelada `cuts=400` y un shadow sobre el binario `56195c9` entregan 0 oportunidades, 0 fills, fee `UNRESOLVED`, outcome shadow `INCONCLUSIVE`. OOS 22:45Z sellado y no analizado. `ECONOMICS_UNCERTIFIED`. Continuidad §15 y [[2026-09-21-historical-research-m0]].
- **2026-09-21 — `ARMED_WAITING_NEXT_WINDOW` — PE-005-R1 fee identities + campaña Sports armada (`c3b1aa1` → recibo `bfa6eaf`, publicado, LIVE_DISABLED):** el tope Gamma/CLOB y la fee de `last_trade` dejan de tratarse como la misma magnitud. Trade dentro del cap → INTERVAL `[0, cap]`; fuera del cap → SUSPECT. `REAL_FEE_READY` sigue false. `d5ce263` no integrado. Canary 90 s en 4584879: ambos libros `OBSERVED_USABLE`, observación `"0"` @ seq 29, cuarentena 0, replay MATCH. Servicio user activo, `started_at` null, 91 moneylines en espera. Reloj de 7 días arranca en la primera captura T−90 (2026-09-21T21:05:00Z). Sin canal de alerta externo. Continuidad §14 y [[2026-09-21-pe005-r1-sports-week]].
...
- **2026-09-21 — `ENGINEERING_STAGE_CLOSED_RESEARCH_REPRODUCIBLE` — HARDENING + U-02 + DISCOVERY (HEAD `85e27ff` intacto, **cero código**, sin recertificar M4, sin push, LIVE_DISABLED):** U-02 entonces **`U02_PARTIAL`** — paridad USDC `e=1` vs docs/SDK; BUY-shares y `TRUNCATE_6DP` vs 5 dp documentados, no pareados; delay deportivo `seconds_delay=1`/`sd=1`; `/fee-rate` 1000 ≠ `fd.r`. Superado en unidad por el dictamen `V2_CASH_CONFIRMED` del mismo día (bitácora anterior). PE-001 SCREEN 41 series / 73 eventos / 7 pares / **0** semántico∩temporal / **0** oportunidades q=20; WNBA familia `RULES_CONTRADICT`; FIBA 863805 template OT OK pero PAST. S04 contratos NOAA KLGA/RJTT inventariados sin vintages. S05 `first_known_at` ≠ `createdAt` demostrado; ningún mercado nuevo en la ventana. Datasets pe001 78/78 y rs-v03 41/41 SHA intactos. Bundle `hardening-20260921/`. No alpha, no live. Detalle [[Polymarket Engine — Continuidad Five-POC 2026-09-20]] §10 y [[2026-09-21-polymarket-final-readiness]].

- **2026-09-21 — `PE001_E3_REALITY_CHECK` — `GO_RESEARCH` (HEAD integración `85e27ff`, sin push, LIVE_DISABLED):** par real WNBA event 986912 ML 4358151 / SP 4778073 identificado; H1 no falsificada (0 ACCEPT, `RULES_CONTRADICT`); implicación contractual no demostrada; U-02 parcial; bundle `pe001-reality-check-20260921`. No es permiso live ni `REAL_DATA_READY`. Detalle en [[POC-S03 — Sports Combinatorial]] y [[2026-09-21-pe001-reality-check]].

- **2026-09-20 — `FIVE_POC_FINAL_CERTIFIED_BASELINE_READY` — CIERRE DEFINITIVO DE LAS CINCO POCs COMPLETADO (código `c915c11` → `56e8fac`, evidencia `c38f6c4`, HEAD `85e27ff` = receipt `certificate-v07`, branch `feature/five-poc-integration`, sin push):** los dos defectos de core reportados por la aceptación quedaron corregidos con regresiones y el wiring Catalog de PE-004 cohorte O quedó conectado a la composición real. P0 `notional_by_scenario`: acumulador iniciado `""` (identidad aditiva faltante) hacía que `ParseDecimal("")` fallara en la rama de patas y la acumulación se saltara en silencio (contabilidad dependiente del orden de fills) — ahora arranca en `"0"` y la corrupción es `KindInvariantViolation` en ambas rutas; el drill S01 publica el `170` que el ledger ya decía. P1 `reserve_held`: leía `Balances["held"]` (clave inexistente; siempre `""`) — ahora suma decimal de `HeldReserves` (reservas vivas por namespace, `HELD`/`PARTIALLY_CONSUMED`), `"0"` sin reservas, reporting-only verificado; se liberan 1:1 con la observación `FinalFill`. P2 Catalog O: `cmd/engine/maturation_catalog.go` resuelve el ancla por `anchor_source=catalog` antes de congelar el manifest (adapter `CatalogFirstKnownAnchor` sobre `InspectEntity` real, sink de rechazo read-only, fail-closed en ausencia/contradicción/cohorte B/`createdAt`, provenance `anchor_source`+`known_at_ms` persistidos; modo fixture intacto; `CATALOG_WIRING_VERIFIED=YES`, `REAL_CATALOG_DATA_READY=NO`). Evidencia v07: 10 drills regenerados con receta idéntica — `dataset_digest` idéntico 10/10, contadores idénticos, obs digests byte-idénticos, `content_hash` cambia sólo por los campos corregidos (BEFORE/AFTER en DRILLS.md v07; v05 intacto) + drill S05O catalog-mode con determinismo en dataset fresco; hallazgo de migración documentado: los digests v05 se reproducen byte-idénticos en `c977447` con `--cuts 5` (default CLI), la deriva de digest era del materializador, no de estos fixes. Quality: build/vet/test/race/archtest 12/12, F5 gates + SFG-07 + LIVE_DISABLED PASS, **M4 RECERTIFICADO @ `c38f6c4` (27/0/0/5, `certificate-v07.json`)**, delta `56e8fac..85e27ff` sin cambios de código. Deuda no bloqueante registrada: capitalLock legacy `"5.1"` hardcodeado, gofmt pendiente en 3 archivos cmd/engine, dominio `windows_s`. Agent run: cierre definitivo five-POC (esta sesión).

- **2026-09-20 — `FIVE_POC_FINAL_ACCEPTANCE_READY` — ACEPTACIÓN TÉCNICA Y HANDOFF DE INVESTIGACIÓN COMPLETADOS (código `cb549c7` → `1bcae43`, HEAD `c915c11` = receipt `certificate-v06`, branch `feature/five-poc-integration`, sin push):** Gate A ratificó la baseline (`c977447` ancestro de `cb549c7`, delta exclusivamente `certificate-v05.json`, árbol limpio); Gate B resolvió la contradicción S05 (A2 = `OPTIONAL_IMPROVEMENT`: los 22 fixtures corren como tests, falta sólo serialización `testdata/pe004/`; A3 = `MISSING_REQUIRED_FUNCTIONALITY` → corregido con `CatalogFirstKnownAnchor` read-only as-of sobre `InspectEntity` real: first_known_at durable, revisión tardía no lo mueve, `createdAt` jamás antedata; cohortes O/B demostradas por CLI con anclas causales, observaciones durables y 0/0/0 por diseño; W intacta `BLOCKED_BY_SFG06`); Gate C cerró la semántica sin bug: NegRisk `accepted`/`baskets`/`fills` son unidades distintas (evaluaciones / planes 1:1 / fills nivel pata: 1838 intentos → 16 fills = 8 canastas COMPLETED, 911 RESIDUAL_HELD sin exposición, verificado en store) y Weather usa el mismo gate `net>0` en ambos runs con `strategy_metrics` frame-scoped last-wins (por eso variante `accepted 0` con `fee_bps 4740` es legítimo), con regresiones pinadas en ambos paquetes y DRILLS.md documentado; Gates D/E/F ejecutaron 5/5 smokes de agente nuevo (fixture→screen→shadow→replay RESOLVED→variante→compare, cohortes B y O, determinismo same-run-id, mutaciones nombradas exactas, `CORE_CHANGES_REQUIRED=NONE`) y auditaron artifacts/provenance (identidad en `experiment_runs`, métricas en `experiment_scorecards`, `dataset_digest` estable entre regeneraciones; contenedor con `capture_id`/`boot_id` aleatorios aclarado); Gate G integró los correctivos por cherry-pick (`5d82312` + `7b83ea6` + DRILLS `1bcae43`), 34/34 build/vet/test/race/archtest, y **RECERTIFICÓ `M4_CERTIFIED_NON_LIVE` @ `1bcae43` (27/0/0/5, `certificate-v06.json`)** con negativo `M4_STALE_CERTIFICATION` re-verificado. Dos defectos de core reportados sin aplicar (notional_by_scenario vacío en ruta de patas; reserve_held lee mapa equivocado) — decisión owner porque invalidarían digests v05. Agent run: aceptación five-POC (esta sesión).

- **2026-09-20 — `FIVE_POC_RESEARCH_READY` — HARDENING DE RESEARCH-READINESS COMPLETADO (baseline `19ce64b` → código `c977447`, HEAD `cb549c7`, branch `feature/five-poc-integration`, sin push):** P0 corrigió el seam de reloj demostrado en la integración (stamps de frame ahora por `frames.ReceiveClock` sobre la receive timeline del journal vía `WithStampClock`/`SetStampClock`; deadlines en reloj real; fixtures históricos deterministas sin `time.Now()`; canario `TestClockSeamHistoricalFixtureStaysFresh`); P1 consolidó la resolución de factories en una única tabla (`strategyCapabilities` — SCREEN/consolidated/SHADOW/replay derivan de ella; consume el refactor candidate de doble registro); P2 ejecutó los cinco mutation drills BASE/VARIANT sólo con `--param` (`core_files_changed: 0`; evidencia + compare en `testdata/research-v05/experiment-drills/`); P3 agregó `engine fixture fivepoc` (journals canónicos deterministas por CLI) y verificó el flujo de operador nuevo con la guía corregida; P4 añadió provenance a artifacts (`engine_sha`/`mode`/`dataset_digest` compartido screen/shadow, `content_hash` intacto) y `engine experiment compare`. Gates F5-G13/G14/G15 verdes; build/vet/34 paquetes/race 34/34; M4 RECERTIFICADO @ `c977447` (27/0/0/5, `certificate-v05.json`).

- **2026-09-20 — `RS_V03_PARTIAL_DATA` — MANDATO POLYMARKET RESEARCH v0.3 COMPLETADO (baseline `7bd264d` → final `f070496`, branch `feature/research-strategies-v01`, sin push):** Fase A resolvió los dos flakes por causa raíz (catalog cancel-race vía notificación del fixture; supervisor drain-deadline vía seam `CaptureCfg` + hook determinista) y elevó `cmd/engine` a 98.24% con el residuo clasificado por inaccesibilidad demostrada (excepción v0.2 resuelta); Fase B validó la línea NegRisk sobre el evento estándar 159954: tamiz automático (685/788 eventos NegRisk vivos son aumentados; 82 elegibles), lectura on-chain pública block-pinned con `getQuestionCount(ctx)=15` == 15 miembros (membership match VERIFIED; payout exhaustiveness honesto UNKNOWN), captura real 30 min y pipeline completo — 663 conjuntos evaluados, 0 rechazos estructurales, 663 INCONCLUSIVE, REPLAY determinista; Fase C capturó 6 h continuas de la moneyline 2284198 (kickoff verificado, books OBSERVED_USABLE, 0 señales en fase temprana ⇒ `NO_SIGNAL_OBSERVED_IN_WINDOW` + INCONCLUSIVE) con la ventana completa preregistrada como runbook; Fase D certificó `M4_CERTIFIED_NON_LIVE` en el SHA final con prueba negativa del harness y sello físico `vcs.revision`. Evidencia en `testdata/research-v03/` + bundles `rs-v02/` y `rs-v03/` del repo de datasets. Agent run: [[2026-09-20-zcode-glm-polymarket-research-v03]].

- **2026-09-19 — `RS_V02_PARTIAL_DATA` — MANDATO RESEARCH DATA UNBLOCK v0.2 COMPLETADO (baseline `0d6a635` → final `7bd264d`, branch `feature/research-strategies-v01`, sin push):** Fase A preservó la evidencia real v0.1 (bundle SHA-256 verificado con replay físico, erratum de manifest_id declarado) y construyó la matriz de identidades; Fase B desbloqueó los datos: `catalog sync --id` dirigido (30829: membresía ×128 VERIFIED + constraints observados — tick 0.001/min 5/fees 1000 bps), keyset demostrado y adapter mínimo con fixtures, arnés de exhaustividad demostrado sobre el hermano resuelto 903193 (30829 queda UNKNOWN con evidencia faltante exacta: `getQuestionCount` on-chain), kickoff Sports verificado (`gameStartTime`==`event.startTime` en vivo y pre-match; proyectado en catálogo); Fase C conectó las estrategias al catálogo por `catalog emit-params` (el canal descubrió el mercado Other 559779 con `negRiskOther=true`, contradiciendo la narración v0.1) y ejecutó los experimentos reales: NegRisk 128-asset SCREEN/REPLAY/SHADOW → 2 sets REJECT `EXHAUSTIVENESS_UNSUPPORTED_STRUCTURE`, INCONCLUSIVE estructural; Sports moneyline 4734898 con kickoff verificado → 0 oportunidades en ventana de 2 min, INCONCLUSIVE con requisito de muestra exacto. Defecto corregido: books multi-sesión (cursor max, regresión test). Certificado `M4_CERTIFIED_NON_LIVE` (27 PASS/0 FAIL) en el árbol final; LIVE_DISABLED intacto; excepción de cobertura cmd 94.4% (14 stmts DEFENSIVE_FAULT_GUARD) declarada. Agent run: [[2026-09-19-zcode-glm-research-data-unblock-v02]].

- **2026-09-19 — `RS_V01_PARTIAL_DATA` — MANDATO RESEARCH STRATEGIES v0.1 COMPLETADO (baseline `9ae5dde` → final `0d6a635`, branch `feature/research-strategies-v01`, sin push):** tres fases con gates, sin rediseño del engine, LIVE_DISABLED durante todo el mandato. **Fase A — `RS-V01-A_PASS` (`7eb3532`):** screen runner extraído a pipeline compartido (journal+store+books+dispatcher), registro de factories, comando `screen-consolidated` que entrega el mismo frame (copias inmutables) a N instancias aisladas y consolida un manifest contando decisiones por instancia sin sumar PnL ni dedupear oportunidades; evidencia: dos instancias con params distintos → manifests distintos y métricas distinguibles, instancias ≡ run solo, determinismo, rechazos honestos. **Fase B — `RS-V01-B_PASS` (`9d737ae`):** `pocdata` (codec top-N de profundidad con frescura por source ms), **POC-S01 NegRisk complete-set** (`internal/strategy/pocs/negrisk`, hipótesis PE-002: gates de elegibilidad membership/exhaustiveness VERIFIED + sin Other + protocol conocido + fee resoluble; detección preliminar por best asks; pricing por profundidad observada vía `economics.BuildQuote`; edge por conjunto; todo ACCEPT con `CONDITIONAL_UNEXECUTABLE`) y **POC-S02 Sports pre-match reversion** (`internal/strategy/pocs/sports`, revisión identificada PE-005-R1 con trazabilidad a la familia PE-005: referencia = mediana de los propios spreads previos sin look-ahead por construcción, kickoff verificable o señales bloqueadas, taker-only v0.1 con maker rechazado como UNCALIBRATED, entrada = sweep sobre profundidad observada, gate económico = si la reversión al nivel de referencia paga el round trip, medición de reversión contra el bid de breakeven dentro de la ventana); wiring con routing incremental de records entre cuts (frames = cortes forward de la grabación, no vistas repetidas del estado final) y filtrado del carril RUNTIME; matriz completa de controles negativos por POC, overflows de frontera exacta que emergen en vez de envolver, E2E `screen-consolidated` con ambas POCs disparando sobre un mismo fixture; ramas defensivas inalcanzables eliminadas por política. **Fase C — `RS-V01-C_PASS` (`0d6a635`):** `RunShadow` con selección de factory por manifest y routing incremental; candidatos acotados llenan por leg desde profundidad OBSERVADA vía Simulator y las canastas multi-leg pasan por la máquina SEQUENTIAL frozen (COMPLETED o RESIDUAL_HELD por rechazo); scorecard con métricas por estrategia y contabilidad de canastas; evidencia `testdata/experiment/evidence/rs_v01_shadow.json` (ambas POCs shadow E2E con fills, paridad de hash de scorecard en journals frescos, outcomes INCONCLUSIVE honestos); **datos reales read-only**: sync de eventos Gamma (397 entidades) + 45 s de Market WS de los tokens YES del evento NegRisk 30829 (57–224 niveles reales) + SCREEN/SHADOW reales que corrieron y produjeron honestamente cero candidatos — los constraints de régimen de esos mercados quedan fuera del techo offset≤2000 de Gamma (brecha documentada) y la exhaustividad no es verificable hoy; `DATA_BLOCKED` con causa exacta en `testdata/research-v01/EVIDENCE.md`; REPLAY sobre el journal real con digest idéntico en schedules batch y 1 (`5a5bb186…`); CLI `experiment shadow --param` repetible. **Calidad:** build/vet/test/race verdes, tidy sin diff, diff-check limpio, piso ≥95% por paquete mantenido también en los 4 paquetes nuevos (negrisk 95.0, pocdata 95.7, sports 95.4, cmd 95.0). **Certificación:** `engine experiment certify --baseline 0d6a63583d6ef105985544e9d6f9ae04e15e64b9` → **`M4_CERTIFIED_NON_LIVE`**, 27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 diferidos live; el pin incorrecto de una corrida previa devolvió correctamente `M4_STALE_CERTIFICATION` (negativo del harness re-verificado). **Nota de identidad:** el mandato citó S01=Sports/S02=NegRisk; se conservó el mapeo canónico del proyecto (S01=NegRisk, S02=Sports, D-010) porque reasignar IDs a otras tesis está prohibido; la tesis sports de v0.1 (reversión pre-match) se registró como revisión PE-005-R1 y la tesis canónica PE-001 (combinatoria) sigue registrada para una iteración posterior. **Resultado global `RS_V01_PARTIAL_DATA`:** implementación, tests, screen consolidado, shadow E2E y certificación completos; la validación sobre datos reales aptos (exhaustividad verificable, constraints alcanzables, kickoff contractual para sports) es la siguiente iteración. Agent run: [[2026-09-19-zcode-glm-research-strategies-v01]].

- **2026-09-19 — MANDATO M4-COVERAGE-FINAL ejecutado (baseline `1c4f123` → `9ae5dde`, sin push):** objetivo único: cerrar el último bloqueador de aceptación M4 (cobertura <95% por paquete) sin recertificación completa, sin rediseño y sin features nuevas. **Preflight:** HEAD `1c4f123` confirmado, árbol limpio, sin otros writers; cobertura baseline reproducible `go test -count=1 -covermode=atomic ./...` con denominador per-package (tests propios; cambiar a `-coverpkg` habría sido manipular el denominador) — 20 paquetes bajo el piso, módulo ~87%. **Ejecución por prioridad** (Supervisor, Account, Strategy, Frames, Replay; luego el resto incluido el composition root), tests de falla reales únicamente: composition root cmd/engine 68.1→95.2 con re-exec físico de `main()` (exit codes reales 0/2 vía `exitWith`), corrupción quirúrgica de páginas SQLite (raíz de `schema_migrations`/`catalog_scans`/`regime_entities`/`reducer_cursors` leída desde `sqlite_master`) para ejercer honestamente las costuras de migración/queries de negocio/proyección, `ErrNotDurable` en los tres adaptadores de journal, encoders con stdout roto (broken pipe), universo compilado + suspects renderizados, fallo de proyección de `record` con WS vivo; supervisor 79.5→95.8 (fallas de boot/backup/restore con requireKind de errores tipados), account 80.5→95.3 (sabotaje de storage con drop de tablas y triggers RAISE, atomicidad sin filas parciales), strategy 78.5→99.4 + neutral 78.3→97.6 (lifecycle/fencing/journal-refusal; límites de dígitos canónicos de `spreadBps` corregidos a la semántica real de `finishArith`), frames 87.6→100 (cortes con dueños estacionados por gates deterministas: presupuesto de frames, fencing de generaciones, journal-refusal síncrono, deadline esperando ack), replay 80.7→97.4 (manifest phantom/foráneo, clipping por cutoff, verdict matrix DeepEqual, vanish del journal), capture 89.5→98.9 (vista read-only, recovery con colas podridas, group-commit por hook, fallas de fsync/seal/manifest), books 90.2→99.6, economics 78.1→97.1 (dinero exacto: VWAP, overflow de coste con 4096 dígitos), simulator 85.4→100 (G-10 edges), regimes 87.0→97.5 + persist 88.9→97.5 (sabotaje SQLite entre drains: DROP/reshape/triggers/vistas sombra), transports 100×3 (sockets reales locales, teardown por fallo de suscripción/heartbeat), experiment 88.0→95.8 (matriz negativa de certify extendida, receipts reales), risk 100, foundation 99.2, protocol 96.4 (drift de tipos por campo). **Defectos reales corregidos por ownership con regresión:** (1) `books.applyBook` — un segundo `book` completo del mismo epoch FUNDÍA los mapas de niveles en vez de REEMPLAZARLOS (semántica frozen M1.5 "book replaces all levels"): liquidez fantasma sobrevivía al snapshot y podía enmascarar un libro cruzado sin SUSPECT; fix: reset de mapas en todo book base + regresión `TestSecondBookSameEpochReplacesLevels` + reparación del test enmascarado `TestCrossedBookIsSuspect` (su delta 0.52 no cruzaba: el SUSPECT venía del bug; ahora ejercita el cruce genuino con 0.54); (2) `replay.RunObservation` — schedule con batch 0 vía API exportada fijaba `to=from` y el pump nunca avanzaba (bucle infinito; el CLI validaba, la librería no): validación fail-closed `schedule entries must be positive integers` antes de abrir el journal + regresión con watchdog de 5s. Menores registrados sin corregir: higiene gofmt de `certify.go`/`reconcile.go` aplicada (baseline ya estaba sin formatear; solo formato), procedencia de páginas events (`marketIndex<0` muerto), error original descartado en doble fallo rollback, UPDATE redundante del flag suspect, `PassedCut` inalcanzable vía API pública, `admitting` sin toggle StopRun, reason codes engañosos en fallo de journal, stragglers de barrera, `ManifestID` no determinista en contenido, frames sin contenido contados como delivered en shadow. **Clasificación del residuo (280 statements):** PROVEN_UNREACHABLE con demostración (crypto/rand fatal en Go 1.27, marshals de structs planos, rand/Rel/sort en paths construidos, guards de decimal canónico, re-parseos de entrada validada,driver modernc LastInsertId/RowsAffected/Open sin error, embed FS de migraciones) y NONCRITICAL_TESTABLE (seams de driver rows.Err/Scan a mitad de iteración sin mecanismo determinista en SQLite in-proceso, ramas de wiring de composición con constantes válidas sobre caminos probados en su capa dueña, ventanas de carrera defensivas) — **cero CRITICAL_TESTABLE alcanzable sin cubrir, cero UNRESOLVED, ninguna excepción solicitada** (el piso se cumple sin excepciones). **Verificación final en `9ae5dde`:** build/vet/test/race/tidy/diff-check verdes; cobertura final por paquete: cmd 95.2, account 95.3, archtest 97.1, books 99.6, capture 98.9, catalog 95.1, config 97.8, economics 97.1, experiment 95.8, foundation 99.2, frames 100, persist 97.5, protocol 96.4, regimes 97.5, replay 97.4, risk 100, simulator 100, strategy 99.4, neutral 97.6, supervisor 95.8, clob 100, datav2 100, gamma 97.5, marketws 100 — módulo 97.15%. **Certificado re-emitido con pin:** `engine experiment certify --repo-root . --baseline 9ae5dde…` ⇒ exit 0, `M4_CERTIFIED_NON_LIVE`, 32 filas: 27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 diferidos live; negativos comprobados: baseline incorrecto ⇒ `M4_STALE_CERTIFICATION`, `--skip-suites` ⇒ `M4_PARTIAL` (13 PASS, 14 in-scope NOT_RUN, receipts jamás inventados). **Estado:** `M4_MANAGER_ACCEPTANCE_READY_PENDING_OWNER` — piso cumplido sin excepciones; única resolución pendiente: `M4_MANAGER_ACCEPTED_NON_LIVE`. Agent run: [[2026-09-19-zcode-glm-polymarket-m4-coverage-final]].
...
- **`S04_BASELINE_ACCEPTED_FOR_S05` — preflight del mandato S05–S13+M4 (2026-09-18, checkpoint `e28c7eb`):** baseline verificado (`527dc73` HEAD, árbol limpio, ambos SHAs esperados presentes); regresión global verde (build/vet/test/race/tidy/diff-check). Parche fuera de scope `internal/capture/writer.go` RATIFICADO: causa raíz correcta (mapa `segPaths` no seguía el rename active→sealed; recovery repoblaba el mapa y por eso los tests de re-apertura no lo veían), fix de 2 líneas coherente con `recover.go` — y ahora con test de regresión dedicado en ownership de Capture que falla pre-fix (apuntando al path `.active` retirado) y pasa post-fix; la excepción de cobertura cmd/engine 89.6% queda confirmada como seam `os.Exit` heredado (main/exitWith, sin lógica nueva oculta). Gofmt aplicado a 12 archivos S04 (higiene mecánica). Cobertura módulo 95.1%. Gamma `offset>2000` permanece documentado como partial honesto. S05 autorizado a comenzar.

- **`M2_S05_PASS` — M2-S05 Regimes/Resolution + constraints CLOB + Data v2 read-only implementado y certificado en su alcance (2026-09-18, checkpoint `5cc6476`):** preflight sobre `e28c7eb` (regresión global verde). **`internal/regimes`** (migraciones 0020–0024 de la banda 0020–0029): entidades de régimen por kind/namespace (gamma | ctf.137 | condition) con revisiones de contenido inmutables, dedup por content_hash, known_at = receive time sin look-ahead y reingestión idempotente (historia como conjunto de contenido, puntero en la última); contenido con raw fields separados —tick/min-size/feesEnabled/maker/taker/base_fee/fd/lifecycle (end/active/closed/archived/accepting_orders/UMA status observados sin colapsar)— y un campo ausente que permanece ausente (sin sustitutos cero); las outcome assets heredan constraints por mercado en revisiones propias; resolución de última vida (resolutions Data v2) con status/reporter/payout vector verbatim/blocks/hash/log index y dedup por contenido; reducer serial single-owner sobre el journal durable con cursor propio (applied_seq ≤ durable_seq imposible, sub-batches acotados por bytes, cuarentena declarada que no bloquea el drain). **FeeResolver v1** (U-02/A-18/FBL-012): puro y versionado — POINT sólo con evidencia convergente (trade-observed + declaradas), INTERVAL [0, base_fee publicada] con supuestos explícitos, UNRESOLVED sin cota, SUSPECT ante contradicción de fuentes con intervalo incierto; nunca base_fee/10000 sin prueba; conversión BpsToFeeCoefficient exacta; versión desconocida falla cerrado. **Fee trade-observed** con known_at=capture_seq (tabla propia, historial auditable; el estado actual se evalúa contra la ÚLTIMA observación — refresh convergente limpia REGIME_SUSPECT, el historial contradice nada silenciosamente); reducer Market WS listo (tick_size_change → bump de revisión del asset; last_trade_price → fee observation) para el wiring de S06. **Invalidación:** todo cambio de tick/min-size/fee/status produce revision bump observable por snapshot (Constraint revision), demostrado con candidatos que quedan stale. **Transport:** `internal/transport/clob` (GET-only /clob-markets/{condition} + /fee-rate query y path forms con fallback documentado, retries acotados 429/5xx) y `internal/transport/datav2` (GET /v2/resolutions con bound ≤20 condiciones). **G-01_REGIME_INTEGRATION PASS** (`testdata/regimes/evidence/g01_regime_integration.json`): la frontera numérica frozen de foundation consume tick/min-size desde revisiones durables (nunca hardcode); el bump de tick cambia inmediatamente la grilla aceptada. **Real read-only VERIFIED** contra servicios vivos: /fee-rate en ambas formas (`base_fee:1000`, byte-idénticas), /clob-markets/{condition} (mts/mos/mbf/tbf convergentes con Gamma), /v2/resolutions (status posed, proposed_price "69" sentinel) — fixtures commiteados con manifest SHA en `testdata/regimes/fixtures/`. **Correctivo upstream demostrado por wire vivo (ownership S02, `internal/protocol`):** el parser de resolutions exigía XOR condition_id/question_id pero el servicio real sirve filas UMA con AMBAS claves — regla corregida (condición presente → condition-keyed, question_id auxiliar; ninguna clave → rechazo), fixtures y G-03/G-01-wire re-emitidos. **Correctivo de presencia (ownership S02):** GammaMarket ahora expone MakerBaseFee/TakerBaseFee/Active/Closed/Archived/AcceptingOrders *Present* (nil colapsaba a false y regimes habría inventado flags); aditivo, catálogo sin cambios de comportamiento. **CLI:** `regimes inspect` (--kind/--namespace/--entity/--suspects/--json) verificado físicamente tras `catalog sync` contra fixture server: revisiones, known_at, fee state, lifecycle y ausencia declarada honesta. **Quality:** build/vet/test/race globales verdes, tidy sin diff, diff-check limpio; rapid fresh seeds + sweep determinista semilla 20260917; coverage: regimes 87.3%, clob 91.4%, datav2 87.8%, cmd 87.7% (seam os.Exit heredado + wiring no-importado), protocol 95.1%, módulo 93.8% — ramas restantes de regimes son defensivas (errores SQL por corrupción, superficie desconocida inalcanzable por la API de capture), registradas como excepción técnica continuadora del patrón S03. Gates: G-01 cerrado completo (foundation+wire+regime), G-03 re-PASS tras correctivo; insumos G-10b listos; G-05+ NOT_RUN; `LIVE_DISABLED` sin cambios. Siguiente: `M2-S06` (Market WS + books + quality).

- **`M2_S06_PASS` — M2-S06 Market WS + book shards + quality implementado y certificado en su alcance (2026-09-18, checkpoint `0159eb5`):** **`internal/transport/marketws`** (gorilla/websocket encapsulado tras seam): suscripción documentada (`{"type":"market","assets_ids":[...],"initial_dump":true}`), PING/PONG texto cada 10s (PONG contado como evidencia de liveness, no journaleado como market data — decisión registrada), una conexión = un autor de epoch (el caller asigna epoch; reconnect = epoch nuevo), reader único que entrega frames en orden de recepción, dial con timeout, close desbloquea. **`internal/books`** (replay-only, sin migraciones nuevas): shards por asset con epochs fencing — book completo abre/re-sanea el epoch, deltas previos al primer book se capturan pero no aplican, price_change asigna size ABSOLUTO (repetir nunca suma; cero elimina), last_trade_price nunca muta niveles, tick_size_change invalida constraints sin reescalar (SUSPECT hasta re-base), gap de evidencia declarado revoca epochs abiertos (revocación consumer-side del contrato capture), SUSPECT ante book cruzado y regresión de source time, REST_OBSERVATION namespace separado con reorden canónico (bids desc/asks asc; wire REST best al final), book vacío válido, sin dedup por timestamp/hash, gate de calidad honesto: OBSERVED_USABLE exige book + constraints de régimen conocidos y no suspect (SYNCING mientras no); cursores applied_seq por shard vía persist.AdvanceCursor (regresión y frontier clamp heredados), replay desde cero reconstruye idéntico. **Wire vivo demostrado y corregido upstream (ownership S02):** el servicio real usa `event_type` (no `type` documentado), frames array-wrapped, y el book trae extras `tick_size`/`last_trade_price` — parser corregido con `ParseMarketWSFrame` (variantes demostradas aceptadas, shape documentado first-class, ni type ni event_type → drift), fixtures reales en `testdata/books/fixtures/` con manifest. **CLI:** `record` (frames → journal durable antes de cualquier consumo, reducers books+regimes drenan en vivo — wiring fee trade-observed a Regimes via journal) y `books inspect` (replay in-memory del journal, constraints wireados) — verificación física REAL read-only contra `wss://ws-subscriptions-clob.polymarket.com/ws/market`: dos ventanas (mercado quieto → OBSERVED_USABLE book vacío válido; mercado activo → books con 34/2 niveles reales, gate SYNCING honesto sin catálogo, PONG recibido, PINGs enviados), test E2E del CLI contra fixture WS server (record → journal → books inspect → fee observation en regimes). **G-05 PASS** 14 escenarios con evidencia `testdata/books/evidence/g05.json` (fault injection: delta-antes-de-snapshot, handover, gap, crossed, regresión, gate constraints, REST separado, no-dedup, replay idéntico, cursores witness). **Quality:** build/vet/test/race globales verdes (13 paquetes), tidy sin diff, diff-check limpio; coverage books 85.8%/marketws 85.7%/cmd 84.6% — ramas restantes defensivas (errores SQL, marshal imposible). Gates: G-05 PASS, G-06 cerrado con consumidores reales (books/regimes consumen sólo hasta durable_seq, demostrado por los reducers existentes), G-05+ NOT_RUN; `LIVE_DISABLED` sin cambios. Siguiente: `M2-S07` (Frame Builder).

- **`M2_S07_PASS` — M2-S07 Frame Builder y delivery runtime implementado y certificado en su alcance (2026-09-18, checkpoints `3044294` + `b9e81e7`):** **`internal/frames`** (replay-only, sin migraciones): dispatcher único con corte forward exacto FBL-008 — C=dispatched_seq fijado bajo lock del dispatcher, barrera `Barrier(cut_id, C, generation)` encolada en TODOS los inboxes FIFO antes de que ningún record >C pueda routearse (reserva atómica); owners actores con inbox acotado que publican snapshot de WATERMARK al cruzar la barrera incluso sin mutación; frame `INELIGIBLE` con razones frozen (`owner_inbox_full`, `owner_passed_cut_without_barrier`, `cut_deadline_exceeded`, `stale_generation`, presupuestos, run no-admitiendo) — jamás sustituto latest; presupuestos K/bytes por run (in-flight + retenido); coalescing implícito por generación (respuestas tardías descartadas por mismatch); `DeliveryFrame{run_id, ordinal, trigger, cut_seq, generation, revision_vector, quality, virtual_time, assets}` con revision vector (slots universe/relationship/regime/quality/clock/config + slots reservados account/risk/liquidity/quote para S11/S13 — la ausencia es la representación honesta) y journaling RUNTIME **descriptor durable antes de la entrega** (ack esperado; round-trip probado contra capture real); capture jamás bloqueado (inboxes acotados, dispatcher nunca toma locks del journal). **G-05b PASS** con evidencia `testdata/frames/evidence/g05b.json`: property de scheduling (rapid fresh seeds, contraejemplos auto-persistidos) + harness determinista de **100 seeds** (20260917–20260916+100) verificando por owner la suma determinista de records ≤C — ningún record ≤C omitido, ninguno >C incluido, snapshot watermark para owners sin eventos, razones INELIGIBLE explicadas, presupuestos respetados. Quality: build/vet/race verdes; coverage frames 82.4% (residual: ventanas de concurrencia y ramas defensivas, registradas). Siguiente: `M2-S08` (Replay).

- **`M2_S08_PASS` + `M3-B_PASS` — M2-S08 Replay de observación y manifests implementado y certificado en su alcance (2026-09-18, checkpoint `4631f3b`):** **`internal/replay`**: manifest build verifica el journal (capture.Verify) y pina su estado verificado — segmentos con payload SHA-256, holes/discontinuidades, coverage por clase, cutoff, seed, clock virtual, delivery policy, config revision — y REHUSA evidencia corrupta (integrity failures → no manifest); observation replay corre books+regimes sobre EXACTAMENTE los records pineados con el batch schedule como única variable libre — **G-07: digest idéntico con 3 schedules distintos** (32 / 1 / 7,3,1); delivery replay resuelve los DeliveryFrames journalleados y declara `NOT_REPRODUCIBLE` por frame con razón exacta (cut_seq > cutoff, revision_vector vacío, payload undecodable) — jamás salto silencioso; igualdad estricta de pins (nombre+hash, hash vacío para segmento activo) rehúsa manifests stale tras seal/rename/tamper — **semilla G-07b**: manifest manipulado y journal corrompido ambos rehusados. **Correctivo ownership S03 justificado por el contrato de replay:** `capture.OpenView` — vista READ-ONLY del journal (sin boot nuevo, sin renombres ni sellos; replay jamás muta la evidencia que lee; el frontier se deriva de las entradas del manifest verificado + descubrimiento del segmento activo en disco por su nombre `.active`, indexado in-place). **CLI:** `manifest build` + `replay` verificados físicamente sobre el journal vivo de S06 (digest estable, assets OBSERVED_USABLE, 0 frames — correcto: record no produce frames). Quality: build/vet/test/race globales verdes (15 paquetes), tidy/diff-check limpios. **`M3-B_PASS` registrado:** capacidad Sincronizar un universo elegible acotado (S04), capturar un mercado real (S06 record live), y reproducir determinísticamente la observación propia (S08 replay) — demostrada end-to-end con el journal real. Gates: G-07 parcial PASS (cierre completo con strategy/account refs en S09/S13); `LIVE_DISABLED` sin cambios. Siguiente: `M2-S09` (Strategy runtime / SCREEN).

- **`M2_S09_PASS` — M2-S09 Strategy runtime y modo SCREEN implementado y certificado en su alcance (2026-09-18, checkpoint `238ada9`):** **API Strategy M1.8 materializada** (`internal/strategy/api.go`): `Strategy/Factory/Descriptor/RunContext/Frame/EvaluationContext/Opportunity/ActionCandidate/Assessment/Feedback` con valores inmutables, candidates tipados con bounds (legs asset/side/limit/size, vigencia, BasketPolicy id), sin signer/wallet/repos/execution ports/red — la superficie de RunContext es identidad/parámetros/mode; errores tipados (`NO_SIGNAL/INSUFFICIENT_DATA/INVALID_MODEL/TRANSIENT_INPUT`); `WAITING_DATA` sin sustitutos cero. **Runtime del actor (OD-1)** (`runtime.go`): mailbox acotado (backpressure = refusal tipado transient que pausa el run, jamás drop silencioso), callbacks seriales, deadline con cancelación de contexto y descarte por run generation, panic recuperado → instancia `FAILED` + retiro de candidatos (política de cancel via Coordinator: stub deny-all hasta S11), callback no-cooperativo → `Fence()` sin acumular goroutines, descriptor journalleado en RUNTIME **durable antes de iniciar** y resultados publicados sólo tras durabilidad. **Fixture neutral** (`strategy/neutral`): single/multi asset, detección de spread best-bid/ask con orden determinista, evaluación exact-decimal → bps entero contra threshold, INCONCLUSIVE ante quote cruzada/insuficiente, sin I/O/reloj/random/goroutines en callbacks. **Gate de imports (G-15b parcial):** `CheckStrategyDir` en ambas direcciones sobre el paquete real de estrategia (neutral pasa; probes con imports prohibidos fallan — scope honesto: el runtime es código del engine y su goroutine del actor es legítima). **CLI `screen`:** journal → replay books → dispatcher cuts (barreras S07) → runtime → neutral → reporte de oportunidades/assessments — **sin órdenes reales ni simuladas, sin secretos**; verificado físicamente sobre el journal vivo de S06 (spread 6666 bps detectado y evaluado en el mercado long-tail real). **G-08/G-09 PASS** con evidencia `testdata/strategy/evidence/g08_g09.json` (10 escenarios: descriptor durable, start-once, serial consistente, panic→FAILED, deadline descarta, fencing, backpressure, stop idempotente sin órdenes, imports gate bidireccional, API sin red/secretos). Correctivo aditivo ownership S01: `foundation.Decimal.Div` (precisión fija 16) e `IntPart` (truncación con rechazo de overflow int64) para el cálculo de bps del fixture — dinero nunca fluye por un cociente sin regla de redondeo upstream. Quality: build/vet/test/race verdes (17 paquetes), tidy/diff-check limpios. Gates: G-08 PASS, G-09 PASS, G-15b parcial PASS; G-07b/10+ pendientes; `LIVE_DISABLED` sin cambios. Siguiente: `M2-S10` (Economics/Simulator).

- **`M2_S10_PASS` + `M2_S11_PASS` + `S10_S11_INTEGRATION_PASS` — M2-S10 Economics/Simulator, M2-S11 Account Coordinator/Risk y la barrera B1×B2 certificadas en su alcance (2026-09-18, checkpoints `1d19254`, `37b8146`, `1bbe445`):** **S10** (`internal/economics` puro + `internal/simulator`): sweep/VWAP/worst-price exactos (BUY barre asks, SELL bids, sin interpolación, shortfall visible), `BuildQuote` con el fee viajando exactamente como el FeeResolver lo produjo (POINT resuelto, INTERVAL acotado, UNRESOLVED/SUSPECT inconcluso — jamás punto medio), BUY exige cash / SELL exige tokens sin corto sintético, fill engine determinista con FOK/FAK/GTD, limit guard, cancel-latency race resuelta determinísticamente (CancelAfter desde submit), liquidity ledger aislado por (ExperimentID, RunID, instance, scenario) sin doble consumo, ledger de portfolio compartido con atribución PEER_CONSUMED_DEPTH y participantes preregistrados, escenarios con label UNCALIBRATED; G-10 fixtures exactas (notional 77, fee 0.153, multi-leg, cancel races) + G-10b (runs independientes idénticos, atribución portfolio, fee evidence sin bleed entre trades) + property de conservación con rapid. **S11** (`internal/account` + `internal/risk`, migraciones 0030–0035 ACCOUNT_FACT): Coordinator único escritor por namespace — reserva+intent PREPARED+outbox en UNA transacción, un attempt/un submit verificado, classifier frozen (error JSON documentado = DEFINITIVE_REJECT; 400 desnudo/404/429/5xx/HTML = UNKNOWN), boot recovery (PREPARED sin attempt → VOID+RELEASABLE; SEND_ATTEMPT_STARTED → UNKNOWN; restore atrasado jamás VOID por ausencia), dedup FillKey WS+REST exactly-once, atribución con invariante bloqueante Σatribuciones+UNATTRIBUTED=inventario, AccountView derivada una vez (available = ledger net − held), BasketPolicy SEQUENTIAL-only con máquina frozen sin aristas de unwind, SELL exige tokens de la instancia, puertos de ejecución DISABLED fail-closed; Risk evaluador puro (caps por instancia/total, min free cash, compare-and-check de revisión, modos LIVE rehusados). **Barrera S10×S11** (`internal/integration`, `S10_S11_INTEGRATION_PASS`): una sola autoridad contable — risk aprueba con compare-and-check, coordinator prepara atómicamente, simulator llena determinísticamente en su ledger aislado, el fill se convierte en hecho contable vía FillKey (dedup exactly-once), reserva consumida parcial/total (FinalFill), invariante de atribución verde, namespaces de experimento aislados, suite global verde (21 paquetes con race). Correctivo aditivo ownership S11: `account.ObserveFill` distingue fill final (reserva RELEASED por conversión a tokens) de parcial (PARTIALLY_CONSUMED). `LIVE_DISABLED` sin cambios. Siguiente: `M2-S12` (Supervisor/Observability/backup-restore).

- **`M2_S12_PASS` + `M3-C_PASS` — M2-S12 Supervisor/Observability/Backup-Restore implementado y certificado en su alcance (2026-09-18, checkpoint `edc0a89`):** **`internal/supervisor`**: arranque en orden frozen (flock exclusivo no-bloqueante → recovery del journal → store+migraciones → validación de ExecutionMode/ActivationLease → markers), markers BOOT_CLEAN/BOOT_OPEN derivados de las discontinuidades del recovery, lease validator fail-closed (sin lease = LIVE_DISABLED + NO_LEASE; token sin documento certificable = LEASE_MISALIGNED_REFUSED — este mandato no emite leases), `readiness_live` SIEMPRE false, watermarks de disco (low pausa replay/exports; critical bloquea nueva exposición) con diagnóstico observable, readiness liveness/public/strategy/live coexistentes. **Backup/restore local consistente (G-14/G-06b):** backup con corte durable B (capture sealed), snapshot SQLite por API del motor (`persist.SnapshotTo` vía VACUUM INTO, WAL-consistente), copia del journal sellado con SHA-256 por archivo, witness de cursors ≤ corte verificado ANTES de publicar (DB adelantada al journal → sin PASS), BACKUP_COMPLETE sólo tras verificación de hashes del bundle; restore en directorio limpio con verificación completa de hashes, recovery sobre la evidencia restaurada, check `journal_seq ≥ applied_seq` (`persist.CheckCursorsWithinFrontier`), archivo faltante o bytes alterados → DEGRADED honesto jamás PASS; fixture GC-día-31: ACCOUNT_FACT intacto sin TTL. **CLI `supervisor run|backup|restore`** verificado físicamente (boot limpio + shutdown clean; backup complete con frontier witness; restore 4/4 archivos verificados). **`M3-C_PASS` registrado:** S09–S12 y la barrera S10/S11 verdes. Quality: build/vet/test/race verdes (22 paquetes), tidy/diff-check limpios. `LIVE_DISABLED` sin cambios. Siguiente: `M2-S13` (Experiments/SHADOW E2E).

- **`M2_S13_PASS` + `M3-D_PASS` + `M4_CERTIFIED_NON_LIVE` — M2-S13 Experiments/SHADOW E2E, harness de certificación y cierre M4 (2026-09-18, checkpoints `d2e73e8` + `ab33405`):** **`internal/experiment`** (migraciones 0045–0047): registro de hipótesis PE-xxx con protocolo preregistrado, run manifests que pinean la evidencia de replay/estrategia/parámetros/escenarios/bankroll virtual/seed con modos INDEPENDENT vs PORTFOLIO_SHARED explícitos, **pipeline SHADOW end-to-end** (vista read-only del journal → replay books → cortes del dispatcher → runtime de estrategia neutral → economics → simulador con ledger virtual → Coordinator virtual → scorecard con denominadores honestos incl. delivery errors), outcomes `GO|ITERATE|NO_GO|INCONCLUSIVE` como veredictos de investigación jamás permisos live (el fixture shadow queda INCONCLUSIVE honesto), **G-07b PASS** (paridad: scorecard idéntico por hash entre runs sobre el mismo manifest), records del carril RUNTIME jamás routeados como observaciones de mercado. **Harness de certificación (`certify --profile no-live`):** consolida la matriz de gates desde la evidencia commiteada (G-01/G-01-wire/G-01-regime/G-03/G-04/G-05/G-05b/G-06/G-02b/G-08+G-09/G-15b-base con bundles físicos verificados) más las suites de fixtures que corren verdes en cada ejecución (G-02, G-10, G-10b, G-10c, G-11b, G-12b, G-14, G-06b, G-13b, G-07b, G-09b, G-13), con los gates live diferidos (G-16…G-19, G-14b) explícitos como NOT_RUN/IMPLEMENT LATER; lógica de outcome conforme a OD-2 — los diferidos son informativos y no bloquean la certificación no-live. **Verificación física:** shadow sobre el journal vivo de S06 (920 frames entregados, 920 oportunidades/evaluaciones/aceptaciones en SINGLE con 1 fill simulado; MULTI con 0 oportunidades refleja honestamente que solo un asset del journal tiene book usable), certify → **`M4_CERTIFIED_NON_LIVE`: 23 gates PASS, 0 FAIL, 0 NOT_RUN en alcance, 5 diferidos**. **`M3-D_PASS` registrado.** Quality: build/vet/test/race verdes (23 paquetes), tidy/diff-check limpios. `LIVE_DISABLED` sin cambios durante todo el mandato. Nota honesta para revisión manager: la certificación consolida evidencia commiteada + suites ejecutadas; el harness `certify` es consolidación de evidencia, no sustituto de las suites (que corren verdes en cada pasada).
- **`SHARED_POC_FOUNDATIONS_INTEGRATED_PENDING_OWNER_REVIEW` — Shared Unblocker integrado sobre la base vigente (2026-09-20, INTEGRATION_SHA `9d0512a912fcce4b9aefc152c7a89b090ff8df1d`):** la rama `feature/shared-poc-unblocker` (8 commits SFG, trabajo original `7bfe3f6` sobre `25f578a`) fue rebasada 8/8 sin conflictos sobre `feature/research-strategies-v01@f070496` (delta base `25f578a..f070496`: untrack de dirs de captura RS v0.3 + evidencia research-v03, intersección de archivos cero con la rama shared) y un commit de integración eliminó un binario ELF `engine` de 19MB accidentalmente trackeado por SFG-01/05/04. Validación íntegra re-ejecutada sobre `9d0512a`: build/vet limpios, `go test ./... -count=1` 31 paquetes ok, race ok en 16 paquetes, 36 tests `TestSFG*` PASS, cobertura = pisos documentados con excepciones declaradas y 0 funciones a 0% en paquetes nuevos, tidy/diff-check/status limpios, LIVE_DISABLED, captura RS v0.3 intocada. M4 recertificado con el harness canónico (`engine experiment certify --repo-root . --baseline 9d0512a…`) ⇒ `M4_CERTIFIED_NON_LIVE` 27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 diferidos — no heredado del SHA anterior; la aceptación manager sigue pendiente como decisión humana. Decisión SFG-06 (G3) según la SPEC de POC-S05 (cohorte W condicional, primer WP A1 sin WS tipado): Caso A — `PE-004-O/B = READY`, `PE-004-W = BLOCKED_BY_SFG06` como tarea separada fuera de la ruta crítica. Baseline única recomendada para arrancar PE-001, PE-030 y PE-004: `9d0512a` (sin push; Review humana pendiente, puente en `[r]`). Matriz de dependencias, readiness contract y receipts por consumidor en [[Polymarket Engine — POC Shared Unblocker]]. Agent run: [[2026-09-20-zcode-glm-5.3-flash-shared-poc-integration]].
- **`M4_MANAGER_ACCEPTANCE_READY_PENDING_OWNER` — Correctivo M4-AUDIT-C2 ejecutado (2026-09-19, baseline declarado `879886f`, baseline final `1c4f123`):** correctivo integral autónomo en cuatro fases, sin rediseño, sin push, LIVE_DISABLED sin cambios. **Preflight:** SHA `879886f` verificado, árbol limpio, sin writers concurrentes, suite baseline verde (build/vet/test 25 paquetes). **P1 — G-11 cerrado (`17cc207`):** nueva superficie de reconcile dentro del ownership de Account (`internal/account/reconcile.go`, migración 0036 `account_reconcile_cases`): (A) fills WS duplicados/fuera de orden con dedup FillKey exactly-once, status atrasado sin regresión (ejes OPEN<PARTIAL<TERMINAL y MATCHED<MINED<CONFIRMED monótonos; UNKNOWN neutral), contradicciones → caso durable `ORDER_STATE_CONTRADICTION` idempotente por (namespace, kind, subject) sin terminalidad inventada, ACK tardío converge con un attempt y sin resubmisión; (B) scan REST paginado (`ReconcileOpenOrders`) con re-lectura de la primera página: mutación entre páginas o fallo de fetch ⇒ `complete=false` + caso `RECONCILE_INCOMPLETE` — una pasada incompleta jamás es ausencia concluyente de órdenes/fills, UNKNOWN retenido; (C) settlement MATCHED→MINED→CONFIRMED monótono, claim de reorg tras CONFIRMED rechazado con caso, FAILED registra FAILED **sin** liberar reserva, **sin** crear saldo, **sin** declarar confirmación, y con fills atribuidos escala a caso `SETTLEMENT_FAILED_WITH_ATTRIBUTED_FILL` en HUMAN_REVIEW_REQUIRED; (D) transferencias exactly-once por transfer id vía outbox: EXTERNAL_IN queda **UNATTRIBUTED bloqueante** (View expone `Unattributed` sobre la unión attributed∪ledger — el inventario externo ya no escapa del invariante), sin asignación silenciosa, SELL cross-instancia sólo vía `INSTANCE_MOVE` explícito idempotente con rechazo de sobregiro; (E) invariantes globales demostradas (conservación, sin doble liberación, sin terminalización por timeout con reloj al futuro lejano, sin resubmisión, UNKNOWN bloqueante). Fixtures versionadas `testdata/account/fixtures/g11/` (events.json + manifest con sha256 pineado y provenance synthetic-frozen; el test verifica el hash antes de confiar) y evidencia `testdata/account/evidence/g11.json` con 15 escenarios expected/actual bajo el patrón emit/re-check. Regresión de colaterales verde con race (G-02, G-02b, G-10c, G-11b, G-12, G-12b vía suites account/simulator/integration). **P2 — G-07 evidencia formal (`5fbeec5`):** `testdata/replay/evidence/g07.json` producida por `TestG07Evidence` (emit/re-check, rehúsa digests drifted): tres schedules {32}/{1}/{7,3,1} explícitamente identificados, digest idéntico, delivery 2 frames / 1 resolved / 1 NOT_REPRODUCIBLE (`empty_revision_vector`); sin recaptura masiva. **P3 — cobertura y fault testing (`4381884`):** en el orden de prioridad del mandato, tests útiles de falla sin borrar código ni excluir paquetes: supervisor 64.7→79.5 (validaciones de boot, mapeo OK/LOW/CRITICAL de watermarks con fallback honesto ante statfs fallido, BOOT_OPEN por hueco de segmento sellado con readiness_strategy retenida, barrera G-06b **en backup** (cursor adelantado al corte rechaza el bundle), restore rehúsa manifest ausente/ilegible/interrumpido y target inutilizable, accessors + shutdown unclean) — **expone y corrige un defecto real: `Boot` nunca copiaba dataDir a cfg ⇒ el diagnóstico de watermark hacía statfs sobre path vacío y nunca podía salir de OK** (el caller `supervisor run` lo compensaba pasando DataDir duplicado); frames 82.4→87.6 (inbox-full en RouteRecord con semántica contractual de consumo de seq, aborto de reservación completa de barrera con rollback de in-flight y ordinal exacto posterior, fencing de generaciones contra respuestas tardías); account 77.7→80.5 (atomicidad ante storage failure sin filas parciales, bordes de forma del classifier, validaciones fail-closed, EXTERNAL_OUT exactly-once); risk 82.9→91.4 (validación de campos de policy y MODE_NOT_PERMITTED); replay 79.7→80.7 (manifest nil/forastero rehusado antes de correr, delivery en frontier vacío honesta); strategy 75.9→78.5 (validación de identidad/descriptor/journal en NewInstance, Start fallido ⇒ no Running, stop idempotente, feedback post-stop rehusado); simulator 79.8→85.4 (inputs inválidos de sweep/depth, ordenamiento por lado de trade, identidad de ledger, atribución peer de ledger compartido); cmd 50.3→68.0 (matriz in-process de runners: usage=2, fallo runtime=1, éxito=0 con salidas documentadas; `screen` sobre directorio vacío es reporte honesto de cero frames). Clasificación de lo restante (para decisión owner): NONCRITICAL_TESTABLE = ramas de error de driver/JSON y unlocks defensivos mapeadas por función en `go tool cover -func`; PROVEN_UNREACHABLE por API = seam os.Exit de cmd (excepción heredada S04) y guards de driver SQLite; UNRESOLVED = ninguna brecha de gate. **P4 — harness confiable (`1c4f123`):** `certify` reconstruido: denominador pineado (27 filas no-live sobre los 26 identificadores de M2.5 + 5 live diferidas = 32), identidad por fila (gate_id, evidence_path, evidence_sha256, verify_command, verified_at, baseline_sha), receipts reales producidos al certificar ejecutando los tests de cada gate suite-backed (comando, baseline, exit code, paquetes, patrón, estado del proceso, evidencia; patrón que matchea cero tests = FAIL; `--skip-suites` degrada esas filas a NOT_RUN — jamás PASS sin recibo), status commiteado distinto de PASS (incluido PARTIAL) falla cerrado, evidencia respaldada por suite lleva además recibo de re-check contra el código vivo (drift de escenarios o de fixture sha ⇒ FAIL), `M4_STALE_CERTIFICATION` cuando el pin `--baseline` no coincide con HEAD, y sólo matriz completa verde en el baseline pineado certifica. Matriz negativa pineada en `certify_test.go` (denominador, evidencia ausente, recibos ausentes, identidad intercambiada, status PARTIAL, escenario fallido en bundle, baseline incorrecto, recibo vacío, paquete injecutable) + `TestCertifyRealTreeWithReceipts` sobre el árbol real. **Regresión y E2E finales en `1c4f123`:** build/vet/test/race/tidy/diff-check verdes; `journal verify` (frontier 6464, NO_INTEGRITY_FAILURES), `manifest build` (11 segmentos, cutoff 6464) + `replay` digest `7f24c081…` idéntico en schedules {1} y {32} — coincide con el digest registrado en la auditoría previa ⇒ evidencia viva sigue válida sin recaptura; `experiment shadow` SINGLE outcome INCONCLUSIVE honesto; `supervisor backup` (backup_complete=true) → `restore` (16/16 archivos, degraded=false, 210ms) → `journal verify` restaurado sin fallos; boot físico LIVE_DISABLED / NO_LEASE / readiness_live false / BOOT_CLEAN. **Certificado final re-emitido en HEAD:** `engine experiment certify --repo-root . --baseline 1c4f123…` ⇒ `M4_CERTIFIED_NON_LIVE`, 32 filas: 27 PASS / 0 FAIL / 0 in-scope NOT_RUN / 5 diferidos. **Higiene vault (separada del engine):** 36 residuos `*.md.tmp.*` (la auditoría reportaba cuatro; la inspección física encontró 36) eliminados con prueba por archivo (PIDs writer muertos, 29 superseded por escritura real posterior, 7 byte-idénticos al vigente, 0 inciertos, ninguno trackeado ni referenciado) y registrados en [[2026-09-19-md-tmp-residue-cleanup]]; commit `2c3c2e14` del vault, sin mezclar con el engine. **Estado y blocker único:** `M4_ACCEPTANCE_PENDING` — el piso Agents-OS de 95% por paquete sigue sin cumplirse (módulo ~87%; ≥95: catalog/config/archtest/gamma/protocol 94.8–97.8; resto 78–91); sin excepciones auto-aprobadas; clasificación por función entregada para que el owner eleve o apruebe excepciones por líneas concretas. Todo lo demás del criterio de aceptación está verde; `M4_MANAGER_ACCEPTED_NON_LIVE` queda como resolución posterior del manager.
- **`M4_ACCEPTANCE_PENDING` — Auditoría final M4 manager/auditor (2026-09-18, baseline declarado `ab33405`, correctivo `M4-AUDIT-C1` = `879886f`):** auditoría física del cierre `M4_CERTIFIED_NON_LIVE` contra criterios frozen M1.15/M2.5. **Preflight:** SHA y árbol limpio confirmados, sin otros writers, evidencia vigente commiteada en el baseline. **Reconciliación de gates (26 identificadores M2.5):** los 11 bundles de evidencia commiteados existen, status PASS interno sin escenarios fallidos, y el manifest SHA-256 embebido en `g03`/`g01_wire` coincide con el hash real de `testdata/protocol/manifest.json`; la agrupación G-08+G-09 preserva verificaciones individuales (10 escenarios separados en `g08_g09.json`). La diferencia 26→23 NO son 3 gates faltantes: son **4 identificadores ausentes del harness** (G-07, G-11, G-12, G-15) compensados aritméticamente por el split de G-01 en 3 filas (+2) y el merge G-08+G-09 (−1). G-07 tenía test físico verde (`TestG07ReplayDeterminismAndDelivery`: digest idéntico en schedules {32}/{1}/{7,3,1} + delivery NOT_REPRODUCIBLE declarado) sin fila; G-12 y G-15 estaban cubiertos por fixtures parciales reales (classifier+single-submit+boot-recovery; puertos disabled+lease misaligned+redaction) sin fila; **G-11 no tenía ningún fixture propio** (sin WS out-of-order, settlement FAILED/reorg, transfers externos, REST paginado concurrente). **Defectos demostrados en el baseline:** (a) `go test ./...` rojo — `TestOutboxPrimitiveIsIdempotentAndSilent` fallaba determinísticamente: la migración 0035 creó `account_outbox` con `namespace NOT NULL`, el DDL del primitivo S03 quedaba sombreado por `IF NOT EXISTS` y `INSERT OR IGNORE` tragaba la violación reportando `inserted=false` en el primer enqueue (llamadores de producción no afectados: el Coordinator escribe con namespace por SQL directo); (b) `TestRecordAgainstFixtureWSServer` flaky bajo race (3/3): el fixture enviaba un solo book y un shard cuyo primer book precede al drain de constraints queda honestamente `SYNCING` hasta el siguiente snapshot — bajo race el book podía aterrizar tras el último drain; (c) el harness `certify` asignaba PASS a un bundle de otro gate puesto en el path esperado (sin chequeo de identidad declarada) y sus 12 filas suite-backed eran PASS incondicionales incluso con cero evidencia; (d) sin pin de baseline en el harness. **Correcciones por ownership (`879886f`, sin cambios de algoritmos/gates/criterios):** primitivo outbox alineado con el schema canónico (namespace en `OutboxEvent`/`OutboxTableDDL`/INSERT) + regresión contra la tabla migrada real; fixture record con books periódicos como el servicio real (race x4 + non-race x4 verdes); `certify` verifica la identidad de gate declarada por cada bundle (mismatch → FAIL → `M4_BLOCKED`; demostrado: swap G-05→path G-04 ahora `M4_BLOCKED`, antes `M4_CERTIFIED_NON_LIVE`), restaura el denominador con estados honestos (G-07/G-12/G-15 PASS suite-backed con detalle de fixtures exactos; **G-11 NOT_RUN in-scope**) y agrega regresión que pinea denominador, rechazo de identidad y degradación sin evidencia. **Verificación física final en `879886f`:** build/vet/test/race verdes (24 paquetes), tidy sin diff, diff-check limpio; certificado ahora `M4_PARTIAL` (26 PASS, 0 FAIL, 1 in-scope NOT_RUN, 5 diferidos). **Cobertura reproducible (perfil `-cover` global):** módulo 84.1%, frames 82.4%, supervisor 64.7%, experiment 68.4%, account 77.1%, strategy 75.9%, replay 79.7%, simulator 79.8%, economics 78.1%, risk 82.9%, cmd 50.4% (composition root); ≥90%: protocol 94.7, catalog 95.1, gamma 97.5, config 97.8, archtest 97.1, foundation 94.0. Clasificación de brechas: **TESTABLE_CRITICAL** = G-11 fixtures (única brecha de gate); **TESTABLE_NONCRITICAL** = inyección de fallos I/O en backup/boot (validación y cleanup), ramas de validación de identidad en `strategy.NewInstance`, backpressure inbox-full de frames (demostrado físicamente en E2E `screen` sobre journal de 4621 records: `pausing the run` exit 1 — comportamiento contractual), helpers de error-mapping de account, `screen` CLI sin paging (backpressure correcto, límite de escala del comando demo); **DEFENSIVAS** = unlocks y ramas de error de driver/JSON imposibles por API; excepción heredada S04 seam os.Exit. Ninguna rama crítica del mandato (barreras/fencing, timeouts/backpressure, snapshot missing/INELIGIBLE, shutdown/crash, backup/restore, leases/desabled, disco, corrupción) quedó sin test o evidencia física; el piso 95% sigue incumplido a nivel módulo y requiere decisión owner por paquete. **E2E selectivos:** `journal verify` (4621 records, NO_INTEGRITY_FAILURES, cursores consistentes), `manifest build`+`replay` digest `7f24c081…` idéntico en schedules default y `1` (G-07 sobre evidencia viva), `screen` (3 frames, 0 oportunidades, RUNNING, sin órdenes), `experiment shadow` SINGLE (920 frames entregados, 920 oportunidades, 0 delivery errors, outcome INCONCLUSIVE honesto — reproduce el cierre S13), `supervisor backup` (backup_complete=true, witness 65) → `restore` a target limpio (8/8 verificados, degraded=false, recovery+frontier 65) → `journal verify` restaurado sin fallos, boot LIVE_DISABLED/NO_LEASE/readiness_live false; sin credenciales ni órdenes reales. **Fix upstream WS `event_type` RATIFICADO como corrección trazada de S06/S02:** fixture real sanitizado (data pública de mercado) con provenance en `testdata/books/fixtures/manifest.json`, separación estricta por superficie (solo `marketws.go`), `type` documentado first-class, `event_type` variante demostrada aceptada, ni-type-ni-event_type → drift fail-closed (test 5), unknown crítico `new_market` → raw preservado + TypeKnown=false, extras book preservados verbatim, compatibilidad book/epoch/quality regresada por G-05 14 escenarios + books suites. Observación menor: manifest de fixtures WS sin SHA por archivo (integridad delegada a Git). **Decisiones owner pendientes:** (1) G-11 — mandatar fixtures de reconcile (WS out-of-order, settlement FAILED/reorg, transfers externos, REST paginado concurrente) o scoping explícito con justificación; (2) coverage — aprobar excepciones por paquete con líneas concretas (supervisor 64.7%, experiment 68.4%, strategy 75.9%, account 77.1%, economics 78.1%, replay 79.7%, simulator 79.8%, frames 82.4%, cmd 50.4%) o mandatar elevación; (3) `certify` — decidir si las filas suite-backed exigen verificación dentro del harness (ej. receipt de corrida) o permanecen como consolidación documentada; (4) ratificar el re-baseline `879886f` como base de la certificación corregida. G-16…G-19 y G-14b permanecen NOT_RUN/IMPLEMENT LATER por diseño; LIVE_DISABLED sin cambios; sin push.
