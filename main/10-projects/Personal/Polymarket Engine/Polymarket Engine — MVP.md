...
- [ ] NORMAL: M2-S02 Protocolo wire — **en ejecución en paralelo** (working tree con seam declarado PASS para S03 en `testdata/protocol/evidence/s02_seam.json`; commit pendiente) #owner/me #type/dev #area/personal
- [/] NORMAL: M2-S03 Capture+Persist — **implementado (2026-09-17), checkpoint y revisión owner pendientes:** `internal/capture` (envelope M1.6 completo, carriles EVIDENCE/RUNTIME con cuotas count/bytes y reserva, journal segmentado length-prefixed CRC32 + footer SHA-256, seal inmutable fsync+rename+dirsync+manifest atómico, recovery con sufijo inválido preservado como evidencia y discontinuidades durables, integridad por clase ACCOUNT_FACT/RESEARCH_EVIDENCE, vista consumer clamppeada a `durable_seq`), `internal/persist` (SQLite WAL synchronous FULL busy deadline vía modernc encapsulado, migraciones embebidas 0001–0003 forward-only con checksum y rechazo explícito de schema futuro, single writer serial con transacciones breves y timeout, cursors por reducer/namespace con regresión y `applied_seq > durable_seq` imposibles, outbox primitive idempotente sin sender, witness `capture_frontiers`), `cmd/engine journal verify` (segmentos/rangos/frontera/holes/tails/clases/checks explicados, exit 0/1/2, cross-check cursors↔journal); quality: build/vet/test/race/tidy verdes, coverage total módulo 95.2% (capture 95.7/persist 93.2/cmd 91.6 por seam os.Exit); evidencia `testdata/capture/evidence/g06.json` (8 escenarios G-06 parcial, cierre S06) + `g02b.json` (4 escenarios G-02b parcial, cierre S11) + insumos G-14, con verificación física del CLI (limpio exit 0; byte corrupto → FAIL explicado exit 1) #owner/me #type/dev #area/personal
