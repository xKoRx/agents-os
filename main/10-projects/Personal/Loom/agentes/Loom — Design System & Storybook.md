...
> - [x] Storybook: instalar `@storybook/vue3-vite` compatible con Vite 8/Vue 3.5, main.ts preview, scripts `storybook`/`build-storybook`, aislamiento de builds, gitignore #owner/agent #type/dev #area/personal ✅ 2026-09-17 (SB 10.6 + addon-docs)
> - [x] Tokens: extraer `:root` → `src/styles/tokens.css` + `base.css` compartidos (app y Storybook cargan los mismos) #owner/agent #type/dev #area/personal ✅ 2026-09-17
> - [x] Componentes compartidos con consumidor real: Button, Badge, Panel+SectionTitle, SectionHeader, MetadataPanel, CopyButton, Breadcrumbs; StateBlock reubicado en ui/ #owner/agent #type/dev #area/personal ✅ 2026-09-17
> - [x] Stories por componente (variantes reales) + autodocs + controls #owner/agent #type/dev #area/personal ✅ 2026-09-17 (9 familias, 43 stories + docs)
> - [x] Migración incremental de las vistas a los componentes compartidos (comportamiento conservado) #owner/agent #type/dev #area/personal ✅ 2026-09-17
> - [x] Tests focalizados nuevos donde aporten valor #owner/agent #type/dev #area/personal ✅ 2026-09-17 (+8 → 126 PASS)
> - [x] Gates G1–G9 (visual G10 con navegador) + commits con evidencia #owner/agent #type/dev #area/personal ✅ 2026-09-17
> - [x] Documentación mínima para agregar nuevos componentes (docs del design system en el repo) #owner/agent #type/dev #area/personal ✅ 2026-09-17 (`src/ui/README.md`)
> - [x] Entrega: bitácora + RESULT en el vault; tarea puente a Review #owner/agent #type/dev #area/personal ✅ 2026-09-17
...
- **2026-09-17 (FUNDACIÓN COMPLETA — ENTREGADA A REVIEW):** implementación completa en `feature/loom-design-system` (commits `5f7270d` design system, `b0e24a2` fix smoke). **Entregado:** tokens en `src/styles/` (app y Storybook importan el mismo entry); 9 componentes en `src/ui/` con contrato tipado (LoomButton md/sm/disabled · LoomBadge con vocabulario `LoomBadgeTone` en `badge-tone.ts` · Panel · SectionTitle · SectionHeader · MetadataPanel + `MetadataRow` · CopyButton + `useClipboardCopy` deduplicada · LoomBreadcrumbs · StateBlock reubicado); migración de App.vue + 5 vistas con estilo y comportamiento conservados (2 correcciones de consistencia demostrables: envelope unificado con `envelopeMetadataRows` — NoteView ahora muestra slug y status-badge igual que ProjectView — y breadcrumbs sin separador huérfano); Storybook 10.6 dentro de `internal/web/` con 43 stories + autodocs + controls y decorator de router en memoria para Breadcrumbs; `src/ui/README.md` con el contrato para nuevos componentes; candidatos sin implementar documentados (DisclosurePanel, NavigationItem, Folder*, NoteInspector, ProjectRelationPanel, SearchResult — requieren review UX). **Gates:** G1 dev server PASS · G2 build estático PASS (52 entries) · G3 stories renderizan PASS · G4 vue-tsc PASS · G5 vitest 126/126 PASS · G6 `npm run build` + `make web` (npm ci) PASS · G7 `make build` + `go test ./...` + `go vet` PASS · G8 API sin cambios (solo dist embebido; e2e 6/6 PASS sobre vault real) · G9 cero referencias a storybook en dist/binario PASS · G10 validación visual real en navegador PASS (5/5 vistas de la app contra vault real; 7 stories + autodocs verificados con screenshot). **Hallazgo y fix fuera de alcance:** el smoke del Makefile esperaba 404 en `/meta` con snapshot publicado — falla pre-existente en `848fb28` (verificado con worktree limpio); corregido a 200 en commit separado. **Hallazgos de integración Storybook 10.6:** `@storybook/addon-docs` debe declararse explícito en `addons` (sin él, docs vacías y stories sin resolver hasta warming) y `storybook-static/` quedó fuera de `go:embed` + en `.gitignore`. **Preservación:** el review UX/IA del owner (`specs/FEAT-LOOM-V01/UX-IA-REVIEW-V0.1.md`, untracked) apareció durante la ejecución y se preservó sin tocar. Sin push, sin merge, sin cierre de la Review v0.1 del owner.
