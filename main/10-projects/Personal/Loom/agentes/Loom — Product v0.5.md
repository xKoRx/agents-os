...
- **RC_READY + HOTFIX (2026-09-19):** edge case de clasificación corregido por mandato de Dirección Técnica y publicado en `origin/feature/loom-v05 @ 94c8fcf` (== local; base `144482e`; sin merge; sin writer; puente sigue `[r]`): `buildHumanActions` apilaba cada wikilink verificado sin deduplicar y una puente que nombra dos veces el MISMO subproyecto (título canónico + alias + ancla) caía en `ambiguous_reference`; la verificación ahora DEDUPLICA por DocumentID verificado — `ambiguous_reference` exige ≥2 subproyectos DISTINTOS, `missing_bridge` y el resto del contrato intactos — con regresión RED→GREEN (`TestHumanActions_DuplicateReferencesToSameSubproject`) y precisión documental en `HUMAN-ACTIONS.md` (identidad de la ambigüedad). Gates @ `94c8fcf`: lint/vet PASS · go 6/6 · race index+serve limpio · vue-tsc limpio · vitest 367/4 · smoke · e2e 10/10 · live-refresh 6/6 · G9 0×"storybook" · secret scan limpio · `verify-v05.mjs` 40/40 en Chromium con binario reconstruido (SPA y fixtures byte-idénticos: storybook/dist/hex/fixtures lint no afectados). Incluye commit mecánico separado (`94c8fcf`) de deuda gofumpt heredada del baseline en `internal/serve` (3 archivos, cero cambio de comportamiento; el gate previo pasaba por el fallback `gofmt`, que no veía esa deuda).
...
> - [r] Entrega al owner (RESULT RC_READY + evidencias + guías de review) #owner/agent #type/dev #area/personal — publicada `origin/feature/loom-v05 @ 144482e`; HOTFIX de clasificación @ `94c8fcf` (dedup de referencias por DocumentID), a review del owner
...
- **2026-09-19 (HOTFIX — dedup de referencias del HAC):** Dirección Técnica reporta el edge case: una puente con dos wikilinks al MISMO subproyecto (título canónico + alias) se diagnosticaba `ambiguous_reference` en vez de aprobación — el comentario de `wikilinkTargetsInText` prometía dedup por resultado de verificación que el caller nunca hizo. Reproducido con RED (`TestHumanActions_DuplicateReferencesToSameSubproject`: canónico + alias de envelope + variante con ancla → exactamente 1 aprobación y 0 `ambiguous_reference` espurios; puente con 2 subproyectos DISTINTOS sigue ambigua), corregido con dedup por `vault.DocumentID` verificado (set `seen` por tarea) y contrato precisado (`HUMAN-ACTIONS.md`: la ambigüedad exige ≥2 DocumentID distintos). Gates sobre `94c8fcf`: lint PASS tras cerrar en commit mecánico separado la deuda gofumpt heredada del baseline `144482e` en `internal/serve` (3 archivos; el gate anterior pasaba vía fallback `gofmt`), go 6/6, race index+serve, vue-tsc, vitest 367/4, smoke, e2e 10/10, live-refresh 6/6, G9, secret scan, `verify-v05.mjs` 40/40 en Chromium con binario reconstruido (SPA/fixtures intactos → storybook, dist reproducible, hex/rgba y fixtures lint no afectados). RESULT: **HOTFIX_PASS** — RC_READY se mantiene @ `94c8fcf`; puente `[r]` intacta; sin merge a master. Run register: `80-agents/journal/agent-runs/2026-09-19-1650-zcode-glm-loom-v05-hotfix-ambiguous-dedup.md`.
