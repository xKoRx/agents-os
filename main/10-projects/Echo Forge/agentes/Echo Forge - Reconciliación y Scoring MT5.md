...
- **Release 6182 — estado real al 2026-09-19 (TOP C9R).** Build 6182 CERTIFICADO en source (`bcf67be` allow-list explícita 6090/6140/6180/6182 + `c1d24c1` cierre C6 R1–R4: diagnóstico missing-build-row sin lista inline stale, SPEC 3.2 alineada, release mode obligatorio `SQX_MT5_REQUIRE_B6182_FIXTURE=1` que convierte fixture ausente/corrupto/skip en FAIL, contract tests 99.1% coverage) y revisión TOP Gate A = PASS: delta sin cambios a gramáticas/scoring/fórmulas, `ParserVersion` sigue `mt5-report.v1`, vecinos 6181/6183 rechazados. Release gate re-ejecutado sobre la integrada `codex/f05-r3-integration` @ `3f6cd110` con el HTM auténtico RERUN-3 (13 074 872 B, SHA256 `21917e14…8b5c`, byte-verificado): paquete completo en release mode **53 PASS / 0 FAIL / 0 SKIP** (7 goldens 6182 + crosschecks contractuales sobre bytes auténticos). La release PRODUCTIVA con 6182 sigue SIN PUBLICAR: requiere owner (versión `release-authority` post-merge, publish `deploy_release.sh --release-only`, binarios con `vcs.revision=3f6cd110…` ya construidos como prueba de build). Hasta entonces, un terminal MT5 auto-actualizado a 6182 vuelve a fallar `mt5_reconcile` fail-closed — comportamiento correcto, no defecto.
