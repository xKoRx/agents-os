...
- **2026-09-25 (gate de segmento e ingreso frontend)** — Se verificó que `ads-signals-frontend/test3` es Legacy y la route `x-rio-scope: alpha` del dominio Playmaker compartido existe sólo en Non-production, con target `alpha-api-nonprod`; Fury no permite elegir ese target en una regla Legacy. Requests de oficina al host Legacy con y sin header respondieron `/ping` y `/v3/api-docs` igual; el host de oficina Non-production rechazó origen externo con `421`. Se inició un Blue Green de `ads-signals-frontend/alpha-nonprod` a `0.0.3-poc-scopes-alpha-header`: réplica sana, boot y swap de tráfico completos; cierre `Finish` pendiente. `signals.adminml.com` selecciona `test3` con `meliLab=test3`, pero `meliLab=alpha-nonprod` cae en production y el acceso directo al scope Non-production responde `401`. El ingreso/autenticación del frontend alpha y una request BFF verificable hacia Playmaker alpha siguen siendo gates de integración.
- **2026-09-25 (continuación: POC desde `test3`)** — El requisito operativo quedó fijado en desplegar el frontend en `test3` Legacy y alcanzar `rio-playmaker/alpha-api-nonprod`. El dominio actual `rio-playmaker-test.melisystems.com` es de exposición **Internal** (sólo mismo segmento); por eso su regla Non-production por `x-rio-scope: alpha` no recibe tráfico desde `test3`. Fury ofrece un dominio **Inter segment** para Non-production con hostname `rio-playmaker-test.nonprod.melisystems.com`. Se preparó un parche local en `ads-signals-frontend/feature/poc-scope-routing` para que sólo `test3` use ese hostname con `baseURL` (el `meliDomain` de RestClient no acepta puntos), manteniendo `X-Rio-Scope: alpha` en todos los verbos. La configuración efectiva `PLATFORM=fury SCOPE=test3` resolvió host, routing y scope como se esperaba; producción y `test2` permanecen en sus destinos previos. La creación del dominio/ruta en Fury, el commit/push y la request real desde `test3` están pendientes; la versión Fury `0.0.3-poc-scopes-alpha-header` todavía no contiene este parche. El Blue Green anterior de `alpha-nonprod` sigue con rollback pendiente de confirmación.
