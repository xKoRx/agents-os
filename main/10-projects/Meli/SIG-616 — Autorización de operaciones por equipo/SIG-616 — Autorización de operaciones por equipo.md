...
- **Actualización F5 — feedback de F4 integrado (2026-09-24):** `feature/operation-authorization-by-team-f5@85a0c3bfc` integra F4 `40d5f9b22` por merge commit, conservando los casos ya cubiertos por F5 y los comentarios aplicados a F4 sobre relaciones same-DP y cascade sin equipo. En los conflictos se preservaron la regla de rol privilegiado F5 cuando hay team y la matriz YAML F4/F5. [PR #1182](https://github.com/melisource/fury_rio-playmaker/pull/1182) apunta a ese HEAD, está `MERGEABLE` y sus cinco checks terminaron `SUCCESS` en CI #5498. Las ramas mock publicadas son `feature/sig-616-auth-p5-committer-test3-v25@edff29c26`, `feature/sig-616-auth-p5-viewer-test3-v26@4f1e29e6` y `feature/sig-616-auth-p5-deployer-test3-v27@d5ef6d48c`. Fury terminó `0.1.21-p5-committer-allowed` (#1740), `0.1.22-p5-viewer-denied` (#1741) y `0.1.23-p5-deployer-allowed` (#1743) en `FINISHED`; la última valida el perfil de prueba para inactivación. El grant deployer sólo está en el profile `test3`; no se desplegó. Smoke F1 y review humano siguen pendientes. No se editaron casos de uso productivos.
- **F5 — cierre de mutaciones, baseline anterior (HEAD `141eacbc5`):** la versión anterior protege por YAML las mutaciones de Data Products, config/rename de pipeline, definiciones e import authorizations que faltaban en la UI actual de Signals. El guard usa owner persistido y deniega ownership incompleto. La auditoría se hizo sobre `ads-signals-frontend@origin/develop:9bf76ffc`; Playmaker no es dueño de favoritos personales ni Entities. Pasaron 31 selectores focalizados, dos checks L0/LOCAL_STACK con cleanup y 4.107 pruebas en regresión completa (0 fallas, 2 skips). Las variantes previas fueron `feature/sig-616-auth-p5-committer-test3-v23@9b2b7d1f2` y `feature/sig-616-auth-p5-viewer-test3-v24@8f91234e4`, con Fury `0.1.19-p5-committer-allowed` y `0.1.20-p5-viewer-denied` en `FINISHED`. La CI del HEAD anterior pasó en el build #5492. La ampliación y sus excepciones están en [[SPEC técnica — Slice 5 — Actions restantes]] como addendum aprobado.
...
| `rio-playmaker` | `feature/operation-authorization-by-team-f5@85a0c3bfc` | `feature/operation-authorization-by-team-f4@40d5f9b22` | [SIG-621](https://spellbook.adminml.com/projects/SIG/specs/SIG-621) | [[SPEC técnica — Slice 5 — Actions restantes]] | Publicado en [PR #1182](https://github.com/melisource/fury_rio-playmaker/pull/1182), `MERGEABLE`; CI #5498 `SUCCESS`; mocks test3 committer/viewer/deployer `FINISHED`; sin deploy |
