...
root: false
status: in-progress
status_detail: "R2 EJECUCIÓN MANDATO (2026-09-18 noche): owner aprobó acciones 1+2 v3, rechazó acción 3 (reboot). ACCIÓN 1: drop-ins fail-closed instalados en proxmox-backup Y -proxy (extensión: proxy=ingreso de backups, user backup, sin dep del mount), verificación manager-level PASS + prueba semántica neg/pos PASS (mount inexistente→exit 5), servicios intactos — PENDING ACTIVATION (servicios running pre-drop-in; reinicio/reboot GATED), FAIL-CLOSED PASS NO declarado. ACCIÓN 2: driver preparado+validado (dryrun PASS), NO ACTIVADO por gate insatisfecho; ventana 06:00-07:25 reconciliada con timers reales; CT 148 rootfs en pool1 Ceph (corrección placement) → auto-gated con pool1 ≥85% (87.21%), set efectivo 6 CTs. Baseline v3 §0 sin drift. R2 IN-PROGRESS; gates residuales: activación fail-closed, reboot persistencia, decisión D."
priority: P2
progress: 0
icon: 🖥️
...
- [x] **AGENT-TASK-02-8**: aplicar fail-closed del datastore PBS (drop-in systemd; bundle v3 Acción 1) — EJECUTADO 2026-09-18 noche bajo mandato owner (acciones 1+2 aprobadas; acción 3 NO). Drop-ins instalados en `proxmox-backup` Y `proxmox-backup-proxy` (extensión justificada: el proxy es el ingreso de backups, corre como `backup`, sin dependencia del mount; sólo-API dejaba el ingreso desprotegido). Verificación positiva manager-level completa (DropInPaths, Requires, RequiredBy inverso, servicios intactos); prueba semántica negativa/positiva PASS (unidad con mount inexistente → exit 5; mount real → exit 0). **PENDING ACTIVATION**: servicios running desde antes del drop-in; daemon-reload no protege instancias vivas; activación efectiva requiere reinicio/reboot (GATED). FAIL-CLOSED PASS NO declarado.
  - tags: [agent, done-2026-09-18, pending-activation, pbs-config]

- [ ] **AGENT-TASK-02-9**: ejecutar medición 7 días (7 CTs; bundle v3 Acción 2) — **NO ACTIVADA: gate insatisfecho** (depende de 02-8 efectivo). Driver preparado y validado (dryrun ×3 PASS): `~/aranea/work/r2-pbs-20260918/measurement/` (r2-measure-day.sh serializado + helper + README-ACTIVACION.md; ventana 06:00–07:25 reconciliada con timers reales; caducidad 7d + auto-disable). CORRECCIÓN material: CT 148 rootfs en **pool1 Ceph** (no local-lvm como decía v3) ⇒ auto-gateado mientras pool1 ≥85% (87.21% al cierre); set activo efectivo = 6 CTs. Activación = 2 comandos en README cuando el gate esté demostrado.
  - tags: [agent, gated, driver-ready, measurement]

- [ ] **AGENT-TASK-02-10**: reboot controlado PBS en ventana autorizada (bundle v3 Acción 3; valida fstab + drop-in + datastore tras reinicio) — GATED ventana. **NO aprobado por mandato 2026-09-18 noche** (acción 3 explícitamente rechazada). Es la vía natural para: (a) demostrar activación efectiva del fail-closed, (b) contraprueba de arranque sano, (c) habilitar la activación del piloto de medición.
