...
- **2026-09-19 (R2 mandato 2 — reboot + activación)** — **RESULT PASS.** *Preflight* (read-only, 22:15–22:21): baseline v3 §0 sin drift (5 huellas), VM 180 = PBS verificada por config live (name/net0/scsi1 serial pbs-data), cero tareas PBS/PVE activas (último match ps = kernel `[arc_prune]`), drop-ins hash 56eefebf intactos, datastore 1G/279G + 429 chunks + snapshot 155 presente, quorum 5 nodos, Ceph HEALTH_WARN (nearfull pre-existente), auth API pre-reboot HTTP 200 (token `backup@pbs!aranea`, secreto trasladado por SSH a `~/aranea/secrets/.t21-pbs-api-token` 600, header correcto `Authorization: PBSAPIToken id:secret`). *Reboot:* único, ordenado vía SSH 22:23 (sin qm reset/stop); ciclo real demostrado por `uptime -s` del guest → **boot 22:24:05** (lección: `qm status` permanece `running` durante el reboot de un guest — proceso QEMU no muere; primer sondeo "+2s running estable" fue falso y se descartó). *Post-boot 15/15:* disco by-id+UUID f7726067, mount rw, datastore main, snapshot intacta, servicios active, **Requires/After de la mount EN VIVO en ambos servicios** (raw del manager: `Requires="mnt-pbs\\x2ddata.mount"` — el grep con escape simple fallaba por el quoting de systemd), RequiredBy inverso, list-dependencies con la mount, API 200, auth 200, aranea-pbs active **5/5 nodos**, rootfs 8G con `/` limpia, chunks 429 sin drift, único escritor del mount = proxy (PID 664, user `backup`). **FAIL-CLOSED PASS declarado (A/B/C/D):** B activa demostrada por el boot real bajo drop-ins; C por el ciclo completo; D heredado de la prueba negativa segura v3 (exit 5/exit 0) + caminos de escritura API+proxy cubiertos; `Requires/After` suficiente, BindsTo no requerido; prueba física con des-adjuntar scsi1 = drill opcional de ventana futura. **Piloto ACTIVADO:** driver reutilizado con 2 ediciones mínimas — (1) CT 148 excluido PERMANENTE (orden expresa; `GUEST_ORDER` 7→6, mecanismo `POOL1_GATED` eliminado), (2) fix `disable_timer()`→`sudo -n` (defecto real hallado en TEST B: el servicio corre como `hermes` y `systemctl disable` sin sudo falla — el auto-disable por caducidad/ABORT habría fallado silencioso). Validaciones: dryrun 6 CTs exit 0 con 148 ausente; TEST A ventana (SKIP fuera de horario, exit 0); TEST C camino ABORT completo con copia /tmp del driver (BASE aislado, preflight simulado en fallo, ventana neutralizada) → ABORT exit 2 y **timer real inactive+disabled por el propio driver**, luego re-enable y limpieza total. Programación: ventana sáb 19 ya vencida al activar (gate PENDING durante 06:00–07:25) ⇒ sin corrida fuera de horario (§7); **primera ejecución PROGRAMADA dom 2026-09-20 06:05**, expiración absoluta **2026-09-26** pre-anclada en `state/{first_run_date,deadline_date}` (auto-disable el 27; máx 7 ciclos). Timer `aranea-r2-measure.timer` enabled+active (waiting), trigger confirmado dom 06:05:00 -03. Sin secretos en logs; jobs.cfg AUSENTE post-ejecución; R1/CTs/Ceph/PVE sin cambios. Mutaciones: reboot PBS + 2 units locales + driver/state — rollback en change log. Evidencia: `80-agents/journal/logs/2026-09-19-r2-reboot-failclosed-activation.md`.
