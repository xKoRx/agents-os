...
- **Errata noche-4 (placement freeze):** congelación final de organización de recursos (fase de optimización de storage; **su gate NO es requisito de Backup/DR** — ver errata noche-5) — placement v2 [[PLACEMENT-DECISIONS-20260920]] §F (MIGRATE=ninguna activa), presupuesto [[CAPACITY-AND-RESERVATIONS]] (sin doble-asignación), plan consolidado [[FIRST-MAINTENANCE-WINDOW-20260920]] §8.2 (viernes=MIGRATIONS_NOT_READY; sábado=K2/K1/P0-1 sin P0-2), gate [[STORAGE-ORGANIZATION-FREEZE]] `STORAGE_ORGANIZATION_COMPLETE` como cierre de la fase de optimización de storage. Las secciones 0-6 de este plan quedan como registro; D-NEW + estas erratas mandan.
- **Errata noche-5 (mandato correctivo BACKUP FIRST — dependencia revocada):** `STORAGE_ORGANIZATION_COMPLETE` **NO es requisito de Backup/DR**. Secuencia aprobada por el owner: (1) optimización de storage — planificación completa desde ahora; (2) Backup/DR — implementación y certificación sobre el placement actual; (3) migraciones — ejecución posterior, cada una protegida por backups verificados; (4) recertificación de cada servicio después de migrar. Reglas fijas: **STORAGE_OPTIMIZATION_PLANNED no es requisito de BACKUP_BASELINE_VERIFIED; BACKUP_BASELINE_VERIFIED por unidad sí es requisito de su MIGRATION_READY.** Los backups certificados vigentes (A1/R1/R1.5/R2/G1A/G1B) no se detienen; la réplica inicial de pool0 se dimensiona y ejecuta cuando SUS gates G-REP-0..5 (capacidad, integridad, ventana) estén satisfechos — sin depender de completar migraciones. Análisis disco por disco: [[ANALISIS-DISCO-POR-DISCO]].
