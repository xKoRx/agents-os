...
- [ ] **T-21b · 21sep · Fix R1 tar-race (second-brain)** — Ejecutor: Ariadna. Alcance: el run R1 07:36 del 21sep falló en second-brain (`tar: main: file changed as we read it`); corregir el paso tar de `~/aranea/bin/r1-backup.sh` (o el flujo) para que el run siguiente capture el vault consistente (snapshot/quiet o tolerancia controlada del race) SIN tocar horarios ni las otras 2 unidades. Dependencias: ninguna. Estado: OWNER_GATE — es mutación de código en hermes del mecanismo congelado R1: diff+aprobación owner antes de tocar. Gate: OK owner sobre el diff. DoD: run siguiente con 3/3 unidades OK + manifest íntegro; si owner difiere, documentar el fallo conocido como deuda en RUNBOOK §0.
- [ ] **T-22 · 22sep · Decisiones de placement y capacidad listas para owner** — Ejecutor: Ariadna (prep) → Owner (decide). Alcance: una página con los bloqueantes 5/7/10 (D-piloto con criterio alternativo post-pérdida del run 21sep; datasets REPL; decomisiones 100/151) + estado Ceph del día (condición de alerta K2). Dependencias: T-21a. Estado: PENDING_VERIFICATION (pendiente lectura Ceph del día y empaquetado). Gate: decisiones son del owner. DoD: bundle de decisiones entregado y fechado en [[ARANEA-CONTINUIDAD-Y-VENTANA-25-26-SEP]].
- [ ] **T-23 · 23sep · Paquete completo de backups (borrador)** — Ejecutor: Ariadna. Alcance: diseño de jobs vzdump B1 (lista Tier 0 pendiente 018 → usar propuesta de contrato §2 + matriz como insumo), exclusiones §6.3, retención propuesta, horarios sin solapar dumps A1 (03:00-03:45) ni R1 (04:00) — TODO en borrador sin activar nada. Dependencias: T-21a. Estado: PENDING. Gate: activación = OWNER_GATE (MP-03). DoD: documento de diseño de jobs revisable en el proyecto, sin comando ejecutado contra PVE/PBS.
