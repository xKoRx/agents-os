...
- **Backups: 6 timers activos y verificados hoy** (A1 03:00/03:20, R1 04:00, R1.5 05:00, R2 06:05, pve semanal sáb). PBS al 17% (48G/295G), verify tasks OK en archive, 6 CTs piloto + 12 snapshots host presentes.
...
1. **CRÍTICO — Ceph nearfull acelerando + slow ops nuevos.** osd.0/2: 87,82/87,78% (818G/932G) vs 85,7% el 19sep (~+19G/OSL día). Los 2 OSD llenos además reportan slow operations BlueStore (nuevo vs assessment, que no tenía slow-ops). Escenarios del assessment siguen: caída de kronos = recovery imposible; NO_GO pool1 vigente. El crecimiento actual (~19G/día si se sostiene) alcanza nearfull-oficial (-7G al 19sep, ya consumido) y presiona backfillfull (~33G hoy) en ~2 días si no frena. ACCIÓN: identificar escritor de pool1 (ver PLACEMENT §Ceph) en la primera acción de la ventana; NO intervenir el cluster fuera del carril Ceph.
...
3. **MEDIO — mcps (113) rootfs 83% (17G/20G)** con ~8,5G reclamables (docker build cache 4,1G + imágenes 4,4G). Plano MCP sano (26/26 containers up), pero un rootfs lleno en el LXC de acceso degradaría el plano de acceso. `docker system prune` = mutación gated en ventana (requiere OK owner; afecta sólo cache/imágenes colgantes).
...
- **Echo (métricas ARGUS)**: 25 sesiones activas {mt4-ttp 6, mt4-real 10, mt4-ftmo 2, mt4-demo 7}, 6 pipes ejecución + 7 referencia, core (140) con pipeline viva; trades/24h = 2 abiertos (cero fallos de journal); kafka_produced fluyendo. Versión bridge/core 2.0.0, deployment=production.
