...
## Delta sesión mié 23 — FREEZE FINAL DE EJECUCIÓN 25-27 SEP (14:0x-14:4x -03, mandato owner ONE-SHOT)

**Mandato**: cerrar definitivamente el plan del fin de semana sin conversar con el owner; ventanas válidas = días hábiles ≥07:00 CLST + madrugada sábado/domingo; eliminar toda dependencia de ejecuciones 04:00; cero mutaciones productivas. **Entregable central: `~/aranea/work/cierre-preparatorio-20260921/PAQUETE-FREEZE-25-27SEP.md` (v3 FREEZE — tabla única orden|ventana|operación|estado|gate owner|preflight|ejecución|PASS|ABORT|rollback|dependencia; el v2 de 22sep queda SUPERSEDED con banner + sha256 `71431a2e…`)**. Freeze T-24 ADELANTADO y ejecutado hoy: `~/aranea/work/continuity-20260923/FREEZE-T24.md` + banner en MANDATO-JUEVES-24 (jueves 24 ya no tiene tarea; sólo quedan las ventanas V1/V2/V3).

- **Hallazgos medidos 23sep**: 3ª noche consecutiva de apagado nocturno de hermes (poweroff 00:39:34, arranque 08:05:41) → R2 23sep perdido (3º) → serie máxima posible **4/7** (D-A muerto; P4=D-B sin cambio). R1 23sep por catch-up 08:05 (run 20260923-080543): traefik OK · hermes-state OK · second-brain **FAIL 4º día** (sha null; patch NO aplicado por gating, hash base `bca1d148…` intacto). A1 catch-up OK (PG/Mongo 08:06-08:07 en PBS).
- **K2 23sep — condición cerca de disparar**: L1 13:40 osd.0/2 = **88,21/88,22%** (margen ≈0,8pp a ≥89%); sondeo 13:49 88,23/88,25% (Δ9,5 min NO válido para la condición; lectura 2 válida ≥14:40 registrada en [[K2-CEPH-RISK-20260920]]). → **CERRADO 22:24 (sesión de cierre)**: L2 válida (Δ8h44m) = **88,31/88,34%** (+0,10/0,12 pp — ritmo nocturno lento, recesión activa); **condición ≥89% ×2 NO disparada; sin escalamiento owner hoy**; protocolo continúa jue 24/vie 25 (acción gated `qm shutdown 125` sigue sólo preparada).
- **P2a certificación RE-DISEÑADA** (cambio de autoridad): los "3 runs 04:00 mié-jue-vie" quedan eliminados; DoD vigente = UNA ejecución real post-fix en horario hábil (orden 2 de la tabla freeze; disparo regular o `systemctl start aranea-backup-r1.service` manual). Fixture no certifica producción; sin run real no hay PASS. Delta aplicado en el GATE (§"DELTA FREEZE 23sep").
- **P1-v3 preflight re-verificado live 23sep — PASS completo**: VFree local-kronos 267,51G · scsi1 intacto (serial pbs-data 300G) · scsi2 libre · ext4 crudo sdb 295G/49G en /mnt/pbs-data · servicios PBS active · VM 180 running. Runbook vigente único (`P1-GROW-IN-PLACE.md`); ningún `qm set scsi1/scsi2` en superficies vivas.
- **W-02 CONGELADO** tal como está (instalación, manifiesto credenciales §2.4, mutex flag manual, ACTIVE, timers espejo, prueba con hermes apagada, rollback — README-W02.md + payload staged, sin secretos copiados). Implementable este fin de semana INDEPENDIENTE de B1; única dependencia dura = P2a aplicado y validado. Rama de ingesta del sábado: **A)** hermes ON ≥05:30 (decisión owner) o **B)** W-02c instalado en V1 + flag ACTIVE en apagado (prueba = corrida sb-etcd con flag); si no hay ninguna de las dos → serie R2 queda con tope 4/7, D-B certifica con la serie real y R2 NO se re-programa (renovar el timer está prohibido).
- **Orden del fin de semana**: **V1 vie 25 ≥07:00** — P2a → P2a-VAL → W-02c (si gates) → preflight T-25 GO/NO_GO (cierre Echo vía canal W-04, query SIN filtro temporal) → 018 si llegó firma. **V2 madrugada sáb 26 (ventana 019)** — prechecks W-04 02:00 → K2-check → K1 (AUTO) → P1-v3 04:30-05:45 → ingesta-proof ~06:05 (rama A/B) → REPORT.md. **V3 madrugada dom 27** = contingencia de la MISMA tabla (sin alcance nuevo; si el sáb se abortó o faltó gate, se reintenta con preflight mandando). **Fuera 25-27**: réplica pool0→pool2 (P6/G-REP-0..5, sólo staging documental), migraciones W5-post, borrado RBD/limpieza destructiva, cloud 020/021, CouchDB A3, renovar timers, P0-2/W1/W2 (cancelados).
- **Gates owner pendientes** (GATE-22SEP + su delta 23sep = único instrumento de firmas): P1-v3 · P2a · W-02a/b/c · 018 · 020/021 · A3 · 112 · G-REP-0..5. Sin firmas, el sábado NO tiene intervención autorizada (sólo K2/K1/prechecks AUTO) — freeze PARCIAL por diseño, listo para ejecutarse en cuanto el GATE reciba respuestas.
- Cambios vault: esta nota + [[K2-CEPH-RISK-20260920]] (filas 23sep). Workspaces: `PAQUETE-FREEZE-25-27SEP.md` (nuevo), GATE con delta 23sep, `FREEZE-T24.md` (nuevo), banners SUPERSEDED en `PAQUETE-EJECUCION-25-26SEP.md` y `MANDATO-JUEVES-24.md`. Cero mutaciones de infraestructura; Echo operando.

## Delta sesión mar 22 — RESOLUCIÓN INTEGRAL DE GATES (22sep 08:00-09:3x -03, mandato owner ONE-SHOT)

**Mandato**: resolver definitivamente P1, T-21b, W-02, R2/018 y consolidar el paquete 25-26sep; preparación y verificación read-only, cero mutaciones productivas. **Paquete consolidado v2**: `~/aranea/work/cierre-preparatorio-20260921/PAQUETE-EJECUCION-25-26SEP.md` (4 estados: READY/READY_AFTER_OWNER_GATE/BLOCKED/DEFER; sin comandos obsoletos). Hallazgos y entregables del día:

- **P1 RESUELTO — grow in-place (P1-v3)**: la opción (b) del paquete del miércoles era inviable por tres razones demostradas: (i) sus comandos mezclaban VG del host (`vgextend local-kronos`) con discos del guest, pero el datastore del guest es **ext4 crudo sobre /dev/sdb SIN tabla de particiones ni VG** (UUID f7726067…, by-id `scsi-0QEMU_QEMU_HARDDISK_pbs-data`); (ii) un 2º disco/dataset en `pool-kronos` (PV /dev/sdc, Patriot P3-1TB, VFree 733,87G verificado) **viola F-06 FROZEN** ("PBS datastore = local-kronos", SSD sdb); (iii) no amplía `main` (los jobs/restore/verify quedarían en un solo datastore). Solución única: `lvextend +250G local-kronos/vm-180-disk-1` (VFree 267,51G, LVs gruesos sin thin pool) + rescan + `resize2fs` online → **≈545G, sin qm set, sin reboot, datos intactos**. Runbook completo con preflight fail-closed, abort por etapa y recuperación: `cierre-preparatorio-20260921/P1-GROW-IN-PLACE.md`. Los comandos `qm set 180 -scsi1/-scsi2` quedan eliminados del paquete (el -scsi1 histórico habría pisado el datastore; el -delete jamás es rollback con datos). Trade consciente registrado: local-kronos queda con 17,51G (asignaciones nuevas de kronos van a pool-kronos, storage lvm activo).
- **ERRATA CRÍTICA de canal**: el GATE-22SEP citaba kronos como `ariadna@192.168.31.100` — **.100 es zeus; kronos = 192.168.31.120** (verificado por hostname en vivo en los 5 nodos). Ejecutar qm/lv de la VM 180 en .100 era un error material. Errata aplicada en el GATE en hermes.
- **T-21b → diff v2 (cambio material)**: el run R1 07:50 de hoy falló en LAS DOS unidades locales (`tar: main: file changed` y NUEVO `tar: profiles/ariadna/skills: file changed` — 3er día second-brain, 1er día hermes-state; manifest `20260922-075044` con sha null ×2). El diff v1 (solo second-brain) habría dejado R1 en FAIL → **SUPERSEDED** (marcado). `T21B-R1-TAR-RACE-FIX-v2.diff` (sha256 1ad62df8…) cubre ambas unidades con staging atómico (cp -a / cp --parents / rsync --exclude); `patch --dry-run` limpio sobre el hash vigente **bca1d148…** (verificado hoy); fixture v2: staging **12/12 íntegro** con escritor concurrente vs tar directo fallando, exclusiones verificadas y **pruebas negativas sin falsos PASS**. Runbook autosuficiente (aplicar/validar/rollback/DoD=run real 3/3): `continuity-20260921/T21B-RUNBOOK-v2.md`.
- **W-02 procedimiento definitivo + payload STAGED** (sin desplegar): `w02-standby-20260922/payload/` (4 pares unit/timer espejo con `ConditionPathExists` + `Persistent=false`, wrapper `aranea-sb-wrapper.sh` con **freshness guard anti-doble-ejecución** — snapshot PBS del día o manifiesto <20h → SKIP, instalador idempotente con sustituciones documentadas). Los 8 aspectos resueltos en `README-W02.md`: exclusión mutua (flag manual en rutina de apagado), manifiesto exacto de credenciales mínimas (4 claves + pw PBS ya presente; `ariadna_pbs` innecesaria en standby), sin catch-up en trading (regla <09:00 días de mercado), monitoreo v1 = logs + verificación post-arranque (alertas push sigue GAP → MP-06), comportamiento con hermes apagada (A1/etcd/pve-config corren; **R1 EXCLUIDO del standby con justificación**: sus fuentes viven en hermes), rollback 100% aditivo. Orden obligatorio: P2a/T-21b v2 primero.
- **R2: D-A matemáticamente INALCANZABLE → P4 = D-B cerrado**: hermes confirmó el **patrón de apagado nocturno** (2ª noche: poweroff limpio 01:01:48, arranque 07:50:42 — journal boots -1/-0). El run R2 06:05 de HOY no ocurrió (non-Persistent por diseño) → serie 1 OK (20) + 2 perdidos (21,22) + 4 disparos restantes (23-26) = máximo posible **5/7 < 6/7**. Si el patrón continúa, 23-26 también se pierden. La decisión D-B (certificar con la serie real tras el 26sep) es la única viva; el GATE lo tenía como baseline. Alternativa operativa para el sábado = flag W-02 (decisión owner al autorizar W-02c). A1 INDEMNIZADO tras el reboot: PG `2026-09-22T10:51:56Z` + Mongo `10:51:22Z` en PBS (catch-up Persistent OK).
- **Ceph (protocolo K2 día 22)**: lectura 1 = **87,03/87,09% a las 08:12** (HEALTH_WARN, nearfull ×2, slow ops BlueStore). Tendencia al alza (85,2 → 86,6 → 87,05 en 24h): la condición K2 (≥89% ×2 ≥1h) puede disparar hoy en sesión US (margen ≈17-18G/OSD ≈ 10-12h al ritmo vespertino). **Sólo se tomó la lectura 1 (<89% ⇒ sin condición declarada hoy); la 2ª lectura del día ≥1h después queda pendiente para la próxima sonda/owner** — serie en [[K2-CEPH-RISK-20260920]].
- **018 — propuesta técnica con evidencia (ticket sigue ABIERTO hasta firma)**: `kafka=RECONSTRUIBLE` (transporte DEV con RF=3 interno; topics efímeros por sesión — evidencia MCP 22sep: ~130 topics con decenas de `echo.commands.<sesión>` huérfanos y certs `mcp-cert-*`; estado de negocio en PG/Mongo ya protegido por A1/G1A) y `argus=RECONSTRUIBLE` (TSDB Prometheus/Loki con retención acotada por diseño; valor operacional no transaccional; 40,5G usados = mayor palanca de alivio pool1 con gate de borrado independiente). Ambas son PROPUESTAS fundamentadas — la firma es del owner (`018 = lista OK + argus=… + kafka=…`).
- **Rutas nuevas**: `cierre-preparatorio-20260921/P1-GROW-IN-PLACE.md` · `continuity-20260921/T21B-RUNBOOK-v2.md` + `T21B-R1-TAR-RACE-FIX-v2.diff` + `T21B-FIX-TEST-v2.sh` · `w02-standby-20260922/` (README + payload) · `PAQUETE-EJECUCION-25-26SEP.md` v2. Superesedidos con marca en header: `continuity-20260923/W-01-PBS-GROWTH.md` (opción b) y `T21B-R1-TAR-RACE-FIX.diff` (v1). Cero mutaciones de infraestructura; Echo operando; timers/R2 intactos.
