...
## Tareas
1. **Stack de replicación nativa** (`~/aranea/work/replicacion-pool0-pool2/`): payloads JSON exactos de `pool.snapshottask.create` + `replication.create` (spec §3: snapshottask recursivo `pool0`, naming `repl-%Y-%m-%d_%H-%M`, 14d, 04:45; replication PUSH LOCAL → `pool2/pool0-replica`, exclude `.ix-virt`/`.system`, 04:50, readonly SET, retention NONE, allow_from_scratch false) + plan de fixture **G-REP-0** (workspace efímero `pool2/fixrep` autodestruido; NO simular con `zpool file`: probar el transporte LOCAL real en dataset de prueba) — aplicación GATED. El script `repl-pool0.sh` (cron+zfs) queda **DESCARTADO**: la API nativa existe (evidencia en SPEC §3); la vigilancia de capacidad vive en la serie diaria de `zpool list` (tarea 3) y el freno duro en las alertas nativas del middleware (G-REP-5).
