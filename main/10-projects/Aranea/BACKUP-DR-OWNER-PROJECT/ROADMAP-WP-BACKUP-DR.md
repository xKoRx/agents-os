...
`ID` WP-B1 · `objetivo` cobertura imagen-level de Tier 0 · `ejecutor` Backup/DR · `dependencias` decisión D-piloto (28sep, retención final) + 018 (lista definitiva) + 019 (ventana inicial) + crecimiento datastore (D3) · `operaciones` jobs vzdump diarios 02:00 T0 (o split para no solapar con dumps), semanal T1/T2; exclusiones §6.3 diseño; retención según D; verify jobs PBS · `riesgo` I/O de full backups sobre hades (MT4/echo idle nocturno medido hoy); datastore debe crecer ANTES · `gate` OWNER (D + 018 + 019) · `validación` 7 días VERIFIED + restore drill de 1 VM T0 completa · `rollback` desactivar jobs (piloto/demostrado reversible) · `DoD` vzdump diario T0 VERIFIED+AUTOMATED.
...
|---|---|---|---|---|
| DR-1 | Pérdida de una VM T0 | restore PBS a nodo alternativo; datos app desde dumps | drill: restore 1 CT + 1 VM a scratch (ya demostrado CT 990/G1A) | borrar target scratch |
| DR-2 | Pérdida de un disco | pool0: rebuild mirror + scrub; Ceph: NO re-replicate kronos hoy (S1-S3 primero); local-lvm: restore PBS | drill: fallar 1 disco simulado no ejecutable sin riesgo → validar procedimiento documentado + healthy check | n/a (documental si no hay ventana) |
| DR-3 | Pérdida de un nodo PVE | migración/restore guests; kronos caído = escenario especial (PBS) | drill de mesa + restore de 1 guest local-lvm a otro nodo | borrar targets |
| DR-4 | Pérdida de Ceph | reconstrucción SOs desde templates + datos ya en pool0; artefactos desde G1B | drill: levantar 1 VM de prueba con SO en local-lvm + MinIO desde G1B (ya demostrado arranque aislado) | teardown scratch |
| DR-5 | Pérdida de TrueNAS/hades | reinstall SCALE + import pools + re-attach iSCSI/NFS | drill: en ventana, export/import de 1 dataset de prueba + checklist config export (WP-B2 prerrequisito) | revert attach |
| DR-6 | Pérdida total Aranea | bundle off-site + Secret Zero + inventory + runbook | drill parcial: descargar resto restic + descifrar en máquina limpia | borrar copias locales de prueba |

Prerrequisito transversal: WP-B2 (config exports) y A7/A8 (off-site) — sin ellos DR-5/DR-6 son procedimientos incompletos. Los drills DR-1/DR-4 pueden ejecutarse dès que A0/A1 estén activos.
