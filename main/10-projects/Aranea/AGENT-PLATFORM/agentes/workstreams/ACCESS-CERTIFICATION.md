...
| `aranea-hasura-prod-ro` | Hasura CE v2.38.0 PROD | admin server-side RO | `get_version`, `get_inconsistent_metadata` (consistente), `export_metadata` | Superficie = exactamente 4 tools client-side (HISTORICAL pre-H1: hoy 3 tools, `export_metadata` eliminado 2026-09-15); sin `run_sql`/reload/mutadores | **PARTIAL** — `get_schema` falló ×2 (Connection closed → Not connected → server desconectado); H1 RESOLVED 2026-09-15 |
