...
- SSH MCP operativo en una sola instancia/puerto con siete perfiles: read-only `sqx-zeus`, `sqx-hera`, `sqx-kronos`, `mt5-kronos` y `echo-runtime-prod` (staged 2026-09-15, pending owner key install en `192.168.31.71`); writable/operator `mt5-kronos-operator` y `docker-echo-dev-operator`.
