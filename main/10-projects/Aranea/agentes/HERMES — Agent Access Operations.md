...
- [[AGENT-PLATFORM - MCP Access Plane]] ya posee las diez capabilities canónicas certificadas y una arquitectura real que debe reutilizarse.
...
- **Nivel vigente (2026-09-16):** A0 **gate demostrado** (A0.1–A0.3 ejecutados y certificados; reconciliación del plane, management path `mcps-ops` probado sin depender de sus MCP, y autoridad Hermes clasificada). A1 **gate demostrado** con golden repair real: GAP-ECHO-010 (2026-09-16) diagnosticado con causa PROVEN, reparado vía management plane, recertificado server+consumer y convertido en patrón reusable (runbook hasura § Failure modes, runbook capability-plane, skill `aranea-mcps-expert`; mecánica en skill Hermes `mcp-access-plane-operations`). A2 tiene precedentes operacionales reales (staging del profile `echo-runtime-prod` 2026-09-15; normalización de configs consumer tri-client 2026-09-16) pero su gate formal no está declarado: falta un cambio scoped con revocación ejercida. A3–A5 sin workload cerrado: NO declarar autonomía completa.
- Hermes gestiona **configuración, capabilities y certificación del plano de acceso**; no desarrolla producto (Echo/Echo Forge), no despliega servicios de aplicación ni toca runtime de trading. El desarrollo de aplicaciones pertenece a sus propios proyectos.
...
- [x] A1.1 Seleccionar una capability existente con recovery verificable como primer golden repair #owner/agent #type/admin #area/aranea — GAP-ECHO-010 (hasura :3006/:3005), blocker P1 real 2026-09-16
- [x] A1.2 Ejecutar diagnóstico/recovery usando management plane independiente #owner/agent #type/admin #area/aranea — root cause PROVEN (mcp-proxy 6.7.16 hijo stdio compartido) + fix `fix-shared-child.mjs` vía `mcps-ops`
- [x] A1.3 Recertificar server-side y desde consumer real #owner/agent #type/admin #area/aranea — dev 50/50 + prod-ro 30/30 server; Daedalus consumer PASS; regresión 401 en 3001–3009 limpia
- [x] A1.4 Convertir sólo el patrón reusable demostrado en runbook/skill #owner/agent #type/admin #area/aranea — runbook hasura § Failure modes + caso capability-plane + skill `aranea-mcps-expert` (routing) + skill Hermes `mcp-access-plane-operations` (mecánica)
...
- **2026-09-15b — GAP-ECHO-004 CLOSED (workload A0.1, parte 2):** owner seed instalado en `.71` (el owner creó `echo-dev` como identidad dedicada — el bundle inicial la asumía preexistente; lección de preflight capturada en runbook y skill). Recertificación `echo-runtime-prod` desde Daedalus real PASS: identity `echo-dev@echo` sin sudo, 11 tools, Gateway/Core RUNNING (`echo-gateway` PID 713, `echo-core` PID 110701; `echo-functions` PID 320982; **Bridge NOT_DEPLOYED**), listeners 80/9080/9090/8080/8090, negative `run-command` → POLICY_DENIED MUST DENY (H2 observable post-seed), leak CLEAN. Observación PROD ahora dual: viewer SSH + `aranea-observability-ro`. Quirk nuevo registrado: el pool de 64 sesiones de ssh-mcp se agota con probes init-only por llamada — usar sesión única por consumer probe; recovery = `docker restart ssh-mcp` (sin cambio de config). Evidencia: [[ACCESS-CERTIFICATION]] § GAP-ECHO-004 CLOSED · change log `2026-09-15-gap-echo-004-closed`.
- **2026-09-16b — Reconciliación de capability requests Echo/Forge (extracción only, sin gates ni certificaciones):** cola vigente reconciliada contra [[ACCESS-CERTIFICATION]], matriz Echo y backlog. **FIRST BLOCKER = GAP-ECHO-010** (async-202 en proxies nginx-wrapped; evidencia 2026-09-16 ×3 proxies; workaround restart certificado; candidato natural A1.1 golden repair, authority AUTO vía `mcps-ops`). Necesidades documentadas sin solicitud runtime activa: etcd scoped (GAP-ECHO-005, gates E-06/E-11/E-12; E-06 desbloqueado pero no iniciado → NEEDS_SOURCE_PROOF para priorizar), viewer Windows MT5 read surface (F-04 2026-09-13; lane deferred clase C), Temporal visibility (T5 deferred; SSH logs + PG read models cubren la evidencia actual; sin MCP nuevo demostrado), MinIO artifacts (NEEDS_SOURCE_PROOF). Hasura DEV CLI residual (E-05 T19) cubierto por `apply_metadata` MCP salvo delta no expresable. E-02 handoff verificado en disco (branch @ `f7ddea18`, bitácora, change log). Detalle: change log `2026-09-16-access-ops-capability-reconciliation`.
- **2026-09-16d — Reconciliación documental canónica del MCP Access Plane (workload owner; cero cambios runtime, cero smokes salvo verificación read-only de configs en disco):** A0/A1 actualizados a estado real — A1 gate DEMOSTRADO con golden repair GAP-ECHO-010 (diagnóstico PROVEN → fix → recert server+consumer → patrón reusable en runbooks/skill; cambio log `2026-09-16-gap-echo-010-shared-stdio-child-repaired`); A2 queda con precedentes reales pero gate formal NO declarado (falta revocación ejercida); A3–A5 siguen abiertos (no declarar autonomía completa). Boundary precisado: Hermes gestiona configuración/capabilities/certificación del plano, no desarrollo de aplicaciones. Reconciliación completa de skill `aranea-mcps-expert`, runbooks (hasura, capability-plane; ssh/postgres/mongo/kafka/flink/observability verificados vigentes), proyecto del plane, Architecture y ACCESS-CERTIFICATION contra la evidencia fechada. Deudas preservadas: aislamiento de bearers Mongo/PG (owner action), ZCode bearer literals, Codex certificación funcional pending-IDE-side (config ya normalizada en disco 18:49 + smoke nativo 10/10 PASS reportado por owner; "PENDING patcher" de los logs de la mañana queda HISTORICAL), M3/M5, host key `.71`. Detalle: change log `2026-09-16-mcp-plane-documentation-canonicalization`.
- **2026-09-16c — Normalización tri-client MCP en Daedalus (ZCode/Codex/Cursor; validación estática; smokes funcionales = owner):** **Cursor READY** — refs Mongo RO/RW corregidas `ARANEA_POSTGRES_MCP_*_BEARER` → `ARANEA_MONGO_FORGE_MCP_*_BEARER` (error demostrado; las variables correctas existen en el chain B4; edición in-place preservando 660/owner/ACL `u:hermes-ops:rw-`; baseline `4a8222d5…` congelado + backup `mcp.json.bak-tri-20260916-151924` en `hermes-managed/`; sweep consumer 11/11 PASS con ciclo MCP completo por entrada + negativos 401 ×10); clon `aranea-postgres-ro-hermes-managed` RETAINED por decisión owner (es la entrada gestionada del operador B2 `mcp-onboard.py`). **ZCode READY con caveat** — docs oficiales no documentan interpolación `${env:}` en headers HTTP (sólo env para stdio) ⇒ config funcional 10/10 conservada con bearer literales, `600 kor` sin ACL (DEFERRED; alternativa = rotación de bearer + canal kor-only, decisión owner). **Codex OWNER_ACTION_REQUIRED** — mecanismo nativo `bearer_token_env_var` (docs OpenAI + strings en binario 0.153.4); normalizador determinista stageado en Daedalus `/tmp/codex-config-normalize.py` (sha16 `024c69bc62181a1a` verificado local↔Daedalus) para ejecución como kor por el owner: backup `.bak-tri-<ts>` 600, rollback byte-identical, round-trip TOML, idempotente, sin secretos (sólo nombres de variables). **Deuda nueva registrada en [[AGENT-PLATFORM - MCP Access Plane]]: bearers Mongo/PG comparten VALOR** (sha `5416e5d2…` RO-RO y `aeca359b…` RW-RW en proxy-secrets; separación actual es de nombre, no de material) — rotación coordinada de 4 proxies propuesta como owner action posterior, NO ejecutada (prohibida en este workload). Detalle: change log `2026-09-16-tri-client-consumer-certification`.
...
- **A-D08:** El ciclo completo de un blocker de acceso (staging → GATED owner seed → recertificación consumer → cierre de gap) cabe en AUTO salvo el seed del host: para `.71` la única mutación fue owner-side; Hermes ejecutó staging, bundle de seed (key leída de disco con fingerprint verificado), recertificación y documentación sin shell humana en el target. Preflight obligatorio del bundle: verificar que la identidad remota del bundle EXISTE en el target antes de asumirla.
