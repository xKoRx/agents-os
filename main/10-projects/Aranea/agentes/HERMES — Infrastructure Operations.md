...
| worker-kronos | 135 | qemu/kronos | MCP viewer+operator + `qm guest cmd` vía kronos + **SSH nativo W1 (post-mandato)** | echo-dev (std user) / ariadna@pve / **ariadna-win (admin, llave `from=.122`)** | whoami OK; viewer run DENIED; CIM DENIED con echo-dev; qm guest cmd LEÍDO; **W1: CIM + Get-ScheduledTask LEÍDOS por SSH como ariadna-win** | MCP no (SSH nativo) | **VERIFIED (native mgmt W1 — scope VM 135)** |
...
**Ejecución posterior del bundle — W1 (misma noche, mandato owner dedicado):** excepción de bootstrap acotada a VM 135 EJECUTADA y certificada (mutaciones reales registradas, reversibles; detalle en el change log `2026-09-18-w1-windows-native-bootstrap` y `~/aranea/work/w1-windows-native-20260918/`): transporte OpenSSH for Windows REUTILIZADO (ya presente del plano consumer; sin WinRM, sin segundo servicio), identidad `ariadna-win` creada (Administrators; password in-guest no conocida por agentes), llave ED25519 `~/.ssh/ariadna_win` instalada en `administrators_authorized_keys` con ACL canónica y `from="192.168.31.122"` (restricción de origen a nivel identidad — firewall/sshd_config/listener intactos por dependencia del plano consumer .161), host key pinneada desde fuente independiente (guest-exec), certificación positiva (lecturas CIM antes denegadas) y negativa (sin credencial/llave equivocada → denied), sin restart de sshd ni impacto sobre el plano consumer (misma conexión .161 viva pre/post), G4 hijo fresco 10/10 PASS `mcp_calls=0`. **Windows native management = CERTIFIED, scope worker-kronos VM 135 únicamente.** L1/L2 siguen pendientes de decisión owner propia.

**Recuperación independiente:** llaves nativas en `~/.ssh/` de hermes-vm (invulnerable al plano MCP) — incluye `ariadna_win` (worker-kronos, revoke = `Disable-LocalUser ariadna-win` o vaciar `administrators_authorized_keys`, break-glass = consola Proxmox VNC); `qm guest cmd` como segunda vía de inspección de guests; consola PVE del owner como break-glass universal; publisher Windows sigue siendo la fuente certificada de superficie worker-kronos (freshness ≤15 min).
...
## 🔐 Matriz de autoridad H4 (2026-09-18/19, provisioning — clasificación read-only, cero mutaciones)

Auditoría en vivo (22:17/01:17 UTC, 5/5 nodos): 59 guests exactos (39 qemu + 20 lxc), **3 templates QEMU (109 win-serv-22, 117 ubuntu-server, 120 k8s-node-2 — todos @hera sobre local-lvm)**, 0 templates LXC locales (vztmpl en nfs-storage/local: debian-12/13, ubuntu-24.04/24.10, alpine-3.21), ISOs Windows+Linux+virtio en nfs-storage. **Hallazgo clave: 0/39 VMs con drive cloud-init** (scan en vivo; los templates traen `ide2 none` → bootstrap in-guest post-clon). Token `ariadna@pve!backup-dr` re-verificado: sólo `VM.Audit`+`VM.Backup` en `/vms` — **sin VM.Allocate/VM.Clone/VM.Config.*/Datastore.AllocateSpace: NO es token de provisioning** (no se amplió). Ceph re-medido desde zeus/hera/kronos (athena sin conf local): **HEALTH_WARN persistente — 2 OSD nearfull + 2 pools nearfull, pool1 87.17% (834G/956.8G, 122.8G avail), 129 pgs active+clean** ⇒ NO_GO provisioning sobre pool1. NFS (TrueNAS pool0): 943G libres. VG pve: 16G free athena/hera, 14.75G hades, **0G zeus/kronos** (thinpool con holgura en los 4; hades thinpool 61.95% = cautela).

| Familia H4 | Operaciones clave | Canal | Autoridad efectiva | Estado habilitación |
|---|---|---|---|---|
| A — Verificación target/VMID/nodo/storage/red | qm/pct list, cluster/resources, pvesm status/list, vgs/lvs, ping/DNS sweep, Ceph health | API (`VM.Audit`) + SSH/sudo | lecturas EJERCIDAS esta noche | VERIFIED |
| B — Creación VM Linux | `qm clone 117/120 --full`, `qm create` + ISO (debian-12.4 / ubuntu-24.04.2 @nfs-storage) | SSH/sudo | NO ejercido (por diseño) | AUTHORIZED_NOT_EXERCISED |
| C — Creación LXC | `pct create` con vztmpl (debian-13-6 recomendado) + rootfs local-lvm | SSH/sudo | NO ejercido | AUTHORIZED_NOT_EXERCISED (ruta mínima habilitada) |
| D — Config post-create | `qm set`/`pct set` (cores/mem/net0/IP estática) | SSH/sudo | NO ejercido | AUTHORIZED_NOT_EXERCISED |
| E — Bootstrap OS/identidad | identidad admin dedicada por guest, llave + `from=.122`, host key pinneada, TZ America/Santiago | SSH/sudo + consola | patrón certificado en H1/W1; in-guest NO ejercido en guest nuevo | AUTHORIZED_NOT_EXERCISED (patrón VERIFIED) |
| F — Templates/imágenes nuevos | `pveam download`, wget ISO, `qm template` | SSH/sudo | NO ejercido; `qm template` vetado sobre guests existentes | AUTHORIZED_NOT_EXERCISED (spec-only) |
| G — Teardown | `qm destroy`/`pct destroy` | SSH/sudo | NO ejercido | OUT_OF_SCOPE salvo recurso propio del ejecutor en su tarea |
| H — Windows (creación) | OVMF+virtio+unattend desde ISO/template 109 | — | W1 certificó administración de UNA VM existente (135), NO creación | NOT_PROVEN — provisioning Windows NO habilitado |
...
**Resultado (2026-09-18):** `H3 ENABLEMENT PASS — VERIFIED SCOPE` — Linux/LXC/Docker(mcps)/systemd con canales nativos demostrados y contratos publicados ([[linux-container-operator-contract]], [[service-lifecycle-operator-contract]]); canal host-mediated `qm guest cmd` verificado vivo (incluye Windows 135); Windows cerrado la misma noche por el **mandato W1** (excepción de bootstrap acotada: SSH nativo + identidad `ariadna-win` sobre worker-kronos VM 135, mutaciones registradas y reversibles, G4 fresco PASS `mcp_calls=0`) — **el alcance Windows certificado queda explícitamente limitado a worker-kronos VM 135** (no implica flota Windows habilitada; WinRM no provisionado). G4 sesión fresca H3: 5/5 PASS, `mcp_calls=0`. `GUEST & SERVICE OPERATIONAL CERTIFICATION` queda explícitamente FUERA de este hito: la demuestra el proyecto ejecutor operando. Detalle W1: change log `2026-09-18-w1-windows-native-bootstrap` + [[windows-operator-contract]].
...
**Resultado (2026-09-18/19):** `H4 ENABLEMENT PASS — VERIFIED SCOPE` — matriz de autoridad H4 (familias A–H arriba) clasificada sobre auditoría read-only en vivo (token API NO-provisioning re-verificado; ruta SSH/sudo = AUTHORIZED_NOT_EXERCISED por diseño); inventario real de templates/imágenes (3 templates QEMU @hera, 0 cloud-init en flota, ISO/vztmpl en nfs-storage); criterio de capacidad GO/NO_GO por storage (Ceph pool1 NO_GO nearfull); contratos de networking/identidad/bootstrap/onboarding/teardown integrados en [[provisioning-operator-contract]]; golden G8 en sesión fresca PASS (2 specs consistentes + 6 negativos clasificados). `PROVISIONING OPERATIONAL CERTIFICATION` y `GO-LIVE` de servicios quedan explícitamente FUERA de este hito: los demuestra el proyecto ejecutor operando. Detalle y evidencia: change log `2026-09-18-h4-provisioning-enablement` + `~/aranea/work/h4-provisioning-20260918/`.
...
- **H5 HIGH-IMPACT ENABLEMENT PASS — VERIFIED SCOPE — 2026-09-19 (mandato owner one-shot; ZERO INFRASTRUCTURE MUTATIONS):** habilitación de alto impacto ejecutada íntegramente read-only: 10 familias A–J inventariadas con identidad estable (índice [[30-resources/aranea/06-high-impact/00-index]]), auditoría en vivo 15:42–15:50 UTC (PVE ×5 quorum 5/5, Ceph 3/3 MONs, TrueNAS FULL, PBS fail-closed, red/DNS), matriz de autoridad H5 con dimensiones separadas (ACCESS/AUTHORITY/RECOVERY doc/RECOVERY demostrado), blast radius CLASS 1–3 (CLASS 3 owner-gated), contratos [[high-impact-networking-dns-contract]] + [[cluster-node-maintenance-contract]] + [[ceph-storage-operations-contract]] (REUSE: TrueNAS=H1, PBS=R2, Hermes=existente), golden G9 sesión fresca PASS (2 escenarios + 6 negativos, cero mutaciones, mcp_calls=0). Gaps honestos: OPNsense sin recovery (SPOF), step-ca stopped sin backup de CA, UPS inexistente, recovery Ceph/OPNsense/step-ca NOT_CERTIFIED. NO se ejecutó ninguna operación high-impact; **ningún contrato H5 autoriza operaciones**. Detalle: `80-agents/journal/logs/2026-09-19-h5-high-impact-enablement.md` + `~/aranea/work/h5-high-impact-20260919/`.
- **H4 PROVISIONING ENABLEMENT PASS — VERIFIED SCOPE — 2026-09-18/19 (mandato owner one-shot; ZERO MUTACIONES):** habilitación de provisioning ejecutada íntegramente read-only: primer gate de publicación OK (9 archivos H1/H2/H3/W1 `PUBLISHED_IDENTICAL` en origin/master, sync externo vivo 22:04, sin push directo); auditoría viva 5/5 nodos (permisos user/token re-contrastados — token `backup-dr` sin VM.Allocate/Clone; 3 templates QEMU 109/117/120 @hera; **0/39 VMs con cloud-init**; ISO/vztmpl inventariados; capacidad df/vgs/lvs por nodo; Ceph HEALTH_WARN nearfull re-medido desde 3 mons — athena sin conf local de Ceph, hallazgo operativo nuevo; red vmbr0 estándar + vmbr1@athena + bridge ceph); criterio de capacidad con veredicto por storage (pool1 87.17% = **NO_GO**; nfs-storage 943G = GO; local-lvm por nodo); contrato del futuro operador creado ([[provisioning-operator-contract]], índice+log actualizados, 26 curados); golden G8 sesión fresca (hijo aislado): 2 specs de ejecución (VM Linux, LXC) resueltas contra estado vivo + 6 negativos GO/NO_GO clasificados sin preguntar al owner; fila de captura en fechas-captura; change log H4. **Windows provisioning = NOT_PROVEN** (W1 ≠ creación). Detalle: `~/aranea/work/h4-provisioning-20260918/` + `80-agents/journal/logs/2026-09-18-h4-provisioning-enablement.md`.
- **W1 WINDOWS NATIVE BOOTSTRAP PASS — 2026-09-18 noche (mandato owner; única excepción de bootstrap autorizada, acotada a VM 135):** canal de administración nativa Windows operando sobre worker-kronos: transporte OpenSSH for Windows REUTILIZADO (ya presente del plano consumer; sin WinRM ni servicios nuevos), identidad dedicada `ariadna-win` (Administrators; password aleatoria generada in-guest, no conocida por agente alguno), llave ED25519 `~/.ssh/ariadna_win` con ACL canónica y restricción de origen `from="192.168.31.122"` en `administrators_authorized_keys`, host key pinneada desde fuente independiente (guest-exec), certificación positiva (CIM `Win32_Service` + `Get-ScheduledTask`, lecturas antes denegadas con echo-dev), negativa (sin credencial/llave equivocada → denied), logs `sshd.log` con acceptance, sin restart de sshd (PID 1056/StartTime intactos) ni impacto en el plano consumer (misma conexión .161 viva pre/post), G4 hijo fresco 10/10 PASS `mcp_calls=0`. Mutaciones W1: usuario + authorized_keys/ACL + known_hosts/config local + fixture borrado — todas revertibles (RB-01/02/03, rollback que NO depende del canal nuevo). Alcance certificado: **worker-kronos VM 135 únicamente**. Evidencia: `~/aranea/work/w1-windows-native-20260918/` + change log `2026-09-18-w1-windows-native-bootstrap`. Drift corregido antes de mutar: IP viva = 192.168.31.128 (= DNS; el .133 del bundle era supuesto). Este carril vuelve a boundary enablement-only.
- **H3 PARTIAL — LINUX ENABLED / WINDOWS BLOCKED — 2026-09-18 (mandato owner; cero mutaciones) — SUPERSDED POR W1:** habilitación guests/servicios ejecutada read-only: canales nativos revalidados 4/4 (+self), inventario revalidado exacto (59=42+17), canal host-mediated `qm guest cmd` verificado vivo en 5 VMs (incluida Windows 135), Docker mcps leído por nativo (26 conts vía sudo), Windows: plano consumidor MCP OK + `echo-dev` usuario estándar (CIM denegado) ⇒ **WINDOWS NATIVE MANAGEMENT = NOT CERTIFIED**; matriz H3 (arriba) + superficies A–H (`h3_surfaces.csv`); 3 contratos del ejecutor publicados; G4 sesión fresca 5/5 PASS (`mcp_calls=0`, `mutations=0`); owner bundle único (W1 Windows admin, L1 daedalus docker opcional, L2 qemu-guest-agent opcional). Detalle y evidencia: `80-agents/journal/logs/2026-09-18-h3-guest-service-enablement.md` + `~/aranea/work/h3-enablement-20260918/`.
...
- [x] I3.2 H3: rutas nativas Linux/Windows management certificadas + contratos de escalation #owner/agent #type/admin #area/aranea — DONE 2026-09-18 con veredicto inicial **H3 PARTIAL — LINUX ENABLED / WINDOWS BLOCKED** (cero mutaciones; change log `2026-09-18-h3-guest-service-enablement`) y COMPLETADO esa noche por el mandato W1 → **H3 ENABLEMENT PASS — VERIFIED SCOPE** (Windows: SSH nativo ariadna-win en VM 135, scope limitado; change log `2026-09-18-w1-windows-native-bootstrap`)
- [x] I3.3 H4: identidad/scope de provisioning clasificado + flujo de onboarding backup/observabilidad documentado #owner/agent #type/admin #area/aranea — DONE 2026-09-18/19: matriz H4 A–H (token API NO-provisioning verificado; SSH/sudo AUTHORIZED_NOT_EXERCISED; Windows NOT_PROVEN), contrato [[provisioning-operator-contract]], capacidad por storage con Ceph pool1 NO_GO, golden G8 sesión fresca (2 specs + 6 negativos), veredicto **H4 ENABLEMENT PASS — VERIFIED SCOPE** — change log `2026-09-18-h4-provisioning-enablement`
- [x] I3.4 H5: mantener high-impact gated hasta aprobación explícita del owner; sólo clasificación de autoridad por familia #owner/agent #type/admin #area/aranea — DONE 2026-09-19: veredicto **H5 ENABLEMENT PASS — VERIFIED SCOPE** (mandato owner one-shot): 10 familias A–J clasificadas read-only, CLASS 3 owner-gated sin excepción, Ceph NO_GO vigente re-medido 3/3 MONs, 3 contratos enablement-only creados, golden G9 PASS con 6 negativos; cero mutaciones — change log `2026-09-19-h5-high-impact-enablement`
...
- **2026-09-19 — H5 HIGH-IMPACT ENABLEMENT ejecutado y certificado (mandato owner one-shot; ZERO INFRASTRUCTURE MUTATIONS) → `H5 ENABLEMENT PASS — VERIFIED SCOPE`:** inventario G1 de 10 familias (A networking/firewall, B DNS/DHCP, C cluster/quorum, D Ceph, E TrueNAS, F PBS, G PKI, H energía/UPS, I control plane Ariadna, J host transversal) con identidad estable y fuente+timestamp (índice [[30-resources/aranea/06-high-impact/00-index]]). Auditoría read-only en vivo 15:42–15:50 UTC: PVE ×5 (quorum 5/5, pvecm, corosync LAN-only, HA vacío, storage.cfg 12 defs con `aranea-pbs` nuevo post-R2, kernel drift athena -29 vs -18, interfaces/bridges, firewall PVE disabled/nft 0); Ceph desde 3/3 MONs zeus/hera/kronos (HEALTH_WARN persistente: osd.0 85.65%/osd.2 85.59% nearfull, pool1 87.26% ≈ sin drift vs H4, .mgr nearfull, fragmentación 0.90, 189 PGs active+clean, sin CephFS, athena sin conf local); TrueNAS FULL revalidado (25.04.1, pool0/pool2 ONLINE, **pool0 scrub 6-sep 0 errores** — riesgo #3 cerrado para pool0; **pool2 sin scrub desde 2025-07-12** — riesgo vigente; 4 NFS+3 SMB+1 iSCSI; ups STOPPED; alerta update disponible); PBS (4.2.5-1, reboot R2 verificado: boot 22:24:05 18-sep, drop-ins fail-closed con `Requires=mnt-pbs\x2ddata.mount` EN VIVO en ambos servicios, datastore main 731M, snapshot única ct/155); athena (OPNsense 130 running, Pi-hole FTL vivo, Tailscale 119 vivo, CA 200 stopped, tailscale-gateway 100.75.167.57); DNS hermes-vm (.31 → upstream .1; `mt5-kronos.lab.aranea.cl`/`dashboard.lab.aranea`/`ca.lab.aranea` resuelven). Matriz H5 con dimensiones separadas (ACCESS CERTIFIED / AUTHORITY CLASSIFIED / RECOVERY DOCUMENTED / RECOVERY DEMONSTRATED / OPERATIONS NOT EXECUTED). Blast radius CLASS 1–3 por familia; CLASS 3 owner-gated sin excepción. Contratos G8: CREATE [[high-impact-networking-dns-contract]] + [[cluster-node-maintenance-contract]] + [[ceph-storage-operations-contract]]; REUSE TrueNAS (matriz H1), PBS (estado R2 de [[BACKUP-DR-OWNER-PROJECT]]), Hermes (recovery existente). Golden G9 sesión fresca (hijo aislado con sólo índice H5 + contratos + referencias): Escenario A (mantenimiento de nodo Proxmox) — hijo descartó athena por SPOF, eligió zeus, verificó quorum/workloads/storage en vivo (3 sesiones SSH read-only) y clasificó **NO_GO condicionado** (osd.2 nearfull en zeus + SQX 108 sagrado sin plan de protección); Escenario B (nearfull de Ceph) **HANDOFF a [[ceph-storage-operations-contract]]** con plan por riesgo ascendente y gates; negativos 6/6 (quorum NO_GO, storage protegido NO_GO, recovery NOT_CERTIFIED, target AMBIGUOUS, CLASS 3 GATED, MCP caído → canal nativo sin provocar falla); `operaciones_mutantes=0`, `sesiones_ssh=3`, `mcp_calls=0`, cero secretos. Owner bundle: NO requerido para habilitación (gaps H = decisión de diseño/capex, no permisos; consola owner = break-glass existente). Índice+log de runbooks actualizados (29 curados), fila H5 en fechas-captura, change log `2026-09-19-h5-high-impact-enablement`. Evidencia: `~/aranea/work/h5-high-impact-20260919/probes/`.
- **2026-09-18/19 — H4 PROVISIONING ENABLEMENT ejecutado y certificado (mandato owner one-shot; ZERO MUTACIONES) → `H4 ENABLEMENT PASS — VERIFIED SCOPE`:** primer gate de publicación: vault↔origin/master reconciliados — 9 archivos H1/H2/H3/W1 byte-idénticos (`PUBLISHED_IDENTICAL`, último sync 22:04, pipeline externo vivo; repo local confirmado consumidor; sin push directo). Auditoría read-only en vivo 5/5 nodos (probes 22:17 y 01:17 UTC en `~/aranea/work/h4-provisioning-20260918/probes/`): permisos user/token re-contrastados (idénticos: `VM.Audit`+`VM.Backup` `/vms` — **sin permisos de provisioning; no se amplió nada**); **templates: 3 QEMU (109 win-serv-22, 117 ubuntu-server, 120 k8s-node-2, todos @hera/local-lvm) y 0 LXC locales**; vztmpl debian-12/13 + ubuntu-24.04/24.10 + alpine-3.21 e ISOs Linux/Windows/virtio en nfs-storage; **hallazgo: 0/39 VMs con drive cloud-init** (templates con `ide2 none` ⇒ bootstrap in-guest post-clon); capacidad medida por nodo (thinpool/VG/root df); **Ceph: HEALTH_WARN persistente (2 OSD + 2 pools nearfull; pool1 87.17%, 122.8G avail; 129 pgs active+clean) re-medido desde zeus/hera/kronos — athena no puede consultar Ceph (sin conf local): hallazgo operativo nuevo registrado**; red: vmbr0 estándar en 4 nodos, athena vmbr0/1/2 con vmbr1 vlan-aware; LXCs: 20 leídos (14 unprivileged, IP estática vmbr0; 5 con rootfs en pool1 = NO_GO actual). Contrato creado: [[provisioning-operator-contract]] (cadena de resolución, rutas R1 clon/R2 ISO/R3 pct create, criterio capacidad GO/NO_GO por storage, recursos protegidos, bootstrap/identidad G5, onboarding G6 con gate backup-BLOCKED, teardown, concurrencia/timeouts); índice+log de runbooks actualizados (26 curados); fila de captura en fechas-captura. **Golden G8 PASS (sesión fresca, hijo aislado con sólo inventario+matriz H2+contrato; los 2 primeros intentos murieron por rate-limit del provider sin ejecutar nada; el 3º — protocolo de ahorro — completó el reporte antes de un 429 final y fue adoptado por el integrador):** **Caso 1 VM Linux `metrics-dev`:** VMID 150 verificado libre EN VIVO (cluster/resources; nota: 151 SÍ existe = win-development @hades), nodo hera, local-lvm re-medido (27.98%, ~243G avail; criterio 32G+20% ⇒ GO), ruta R1 `qm clone 117 150 --full` + `qm set`, IP .150 probada (ping RC=1 + getent RC=2; libre≠disponible: el ejecutor re-verifica al ejecutar y registra), identidad G5, gates G6 A–H, validación y rollback. **Caso 2 LXC `redis-dev`:** VMID 161 verificado libre (160/162 existen), `pct create 161 nfs-storage:vztmpl/debian-13-standard_13.6-1_amd64.tar.zst --rootfs local-lvm:8 --unprivileged 1 ...`, IP .153 probada viva (evita .152 postgresql), identidad post-create vía `pct exec`, gates completos. **Negativos 6/6:** N1 `local-sqx-hera` REJECTED (protegido; 69.78% vivo); N2 200G en pool1 NO_GO (ceph df 87.23%, 116G avail vs ~359G requeridos con criterio); N3 afirmación "150/.150 libres" → GO sólo tras verificación propia viva (la afirmación no es probatoria); N4 vía token API POLICY_DENIED (sin VM.Allocate/Clone); N5 backup obligatorio → GO-LIVE BLOCKED (0 jobs PVE + PBS sin datastores); N6 "la VM de la base de datos" AMBIGUOUS (desambigua el proyecto ejecutor, no el owner). Cero preguntas al owner; cero mutaciones (3 sesiones SSH read-only, exit 0). Transcript: cache de delegación `deleg_cd323ac7` de la sesión. Owner action bundle: **NO REQUERIDO** (la autoridad existente alcanza para el objetivo enablement; sin permisos/tokens nuevos pedidos). Separación preservada: `PROVISIONING ADMINISTRATIVE ENABLEMENT != PROVISIONING OPERATIONAL CERTIFICATION != GO-LIVE`.
- **2026-09-18 — W1 WINDOWS NATIVE BOOTSTRAP ejecutado y certificado (mandato owner; excepción de bootstrap acotada a VM 135) → `H3 ENABLEMENT PASS — VERIFIED SCOPE`:** target proof doble en vivo (VM 135 @kronos RUNNING; IP viva .128 = DNS, drift .133 corregido antes de mutar; origen .122 verificado) + discovery read-only vía `qm guest exec` (OpenSSH for Windows 9.5 YA RUNNING del plano consumer con authorized_keys de echo-dev; ariadna-win inexistente; administrators_authorized_keys inexistente; regla firewall preexistente TCP22; sshd_config con Match Group administrators). **REUSE** del transporte: OpenSSH for Windows (alternativa WinRM descartada: segundo servicio sin necesidad). **Mutaciones W1 exactas (todas en VM 135, revertibles):** (1) usuario local `ariadna-win` + Administrators, password aleatoria generada y descartada DENTRO del guest (autenticación W1 = 100% llave); (2) `administrators_authorized_keys` creado con ACL canónica (`inheritance:r; SYSTEM:F; Administrators:F`) y línea única `from="192.168.31.122"` + llave nueva `~/.ssh/ariadna_win` ED25519 (fingerprint `SHA256:EBKQ…`); (3) known_hosts de hermes-vm pinneado con host key obtenida por fuente independiente (ssh-keygen inside-guest `SHA256:OKjl…`); (4) alias `worker-kronos` en ssh_config; (5) fixture `C:\Windows\Temp\w1.ps1` borrado tras uso. Instalador idempotente sha256 `26bf13f6…` stageado byte-exacto (base64 chunked + certutil-verify, vía sesión kronos existente). NO tocado: echo-dev, KoR, Administrator, Guest, sshd_config (mtime/size idénticos), firewall (regla preexistente intacta — dependencia del plano consumer), servicios, tareas, MT5; sshd SIN restart (PID 1056/StartTime 9/10 intactos). **Certificación:** positivos (whoami/groups token admin S-1-5-114+S-1-5-32-544; CIM Win32_Service sshd; Get-ScheduledTask AraneaEvidencePublish — lecturas 0x80041003 con echo-dev ahora PASS), negativos (sin credencial → denied; llave equivocada → denied; origen ≠.122 bloqueado por from= nativo), logs `sshd.log` (`Accepted publickey for ariadna-win from 192.168.31.122`), no-impacto (misma conexión consumer .161:42758 viva pre/post; qm guest exec vivo). **Incidente de proceso corregido:** marcador de idempotencia dijo "already present" con archivo vacío (bug `Add-Content -Raw`); el ground truth in-guest mandó sobre el log y la llave se instaló con AppendAllText verificado — lección: verificar contenido del archivo tras cada escritura en staging multi-hop. **Golden G4 (hijo fresco aislado, sin MCP, sin secretos en prompt): 10/10 PASS, `mcp_calls=0`** — nombre lógico → VM 135@kronos → .128 → identidad → host key → SSH → CIM read → scheduled task → revoke/recovery (primer intento de hijo murió por hang del provider sin ejecutar nada; registrado). Contrato actualizado: [[windows-operator-contract]]. Evidencia: `~/aranea/work/w1-windows-native-20260918/` (README + evidence_certification + g4/ + discovery/). Veredictos: **W1 PASS; H3 ENABLEMENT PASS — VERIFIED SCOPE** (Windows scope = VM 135). Después de W1, el carril vuelve a enablement-only.
- **2026-09-18 — H3 PARTIAL — LINUX ENABLED / WINDOWS BLOCKED (mandato owner; ZERO MUTACIONES):** habilitación guests/servicios ejecutada íntegramente read-only (sondeos 23:14–23:31 UTC, canales nativos, sin instalar/activar nada). **G1 Linux:** sudo NOPASSWD revalidado truenas/pbs/mcps (daedalus sin sudo, consistente H0); inventario revalidado exacto 59=39+20=42 running+17 stopped; hallazgo: canal nativo host-mediated `qm guest cmd` verificado vivo en 111/118/123/135/138 (get-osinfo: sqx-hera Ubuntu 24.04.3, worker-kronos Windows 10 IoT LTSC 2024) — disponible sólo donde `agent: 1` (vm128/155/154 verificados sin agent); Docker mcps leído por nativo vía `sudo docker` (26 conts; `hermes-ops` NO en grupo docker — hallazgo registrado). **G2 Windows:** plano consumidor MCP verificado (whoami viewer+operator, viewer run-command POLICY_DENIED correcto) y `worker-kronos\echo-dev` demostrado usuario estándar (`Get-CimInstance` 0x80041003, `Get-ScheduledTask` CIM denegado; schtasks RPC no ejercido por guard ⇒ UNKNOWN, no PASS) ⇒ **WINDOWS NATIVE MANAGEMENT = NOT CERTIFIED**; SSH/WinRM/admin delegado ACCESS_NOT_PROVISIONED (no instalados; spec única W1 en owner bundle). **G3 superficies:** matriz A–H (`h3_surfaces.csv`) — Docker read VERIFIED mcps, systemd system/user VERIFIED (read), Compose/Task Scheduler NOT_EXERCISED/NOT CERTIFIED, observabilidad heredada vigente. **G4 sesión fresca** (hijo aislado, artefactos verificados por el integrador): 5/5 PASS — sqx-hera 123@hera, mcps 113@hades, hermes-gateway-ariadna.service (MainPID 915), host Docker mcps (docker ps -a estados), worker-kronos 135@kronos — con `mcp_calls=0`, `mutations=0`, resolución por inventario vivo y management independence demostrada (Linux y Docker 100% nativos). **Contratos creados:** [[linux-container-operator-contract]], [[windows-operator-contract]], [[service-lifecycle-operator-contract]] (índice 25 curados). **Owner bundle único** (W1/L1/L2) en `~/aranea/work/h3-enablement-20260918/owner_bundle_draft.md`; anomalía sqx-hera unused0/unused1 reconfirmada intacta. Cambios: 3 runbooks nuevos, índice+log runbooks, fechas-captura, matriz H3+gate+I3.2+Estado+Bitácora en esta nota, change log `2026-09-18-h3-guest-service-enablement`. `GUEST & SERVICE OPERATIONAL CERTIFICATION` queda para los proyectos ejecutores. Windows pasará a PASS cuando el owner ejecute W1.
- **2026-09-18 — H2 ENABLEMENT ejecutado y certificado (mandato owner; ZERO MUTACIONES):** auditoría read-only en vivo 5/5 nodos (PVE 8.4.20, quorum 5/5, 59 guests verificados 39+20 exactos, 10 storages, HA vacío, sudoers/`from=` verificados); permisos efectivos contrastados usuario↔token (idénticos: `VM.Audit`+`VM.Backup` en `/vms`); API sondeada con 19 GET de read-intent — clasificación 200 reales vs 403 reales vs 200-filtrado-vacío (`/storage`, `/pools`, `/cluster/tasks`: hallazgo nuevo — filtrado por permiso, NO vacío real); Ceph HEALTH_WARN (osd.0/osd.2 nearfull; pool1); 0 snapshots en guests protegidos verificados; discos SQX con `backup=0` leídos en config. Matriz H2 de 34 operaciones A–G (9 AUTHORIZED_AND_VERIFIED / 24 AUTHORIZED_NOT_EXERCISED / 1 OUT_OF_SCOPE = destroy owner-gated) en `~/aranea/work/h2-enablement-20260918/h2_authority_matrix.csv`; contrato del futuro operador creado en `30-resources/runbooks/proxmox-lifecycle-operator-contract.md` (índice+log actualizados, 22 curados); fila de captura añadida a fechas-captura; G4 sesión fresca PASS (hijo aislado: sqx-hera=VM123@hera running verificado API+SSH, clasificación por canal, independence del plano MCP — que al inicio del mandato respondía 401 auth, ya sin el 503 de H1, sin intervención de este carril). Separación API-TOKEN vs SSH-ROOT-EQUIVALENT explícita en toda la matriz. Sin owner action bundle: la autoridad existente alcanza para el objetivo enablement. Cambios: nota del proyecto (matriz H2, gate H2, I3.1, Estado, Bitácora), runbook nuevo, índice/log de runbooks, fechas-captura, change log `2026-09-18-h2-proxmox-enablement`. Veredicto: **H2 ENABLEMENT PASS** — `PROXMOX LIFECYCLE OPERATIONAL CERTIFICATION` queda para el proyecto ejecutor.
