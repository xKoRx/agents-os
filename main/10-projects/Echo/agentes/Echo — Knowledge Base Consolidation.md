...
- **FASE G EN CURSO** — documentation-verifier (MAX), ciclo 1/2. Fase F COMPLETE: propuesta wiki en `artifacts/06-wiki-draft-plan.md` — set: `applications/echo/00-index.md` (CREATE), `echo/echo-core.md` + `echo/echo-forge.md` (UPDATE-MOVE reescritas), `echo/echo-forge-integration-boundary.md` (CREATE compartida), changelog + 6 contratos + 2 Fuentes (MOVE intactos), 2 notas source nuevas, 5 auditorías → supersede SÓLO tras PASS. Contradicción "Forge entrega vía API" eliminada de drafts. Vault HEAD real `12d250a` (drift post-baseline, reconcilia H). Publicación en un solo cambio con `materialize_schema_note.py`.
