...
- **FASE D EN CURSO** — echo-forge-integration-cartographer lanzado. Fase C COMPLETE: cartografía Forge @ `9fad768c` en `artifacts/03-forge-cartography.md`. Verdad implementada del handoff: **NO existe transporte real Forge→Echo**; F-04 (sólo en `feature/f04-magic-version-handoff`, no en origin/master) es librería sin cablear (`BuildHandoffManifest`, `DeliverHandoff` write-once con UNKNOWN_RECEIPT sin re-POST, tablas PG migración 015), única ingress = `FakeConsumerIngress` in-process (sólo tests); cero callers de producción (pipeline termina en FinalistPromotion V2 + Apply). Magic V1 sí cableado (`AllocateMagicV1` gated por `UseDurableMagicAllocation`); `SealStrategyVersion` implementado sin caller. Pipeline = `GenericSQXWorkflow` secuencial + `ForgeCampaignWorkflow` multi-wave sobre Temporal SDK v1.44.1; MT5 real vía tester.ini + reporte parseado + reconcile + score shadow. Persistencia: PG (autoridad, migraciones 001–016), Mongo (evidencia), MinIO (`sqx-strategies`), etcd. Unknown: dónde se cableará Seal→Manifest→Deliver (sólo inferible).
- Fase B COMPLETE: orquestación Echo = **Flink StateFun, NO Temporal**; E-02 implementado en branch (PHYSICAL_PARTIAL); E-01/E-04 receptor completo con replay idempotente/409 + 30+ tests CONTRACT, Spec-Active, sin emisor en Symphony.
- Fase A COMPLETE: topología = subdominio `30-resources/applications/echo/` con `00-index.md` propio; páginas `echo-core.md`, `echo-forge.md`, frontera `Echo — Forge Integration Boundary` (compartida), contratos F-01…F-04 + V1 intactos como source-of-record; 5 auditorías datadas → superseded tras verification PASS. Detalle: `artifacts/01-knowledge-architecture.md`.
