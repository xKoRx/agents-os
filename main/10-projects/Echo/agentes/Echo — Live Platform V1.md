...
- **E-06 PLANNING V1.2.0 READY FOR MANAGER REVIEW (2026-09-16):** [[Echo — E-06 Reference Enrollment and Binding]] SPEC v1.2.0. **Manager resolvió CASE C → CASE B**: zero-order exige `effective_magic` físicamente atestiguado por la instancia real del strategy EA (excepción acotada C-3 §7.2a, sólo identidad runtime) e igual al pin; ausente/mismatch ⇒ fail-closed sin OBSERVING. Discovery físico: producer del strategy EA = exporter Forge (`EchoForgeMT5Exporter.java`, repo `xKoRx/symphony`; `MagicNumber` = `input` externo estampado desde allocation V1 y verificado por readback/seal F-04); atestación vía variables globales del terminal, relay verbatim del Echo collector. Inyectar atestación cambia bytes ⇒ **nuevo** `strategy_version_ref` (canonical_strategy_id estable); Versions históricos intactos fail-closed. AutoTrading B2 intacto; hook SQX de coverage sigue DEFER. Docs-only; 0 product source Echo; 0 bytes Forge mutados. NORMAL no lanzado. E-01…E-05 no reabiertos.
