...
- **SHARED DEV ROLLOUT BLOCKED — PREDECESSOR_MIGRATION_MISSING (2026-09-16).** Rollout operacional 063+Hasura a SHARED DEV detenido fail-closed en el gate de migraciones (CASE C): `echo-develop` PG 17.6 @ `192.168.31.220` (vía `mcp_echo_dev_rw`, sólo SELECT) tiene **062/E-02 aplicada y verificada** (`journal_quarantine`: 12 columnas exactas, PK, `status_check`, índices `status_received_idx` y `pending_fact_reason_uidx`, 0 rows) pero **061/E-03 NO aplicada** — ausencia global en todos los schemas de `strategy_identity_mappings`, `strategy_versions`, `promotion_records`, `strategy_identity_aliases`, `fn_semantic_text_valid`, `fn_identity_bwc_write_once` y `mig061_backup_*`; **063 no existe y sin drift** (cero objetos `canonical_scopes`/`canonical_trade_sets`/`canonical_metric_sets`/`v_canonical_*`). No se aplicó 063, no se aplicó metadata Hasura (`canonical_analytics.yaml` sha256 `690bb149…`, no aplicado), Hasura DEV v2.38.0 @ `192.168.31.75` metadata consistente pre-existente sin cambios; no hay tabla registry de migrations en el cluster (autoridad = introspección física). Estado E-05 intacto: CLOSED — SOFTWARE / INTEGRATED. Cero side effects; PROD untouched; `lab-canonical-a0` no lanzado; master `5dd998f1` verificado y checkout principal restaurado a `feature/e02…`. Artefacto 063 listo: `063_analytics_convergence_a0.up.sql` sha256 `77dfec65…` @ master `5dd998f1`. Siguiente gate: **MANAGER DECISION** — autorizar rollout 061 de E-03 a SHARED DEV primero, o enmendar explícitamente el gate (063 es FK-independiente de 061 por diseño) y re-ejecutar este rollout.
