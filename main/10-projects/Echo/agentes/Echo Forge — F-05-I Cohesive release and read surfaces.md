...
- **Corrección T1 (2026-09-16, commit `2c5d34f`):** defecto confirmado por manager en `releasematrix.Parse`: la verificación de trailing data vía `dec.More()` no garantiza el fin del documento JSON (More hace peek del siguiente byte y devuelve false ante `}` o `]` pendientes), por lo que una matriz válida seguida de `}` o `]` era aceptada, violando el contrato de un único objeto JSON con sólo whitespace posterior. Corregido con la técnica correcta: segundo `Decode` en variable descartable que debe terminar exactamente en `io.EOF` (nil u otro error rechazan el documento); `DisallowUnknownFields`, `Validate`, API pública, modelos y marshaling canónico intactos. Test de regresión `TestParseTrailingDataAfterValidMatrix` (inputs desde `Embedded()`): demostrado que FALLA contra `dec.More()` con `}` y `]`; PASS con la corrección en los 5 casos contractuales (`}`/`]`/segundo objeto/texto inválido rechazados; sólo whitespace aceptado). T1 permanece en Review; manager review pending.
