...
- Sobra: misma tabla topográfica con 6 enlaces `file:///Users/rjara/...` (violaciones de máquina), y un bloque "Inventario de Workers" con 8 líneas de infraestructura operativa (FQDNs, IPs, usuario `kor` y CONTRASEÑA en claro `cascada123`) — secreto hardcodeado y conocimiento operativo que pertenece a `.agents/skills/worker-ssh/SKILL.md`, jamás a un router always-on. Este hallazgo de seguridad debe reportarse al parent.
