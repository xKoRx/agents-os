...
## Post-certification implementation erratum — V3-006

- La certificación histórica E-01 `CONTRACT_PASS` / `VERIFIED / CLOSED` del `2026-09-10` se preserva sin reescritura; esta nota registra un erratum de implementación posterior, no una nueva semántica ni una reapertura global de E-01.
...
- **Independent Verifier PASS (2026-09-13):** `S0_ERRATUM_VERIFICATION_PASS` / `READY_FOR_CONTROLLED_INTEGRATION` contra evidence `da469d50` (implementación `8a979fb5`, baseline `a99f9a63`, pin `91671f6f`). Repro del defecto re-derivado con fixtures propias del verificador contra el pin; BWC de 21 casos previamente válidos byte-idéntico calculando cada lado con sus propios binarios; permutaciones 3-métrica (×6) y 4-métrica (×24) convergentes; comparator 720 permutaciones sin mismatch; corpus G01–G36, race, coverage contracts `95.1%`, vet y gofmt PASS; SPEC diff 0; E-04/F-04 `NON_EFFECT`; E-05 no mutada. Sin findings materiales. Evidencia en `specs/FEAT-SDK-CANONICAL-CONTRACT/VERIFICATION.md` (sección INDEPENDENT VERIFIER), commit `7e628bf5` publicado sólo en la branch del erratum. Pendiente: integration review del Manager; no merge, no tag, no release.
- **Controlled integration FF (2026-09-13):** `S0_ERRATUM_INTEGRATED`, `MASTER_PIN = 7e628bf5fcadd92dc5398663d9b99a239a95ef7a`. Preflight exacto (`origin/master == a99f9a63`, `origin/fix == 7e628bf5`, merge-base `a99f9a63`, rev-count `0 3`); scope delta confirmado en los 4 paths autorizados. `master` avanzó `a99f9a63..7e628bf5` por `merge --ff-only` en worktree limpio dedicado (checkouts E-02/E-05 intocados) y push normal sin force; master contiene exactamente los tres commits certificados (`8a979fb5` implementación, `da469d50` evidencia de implementación, `7e628bf5` evidencia de verificación independiente), sin merge commit, squash, cherry-pick, tag ni release. Gates pre-push y post-integración sobre el pin remoto: tests, corpus G01–G36, race, coverage contracts `95.1%`, vet, gofmt PASS; worktree limpio. La certificación histórica del pin original `91671f6f` se preserva como historia; E-01 permanece closed sin reapertura.
...
- **2026-09-13 (CONTROLLED INTEGRATION — erratum V3-006):** `S0_ERRATUM_INTEGRATED`. Preflight Git PASS (`origin/master == a99f9a63`, `origin/fix == 7e628bf5`, merge-base exacto, rev-count `0 3`); scope recheck de los 4 paths autorizados; última autoridad vigente en `VERIFICATION.md` = `S0_ERRATUM_VERIFICATION_PASS` sin verifier contradictorio. FF-only local verificado (`HEAD == 7e628bf5`, status vacío, delta == certified) con race-check pre-push y push normal `a99f9a63..7e628bf5`; `origin/master` confirmado físicamente en `7e628bf5fcadd92dc5398663d9b99a239a95ef7a` (nuevo pin de master S0). Gates post-integración sobre el master remoto: test/corpus G01–G36/race/vet/gofmt PASS, worktree limpio. E-05 V3-006 authority blocker resuelto en master; E-05 sigue en `3bc5dca9` esperando reconciliation + Full Verifier #4. Interlock 063/062 intacto; E-04/F-04 `NON_EFFECT` registrado.

- **2026-09-13 (VERIFIER INDEPENDIENTE — erratum V3-006):** `S0_ERRATUM_VERIFICATION_PASS`. Target evidence `da469d50`, implementación `8a979fb5`, baseline `a99f9a63`, pin certificado `91671f6f`; `origin/fix == da469d50` y merge-base con `origin/master` exacto. Delta limitado a los 4 paths autorizados con `SPEC.md` diff 0. Repro, BWC de 21 casos, permutaciones, comparator, corpus G01–G36, race/cover/vet/gofmt, non-effects E-04/F-04 y no-mutación E-05 todos PASS en worktrees limpios propios del verificador. Sin findings materiales; dos observaciones no bloqueantes registradas en `VERIFICATION.md`. Evidence commit `7e628bf5` (sólo docs) publicado en `fix/s0-metric-formula-identity-erratum`. La lane queda `READY_FOR_CONTROLLED_INTEGRATION` a la espera del Manager; no merge/tag/release ni reconcile E-05.
