...
- **Recuperación operacional E-06 — prep `0.2.103` completa, solicitud owner lista (2026-09-19, mandato maestro recuperación).** Re-verificación en vivo 22:14–22:25Z sin cambio de veredicto (`BLOCKED`/`OWNER_APPROVAL_REQUIRED`): campaña RERUN-5 `c42dd637`/FlowRun `c06b09a9` sigue RUNNING (compiles 21:38Z sobre `0.2.102`; 3 backtests activos; monitor vivo en `~/aranea/work/f04-cert-f04-01-rerun5/`) → drain canónico vigente, cero efectos laterales sobre ella. Preparación NO destructiva completa y verificada: (1) `origin/codex/f05-release-prep`@`66faa42` es ancestro estricto de `3f6cd110` (FF 7 commits, delta inverso 0); (2) binarios de `release-build/` con `vcs.revision=3f6cd110` y `SHA256SUMS` 3/3 OK; (3) `release-authority` read-back 22:20:27Z: published `0.2.102`, CONSISTENT, **candidate `0.2.103`** (versión asignada por autoridad); (4) tests de release (S1–S17, 53/53, fixture auténtico) ya verificados sobre este SHA — no se repiten; (5) fixtures disponibles y byte-verificados (`mt5-export.htm` `090ca4d1…`; FIX-B6182 `21917e14…` 13074872 B); (6) rollback documentado (`deploy/<prev>` inmutable + `sqx/exporter-plugin/docs/ROLLBACK.md`). **Gap de capacidad declarado:** la sesión de agente no tiene JDK local ni SSH al host SQX, de modo que el build+`INSTALL=1` del JAR R3 (§1c del prompt) es owner-manual incluso con aprobación. **Única acción de autorización entregada:** `~/aranea/work/f04-cert-f04-01-c9r/OWNER-REQUEST-0.2.103.md` — SHA fuente, versión asignada, targets, comandos exactos del runbook existente (merge FF, `deploy_release.sh --release-only 0.2.103`, JAR `SQX_DIR`+`INSTALL=1`), alcance, drain (`CAMPAIGN_TERMINAL` + `MT5_PROCS=0` antes de rollout/JAR; lineage de la corrida viva = NO CERTIFICABLE PARA E-06 R3, NO `NO_ELIGIBLE`), rollback y las 3 decisiones en orden (corrida viva → merge/release/deploy/JAR → corrida nueva con identidad fresca; request_id `…f69f9f3b` consumido write-once). F-04 NOT CLOSED; E-06 NO cerrado.
