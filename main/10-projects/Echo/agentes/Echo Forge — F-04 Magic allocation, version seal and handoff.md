...
- **Dependencia `aranea-ssh` (2026-09-13):** previous `UNHEALTHY / HTTP 503 session limit 64`; current `BLOCKED — MCP RECOVERY FAILED` por falta de autoridad administrativa para observar/reiniciar el runtime propio de `mcps` (LXC 113). El endpoint sigue bloqueando `initialize` autenticado; F-04 permanece no physical-ready, release `0.2.98` se conserva, y T2.11/T2.12/T2.13 permanecen OPEN.
