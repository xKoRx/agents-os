...
- **M6 EJECUTADO — IDENTIDAD ANALÍTICA instrument_id/side (envelope + 065 in place) (2026-09-22, NORMAL sobre Manager Review M6; RESULT PASS):** push FF `6c7c2531..a5044112` a la única branch (read-back `origin == a5044112`; master `5dd998f1` intacto; migraciones byte-intactas salvo 065 autorizada). **Pre-flight físico:** 065 NO aplicada en PROD `echo` ni DEV `echo-develop` (sólo 029/063) ⇒ corrección IN PLACE autorizada. **M6-A:** `TradeFactEnvelopeV1` porta `instrument_id` (CanonicalSymbol EXACTO; jamás broker_symbol/lookup) y `side` (BUY→LONG/SELL→SHORT exacto; inválido/vacío ⇒ fail-closed SIN publicar) FUERA de S0; wire literal intacto; `FactRefForV1` FROZEN sin cambio — la divergencia analítica se arbitra por el `fact_digest` (vista §5.1 con los campos nuevos: jamás converge como replay). **M6-B:** 065 añade columnas/constraints write-once (`chk_e7_raw_analytical_identity`, `chk_e7_lifecycle_open_identity`, guard `E07_LIFECYCLE_OPEN_IDENTITY_REQUIRED`/`WRITE_ONCE`); el OPEN aceptado FIJA los pins; late CLOSE no fabrica (NULL); hechos posteriores reutilizan el pin; contradicción explícita ⇒ `ErrLifecyclePinConflict` ⇒ cuarentena `IDENTITY_CONFLICT` (campo `instrument_id/side`) + raw SKIPPED — jamás reescritura; `trade_deals` sin duplicación (ledger se une al lifecycle). **ROJO→VERDE:** wire pre-corrección sin ninguna clave analítica (capturado contra árbol pristine) → verde completo. Gates: harness E-07 PASS (up/down/up + matriz M6, :15485); stores/consumer `-race` PASS; `TestE10*` 24/24; failing set 121/121 idéntico; run.sh E-08/E-09 PASS con el 065 corregido; arnés E-07 excluye 066–069 en rebuild (patrón M2-C2; falla preexistente demostrada). Contrato: SPEC §18 (ERRATUM-B); evidencia: VERIFICATION §11. `MQL_COMPILE_PENDING` y `PHYSICAL_PENDING` sin cambio (el productor V1.1 ya porta canonical_symbol/side); `FINAL_CLOSED=NO`. Consumidor: E-10 T05 COMPLETADO (ver nota E-10). Agent run: `[[2026-09-22-zcode-glm53-e10-m6-e07-t05-completion]]`.

- **UBICACIÓN DE CÓDIGO (2026-09-21):** `3765f2ba` es ancestro de `origin/feature/e09-execution-copy-reconciliation-fidelity` @ `865532078f2c1993e7a3a542a78a9db0fad1f015`. La ref remota `feature/e07-raw-facts-deal-lifecycle` fue retirada. El worktree `/tmp/echo-e07-raw-facts-deal-lifecycle` quedó en `3765f2ba`. PHYSICAL_PENDING y FINAL_CLOSED=NO sin cambio.
...
- **2026-09-22 — M6 identidad analítica ejecutado (una sesión autónoma):** defecto demostrado por el Manager (el DTO Reference porta CanonicalSymbol+Side y buildEnvelope los descartaba) corregido @ `6b284a63`+`75b33bed`. ROJO conductual capturado contra `6c7c2531` pristine (probe temporal retirado; worktree quedó byte-intacto): el envelope publicado NO portaba ninguna clave analítica. 065 corregida in place tras demostrar (MCP RO/DEV) que jamás fue aplicada en entornos compartidos; down simétrica. Cuarentena de contradicciones analíticas en la ruta Core con raw sellado SKIPPED (terminal 029; el sweep jamás la reproyecta). Arnés `tests/trade_facts_e7`: rebuild excluye 066–069 (drift preexistente demostrado: el loop viejo abortaba en el interlock 066) + aserciones M6. Evidencia completa: VERIFICATION §11.
