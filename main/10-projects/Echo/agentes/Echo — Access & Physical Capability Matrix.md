...
| GAP-ECHO-004 REDEFINIDO 2026-09-15 | HIGH | `.211:8090` MUERTO (22/8090 closed — host down). Runtime vivo real = PROD `192.168.31.71` = `prod.echo.gateway.lab.aranea` (PTR, `/health` 200 `{"status":"ok"}`; 8082 cerrado; Go net/http). DEV runtime NO desplegado (docker-echo-dev sólo Flink/Hasura/Portainer). Viewer SSH `echo-runtime-prod` staged en `aranea-ssh` — pending owner key install | E-02, E-06…E-13 | OWNER: instalar `~mcps:/opt/mcp/ssh/keys/echo-dev.pub` en `authorized_keys` de `echo-dev` en `.71`; después recertificar identity+health+logs+listeners+negative run-command |
