...
**GAP-ECHO-004 CLOSED 2026-09-15 (was HIGH)**: owner seed instalado y viewer `echo-runtime-prod` certificado end-to-end desde Daedalus real — identity (`whoami=echo-dev`, `hostname=echo`, `id uid=1001` sin sudo), Gateway/Core RUNNING (`echo-gateway` PID 713, `echo-core` PID 110701, `echo-functions` PID 320982, como `kor`), **Bridge NOT_DEPLOYED** (registrado con evidencia, no se levanta), listeners 80/9080/9090/8080/8090, negative `run-command` → POLICY_DENIED MUST DENY, leak check CLEAN. VEREDICT: GAP-ECHO-004 CLOSED. Evidencia completa: [[ACCESS-CERTIFICATION]] § GAP-ECHO-004 CLOSED. | ninguna — capability operativa; viewer no cubre systemctl/docker/curl productivo (clase `safe` también rechazada); logs productivos siguen vía `aranea-observability-ro` |
