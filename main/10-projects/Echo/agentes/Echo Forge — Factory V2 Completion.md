...
**(Actualizado 2026-09-21.)** F-05-I está `IMPLEMENTED / SOURCE VERIFIED` desde el 2026-09-16 (T7-CLOSE `3d0e8c9`; ver bitácora) — esa preparación de CERT-F05-01 ya está entregada. Recertificación CERT-F04-03/CERT-E04-01 sobre Daedalus (2026-09-21): Core/Gateway DEV sanos pero ingest Echo misconfigured + golden bodies bloqueados + defectos F-INT-* STILL_REPRODUCIBLE @ `25a5122` ⇒ **`CERT_F04_03_BLOCKED`**, G6 NOT_EXECUTED. Evidencia: `~/aranea/work/cert-int-qa-20260921/FINDINGS-CERT-INT-QA-20260921.md`. No queda tarea de ejecución Forge desbloqueada sin owner actions (golden RO + ETCD ingest + Echo manager fixes); el orden frozen sigue CERT-E04-01 → CERT-F04-03. Pendiente de decisión manager (no ejecutar autónomamente): refresco de `sqx/core/releasematrix/release-matrix.json` para reflejar CERT-F04-01/02 PASS — exige ampliar la allowlist histórica física del guard (`FROZEN_CONTRACT_COLLISION` potencial, STOP previsto por la SPEC F-05-I).
