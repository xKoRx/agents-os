...
> - [ ] [[Echo Forge — Forge Explorer v0]] Forge Explorer v0 — **DEFERRED_BY_OWNER 2026-09-21** (implementado en rama `codex/forge-explorer-v0` @ `648d5e6`; no integrar ni desplegar) #owner/agent #type/dev #area/echo
...
**(Actualizado 2026-09-21 — SCOPE CLOSURE / MIGRATION.)** **FACTORY_V2_PHYSICALLY_CERTIFIED — ratificado con cadena completa** (deudas de superficies resueltas; ver bitácora RECOVERY y manifiesto de reconciliación). CERT-F05-01/02/03 PASS en la misión F05 FINAL COMPLETION (release `0.2.105` @ `745bc8b`; campaña FULL `0ce72173…` COMPLETED/TARGET_REACHED con 3 finalistas; manifest final + debt aceptada en `~/aranea/work/cert-f05-20260921/CERT-F05-03-MANIFEST-PASS-20260921.md`). **Owner actions menores del manifest RESUELTAS 2026-09-21** (bitácora RECOVERY: mongo/uri flowkit provisionado, `run get` certificado; warnings PG leídos del Decision PG). **Forge Explorer v0 → DEFERRED_BY_OWNER (mandato 2026-09-21): no implementar más, no integrar, no desplegar; rama `codex/forge-explorer-v0` @ `648d5e6` conservada.** Ya no queda ninguna tarea de ejecución Factory. **Siguiente (fuera de este subproyecto): ejecutar la migración de Forge al monorepo `xKoRx/echo` según el mandato de cierre 2026-09-21 — baseline `0.2.105` @ `745bc8b`, blockers: FF master, decisión `codex/f05-post-cert-delta`, sdk `c7f1149`, secretos versionados, lanes E-06.**
...
- **2026-09-21 — Mandato TOP SCOPE CLOSURE: Factory V2 cerrada; Explorer DEFERRED_BY_OWNER; siguiente = migración.** Auditoría de cierre verificada contra repos (symphony @ `745bc8b`, echo @ `5dd998f1`/`7c843e9c`, sdk @ `c855944`): las 7 certificaciones se mantienen (no hay regresión demostrada); la matriz post-cert `145d6be` declara pipeline/physical/deploy DONE salvo f01/f02 physical OPEN (documentadas; el FULL campaign ejercitó ambas) y b1a/b1b/b2 deployed OPEN (higiene de fila, el binario desplegado es el mismo). El POST automático de handoff es config-only sobre código certificado (`sqx-worker/main.go` → `HTTPIngress`; fail-closed a `HANDOFF_CREATED`); las certificaciones independientes satisfacen F-04 §11 — smoke en-campaign configurado queda para el primer join real. Deuda de migración exacta: master symphony `0b9742b` 38 commits atrás de la línea certificada; `codex/f05-post-cert-delta` (+2: F-INT-04 fix, pin contracts→`5dd998f16aea`, sdk→`c7f1149` rama sin merge, refresh matrix DONE) verificado sin release; lanes E-06 divergentes (+5/+7); secretos versionados (p.ej. connStr en `scratch_query_test.go`) a limpiar antes de mover historial. Detalle en [[Echo Forge]] y [[echo-forge-integration-boundary]].

- **2026-09-21 — RECOVERY F05: reconciliación RATIFICADA + deudas resueltas + F-INT-06 corregido.** Diagnóstico corregido: MongoDB nunca estuvo roto — el gap era provisioning del namespace del CLI (`/sqx-flowkit/production/mongo/*` ausentes; los workers tienen los suyos en `/sqx-worker/production/`); MCP Mongo con fallo transitorio de sesión. Provisionado `mongo/uri`+`mongo/database=forge` por copy server-side (rev 58876/58877, SHA256 fuente==target, secreto nunca impreso). `run get` certificado desde build @ `745bc8b`: exit 0, `forge-result.v2`, determinismo byte-idéntico con `--ranking`, negativos 4/2/2/10, consistencia total con CERT-F05-02; **warnings 3/3 finalistas = `PNL_SIGN_FLIP`** leídos del Decision PG (semántica: signos opuestos net_profit baseline/candidato). Byte-verify MinIO HTM rank-1 (`e1a4de74…`) re-verificado; Temporal Completed/0 Running. **CERT-F05-01/02/03 + FACTORY_V2 ratificados** con errata/superseding en `~/aranea/work/cert-f05-20260921/F05-RECONCILIATION-20260921.md`. Delta `codex/f05-post-cert-delta` @ origin (`14612d1`: F-INT-04 fix+regresión, pin contracts→5dd998f, refresh frozen de release matrix — 8 filas física DONE, 14 deployed DONE con 0.2.105, allowlist exacta; `145d6be`: stdout puro activando opt-in SDK `TELEMETRY_LOG_STDERR` @ `codex/telemetry-stderr-opt-in` `c7f11496b6f6`, nuevo hallazgo **F-INT-06** — el CLI violaba §1 del contrato F-05-I con logs en stdout; detectado por el Explorer como primer consumidor estricto). Sin release 0.2.106, sin restart de flota, sin campañas.
- **2026-09-21 — Forge Explorer v0 IMPLEMENTADO (NORMAL despachado y completado).** Manager review del paquete frozen + baseline verificado; EX0–EX6 con commits atómicos `01ceaa8`→`648d5e6` push FF a `origin/codex/forge-explorer-v0`. Gates §6 GREEN (build/test/vet/gofmt/diff-check/imports sólo stdlib/regresiones). Smoke con fixtures + **smoke real read-only contra production**: navegación HOME→CAMPAIGN→RUN (3× PNL_SIGN_FLIP reales)→STRATEGY→RELEASE, 404/405 correctos. `PHYSICAL CERTIFICATION NOT RUN`. Binario accidental purgado del historial antes del push.
- **2026-09-21 — CERT-F05-02 PASS + CERT-F05-03 PASS → FACTORY_V2_PHYSICALLY_CERTIFIED.** Campaña FULL `0ce72173…` (FlowRun `9d1b41ac…`) sobre `0.2.105`: COMPLETED/TARGET_REACHED, 3 finalistas (magics `26090011019/020/021` write-once), funnel 100% verde (20 retester → 17 optimizer → 17 WFM → 3 select/apply/reretester → 3 compiles Windows → 3 backtests físicos → 1 promotion), slot MT5 `slot-1` serializada (4 muestras de lease con handover limpio), HTM 3/3 byte-verificados (`e1a4de74…`/`68b42e65…`/`3cc7c852…`), delivery `HANDOFF_CREATED ×3` con 0 POST (ETCD echo 0 keys), result surfaces inspectadas (campaign get/run stages/strategy get ×3/release-matrix; `run get` fail-closed por mongo/uri sin provisionar = owner action). Manifest final con veredictos por gate, no-regresiones y debt aceptada: `~/aranea/work/cert-f05-20260921/CERT-F05-03-MANIFEST-PASS-20260921.md`. F-05 → `PHYSICALLY CERTIFIED / CLOSED`.
