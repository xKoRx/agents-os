...
> - [x] **C2** Auditar personalmente evidencia SQX vs GUI/report y congelar campos confiables #owner/me #type/research #area/echo — **EVIDENCE_COMPLETA 2026-09-26 (9.ª sesión):** fix NDX canónico integrado (rama `fix/ndx-canonical-instrument`, master `61cae1f`, release 0.2.108, flota 3/3), wave1c FlowRun `254977a0` COMPLETED con **727/727 Evaluations+MetricSets persistidas** (scope canónico `ndx`, provenance IMPORTED_HISTORICAL). Pendiente del owner: auditoría personal de campos vs SQX GUI (trust matrix) — la evidencia ya existe para contrastar.
> - [/] **C3+C4+C5 (experimento wave1c)** — EJECUTADO 2026-09-26: classification 727/727 → 8 tipos lógicos; ranking weighted_combination_minmax 30/30/40 PER_LOGICAL_TYPE top_n=5 (durable config histórica `0221b984`, procedencia G7 — ratificación owner abierta); **TopProjection 34 → grouped First Retester `FirstRetestConfig.cfx` SHA c0ea66f1… → 34/34 COMPLETED**; reproducibilidad del ranking verificada 727/727; **RANKING REVIEW PACKET entregado** (`~/aranea/work/forge-shot1-wave1c-20260926/artifacts/`). Revisión owner C3/C4/C5 = siguiente gate humano.
...
- **2026-09-26 (9.ª sesión — SHOT 1 CANDIDATE READY: NDX canónico integrado + wave1c COMPLETED + classification/ranking/First Retester ejecutados + Ranking Review Packet):** sesión ZCode/GLM-5.3-Flash en Daedalus, mandato "SHOT 1 — CORRECCIÓN + CANDIDATE EXECUTION" (TOP agent). **Corrección conceptual (decisión owner en mandato): canonical = NDX como concepto de mercado; USATECH/USATECHIDXUSD/USATECHIDXUSD_darwinex son ALIASES de NDX** (revierte la dirección alias→canónico propuesta en la 8.ª). Rama corta `fix/ndx-canonical-instrument` @ `55667ab` (+cherry-pick del knob de producto `SQX_WATCHER_INTAKE_DEADLINE` con test nuevo `fc7a528`/`ffb4d06`; el knob candidate-local auto-upgrade NO se mergeó; normalización go.work.sum `0dfde58` ⇒ releases vcs.modified=false). **Release 0.2.108** (commit `61cae1f`, worker SHA `ff394f7c…`, vcs.modified=false verificado) + rollout 3/3 Zeus/Hera/Kronos. **wave1c despachada y COMPLETED** (FlowRun `254977a0`, request `NDX_SQX_v1_wwave1c`): cohort verificado byte-exacto 727/727 vs freeze (agregado `79a67e80…`; los 4 "(1)" renombrados `.d1` según freeze — el databank los conserva); intake con deadline 15m (knob producto); **import 727/727 COMPLETED con scope canónico "ndx"** (fix NDX verificado en evidencia durable); classification 727/727 → **8 tipos lógicos** (431/232/28/16/11/5/2/2); ranking 30/30/40 PER_LOGICAL_TYPE **top_n=5 de la durable config histórica `0221b984`** (procedencia G7 documentada, ratificación owner abierta); **TopProjection 34 → grouped Retester batch_size=1 (contrato FEAT-SQX-IMPORT-CAMPAIGN-B §2.1) con `FirstRetestConfig.cfx` SHA `c0ea66f1…` (read-back OK; `RetestPrecision.cfx` copiado byte-exacto a Hera/Kronos tras descubrir que sólo Zeus lo tenía) → 34/34 retests COMPLETED**; match exacto 1:1 proyección↔retest a nivel artefacto. **Reproducibilidad del ranking verificada 727/727** (recompute independiente min-max+pesos == snapshot; ranking_value == Σ componentes). Resultados de retest extraídos con corrida técnica `wave1c_r` (`79e0cc50`, 34/34 MetricSets): PF_retest 1.46–2.18, 33/34 ≥ 1.5, degradación sistemática vs pre (ΔNP media −2,041; spread 1.5 + RetestPrecision); convención de lectura PF≥1 & NP>0 ⇒ 34/34 sobreviven. Findings: top_n/particiones-2 requieren ratificación owner; outliers comprimen min-max; CFX retest sin CustomAnalysis ⇒ sin métricas nativas del stage (extracción vía wave1c_r; BEFORE_NEXT_RETESTER). Residuos reconciliados sin deletes (2 FlowRuns PENDING propios = FAILED_ATTEMPT_KEEP; cohort/históricos intactos). **OWNER REVIEW PACKET (paths + SHA256):** `~/aranea/work/forge-shot1-wave1c-20260926/artifacts/` → `RANKING-AUDIT.csv` `de942ff1f5e3e88b…` (727 filas), `RANKING-AUDIT.json` `963fa699815d8529…`, `RANKING-PRE-RETESTER.md` `3b6999c05455095d…` (freeze pre-retester), `RETESTER-RESULTS.csv` `b8b0b797978b9cb6…` (34 pre/post+deltas+SHAs), `RANKING-REVIEW.md` `6576cfe14e1f50f9…`; evidence pack `EVIDENCE-WAVE1C.md`. **Veredicto: SHOT1_CANDIDATE_READY (pendiente revisión owner del packet + trust matrix C2).** NEXT EXACT: `Shot 2 — Independent Verification. Agente fresco recalcula Classification/Ranking desde exact Evaluation/MetricSet refs, falsifica NDX canonicalization, normalización, pesos, ranks, cutoffs y verifica que el grouped First Retester consumió exactamente la TopProjection congelada. Cero product fixes durante Shot 2.`
- **2026-09-26 (8.ª sesión — Shot 1 Ranking→Retester: `SHOT1_BLOCKED_PRODUCT`; causa raíz C2 = instrumento NDX no-canónico, no la GUI; sync histórico flota EQUAL):** sesión ZCode/GLM-5.3-Flash en Daedalus, mandato: validación Ranking → subflujo Retester agrupado con `OWNER_RETESTER_CFX` = Zeus `/home/kor/sqx/user/configs/FirstRetestConfig.cfx` (owner dijo `config/`; path real `configs/`), SHA `c0ea66f1…`, 6389 bytes, inspección OK (USATECHIDXUSD_darwinex H1, Darwinex, M1 2016→2026-05, databanks input/output, templateFile absoluto Zeus-local). **Intermisión owner en vivo: alineación de datos históricos Zeus→Hera/Kronos ejecutada ANTES de despachar a flota** con la skill canónica `sqx-instrument-sync` v3.0.0 (`sync_user_data_zeus.sh`): audit DIVERGENT total=88 (Hera/Kronos 893 MB vs Zeus 6,1 GB; faltaban `DEUIDXEUR_darwinex` y `NDXm_darwinex_TICK`); bloqueo resuelto = host keys ED25519 stale en known_hosts de Zeus (verificación anti-MITM por doble camino + backup `known_hosts.bak_sync_20260926` + re-add); ventana de mantenimiento dada por el owner al cerrar la GUI de Zeus; **SYNC EQUAL** hash maestro 3/3 `07f1f285…` (hera 30 cambios/74s, kronos 60/75s). **Corrección de la continuidad (logs worker Hera):** el 25-sep 05:00Z SÍ hubo retry del import hop (attempt 2 en Hera): el sqcli de las 727 corrió completo ("Success marker encontrado") y el fallo real fue **`persist_import_evidence: overview scope instrument "NDX": contract_conflict` (no-reintentable)** — `CanonicalSymbol("NDX")==""` porque el catálogo canónico (`canonical_scope.go`, proyección Echo: majors+xauusd+alias `usatechidxusd_darwinex→usatechidxusd` ratificado G7 `b697815`) no contiene NDX ⇒ 0 Evaluations/MetricSets para las 727 con identidades adoptadas bajo `instrument="NDX"`. Verificación en vivo del re-despacho mismo-request (`NDX_SQX_v1_wwave1b`, manifest reconciliado byte-exacto 727/727 put-if-absent): el workflow Temporal del FlowRun está **wedged** — el dispatch devolvió el mismo run_id `01a0d6e8…` sin que ningún worker tomara tareas ⇒ re-ejecución del FlowRun 1a4d66d6 imposible por identidad. Intento de spec corregido (instrument=`USATECHIDXUSD_darwinex`, resto byte-idéntico): fail-closed en adopción (`strategyV2AttributeConflict`, `adopt_strategy.go:268`) ⇒ corregir el spec tampoco es viable sin cirugía de identidades (prohibida). **Desbloqueo propuesto (decisión owner): (1) rama corta R7 con alias `ndx→usatechidxusd` en `canonical_scope.go` + test (o incorporar NDX al catálogo Echo); (2) re-despacho técnico wave1c = nuevo FlowRun con las mismas 727 byte-exacto y mismas identidades (precedente wave1b); (3) worker 0.2.108 o candidate-local aislado; (4) recién entonces classification→ranking→exports→group ranking_snapshot→Retester con el CFX owner.** Limpieza 100% del intento corregido (5 objetos MinIO borrados ×2 por retry del watcher, verificado count=0; staging restaurado; watcher detenido; fleet sin mutaciones). Residuo documentado: posible fila `sqx.flow_runs` del intento corregido (mint previo a la adopción fallida; clase ya conocida). Evidence pack: `~/aranea/work/forge-retester-shot1-20260926/EVIDENCE-RETESTER-SHOT1.md`. Repo sin delta de producto. **NEXT EXACT: (1) owner decide desbloqueo (alias R7 + wave1c vs alternativa); (2) merge + release + re-despacho; (3) con enriquecimiento COMPLETED continuar este Shot desde classification.**
