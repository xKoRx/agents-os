...
- **Prework ambiente Fase 1 (2026-09-24): `PREWORK_BLOCKED`** — Forge DEV aislado demostrado físicamente en Daedalus (watcher 7/7 pipeline contra ETCD/MinIO/PG/Temporal DEV; workflow real `GenericSQXWorkflow` en `sqx-dev`; cola `sqx-main-queue` con cero pollers ajenos ⇒ sin fuga a flota compartida; worker DEV fail-closed sin `sqcli`). **Blocker único: SQX DEV en Daedalus no tiene distribución (Build 142 ausente en host/MinIO, sin SSH a flota) ni licencia autorizada ⇒ Gates C/D imposibles.** Corrección aplicada: 2 claves ETCD DEV del watcher (`temporal/namespace` `sqx`→`sqx-dev`, `sqx/task_queue` `sqx-dev-queue`→`sqx-main-queue`; rollback = restaurar valores previos). `PREWORK_MT5 = DEFERRED_UNTIL_C6`. Detalle en [[Echo + Echo Forge — Environment Contract]] §5.8 y memoria de continuidad.
