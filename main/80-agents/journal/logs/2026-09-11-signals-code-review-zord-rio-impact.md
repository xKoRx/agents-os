...
- Segundo refinamiento solicitado por el owner: piloto local de reviewer RIO independiente y exclusión absoluta de proyectos no Meli. `zord check rjara-rio-impact` confirmó configuración válida, source `global`, hook `manual` y `enabled: false`; `zord list` mostró 9 disponibles y el reviewer apagado; el dry-run estándar en `rio-playmaker` seleccionó 7 reviewers y el mismo dry-run con `--include rjara-rio-impact` seleccionó 8, identificando el nuevo reviewer como global. Ambos finalizaron con exit `0` y sin cambiar el working tree preexistente.
- Validación posterior del vault: schemas `skill`, `runbook`, `change_log` y `tool` con `errors=0`; lint estricto sobre skill, runbook, tool, índice y change log con `ERROR=0 WARN=0`. El gate negativo quedó duplicado en trigger, procedimiento, hard rules y prompt del reviewer: un target no Meli o de identidad incierta termina `NOT_APPLICABLE` antes de invocar Zord.
- `graphify-obsidian update` se reintentó después del refinamiento y volvió a quedar bloqueado únicamente por deuda ajena al delta: `Sin título.md` sin frontmatter y el proyecto Echo con `status: closed`; el wrapper conservó el último índice válido.
