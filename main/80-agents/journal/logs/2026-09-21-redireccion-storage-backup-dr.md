...
## Addendum noche-3 (hardening final de storage y replicación — auditoría RO, cero mutaciones)

**Mandato**: ONE-SHOT hardening (Principal ZFS Engineer + Backup/DR Auditor). **Supersede 3 afirmaciones de la noche-2**: (1) "/replication 404" → la ruta EXISTE (401 sin auth; familia `replication.*` en middleware; enum `LOCAL` en el código instalado) — el mecanismo definitivo es el **stack NATIVO zettarepl LOCAL** y el plan cron+zfs queda DESCARTADO; (2) "0 periodic-snapshot tasks" → había 1 legacy (id=1, dataset inexistente `pool0/vm_storage/hades-cdfezs`, estado HOLD); (3) "doble conteo ~1,8T por orígenes borrados" → histéresis `usedbychildren` de 442 snapshots legacy de `pool2/backup` (2,05T; pares snapshot+head coexisten — delete-by-txg). Números corregidos: envío inicial **2,35T base `refer`** (ledger §1 SPEC); pool0 free **1,64T zpool** (no 945G); margen post-full **≥1,71T** → CAPACITY_GO por aritmética. Nuevos: gates **G-REP-0** (fixture `pool2/fixrep`) y **G-REP-5** (alertas; `smtp=false` medido); horario 04:45/04:50 fundado en dependencias reales (timers hermes); `pool.scrub` nativo confirmado para tarea mensual pool2; errata adicional: snapshottask id=1 apunta a dataset inexistente (HOLD stale). **Viernes: MIGRATIONS_NOT_READY** (sin migraciones en la ventana 26; W5 = reevaluación post-activación réplica).

Archivos: vault — POOL0-TO-POOL2-REPLICATION-SPEC (hardening §1/§2/§3/§3bis/§4/§5/§6/§7) · MANDATO-REPLICACION-SPEC · MANDATO-PREP-SPEC · MANDATOS-MIERCOLES-23SEP · ROADMAP-WP-BACKUP-DR (fila A6) · ARANEA-CONTINUIDAD (delta noche-3) · zfs-replication-runbook. Workspace — `~/aranea/work/replicacion-pool0-pool2/` (REPLICATION-PAYLOADS.md, ALERTA-SMTP-PENDIENTE.md) · `continuity-20260923/TABLA-APROBACION-23SEP.md` (fila P6; urgencias P1/P4+P6).
