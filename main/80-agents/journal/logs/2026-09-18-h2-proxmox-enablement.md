...
- G4 sesión fresca Ariadna (hijo aislado, read-only): resolución de `sqx-hera` desde inventario (VM 123 @ hera, running) con verificación viva API+SSH (uptimes coincidentes), ACL/rol leídos en vivo (`AriadnaVMBackup` = `VM.Audit,VM.Backup`, privsep=1), negativo demostrado: POST start/shutdown con el token → 403 `VM.PowerMgmt` (sin ejecución), clasificación de operaciones por canal y management independence (cero llamadas MCP). Resultado resumido en la nota del proyecto; transcript en `~/aranea/work/h2-enablement-20260918/g4_child_transcript.log`.
