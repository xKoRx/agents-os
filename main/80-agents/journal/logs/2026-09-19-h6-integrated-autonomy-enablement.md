...
- **G7 golden end-to-end (G8 del mandato):** 4 escenarios A/B/C/D ejecutados por hijo aislado (sesión fresca) con SOLO proyecto + catálogo + matriz + contrato + referencias canónicas (sin respuestas): A nuevo servicio DEV = **GO condicionado a EXECUTOR_NOT_DEFINED** (capacidad verificada viva GO: local-lvm @hera 254.8G avail; pool1 NO_GO 87.28% vivo, N4; sin proyecto ejecutor ⇒ N9 + ficha propuesta; VMID/IP revalidables, ninguna reserva); B incidente de servicio = **diagnóstico read-only completo + READY_FOR_EXECUTOR no ejecutado** (sqx-hera VM 123 @hera verificado vivo por canal host-mediated; hallazgo: `symphony-worker.service` inactive+disabled posiblemente intencional — el hijo distinguió "estaba roto" de "lo rompí", no reinició nada y escaló confirmación de estado esperado al dueño del servicio; handoff a [[service-lifecycle-operator-contract]] con pre-state, timeout, validación y prohibición de tocar unused discs); C Backup/DR = **READY_FOR_EXECUTOR → [[BACKUP-DR-OWNER-PROJECT]]** (ejecutor SÍ existe; estado R2 correcto: piloto 7d ACTIVE, primer disparo pendiente 20-sep 06:05; GATED por tickets 018/019 + decisión D post-piloto; go-live BLOCKED hasta protección demostrada con restore drill); D Ceph nearfull = **NO_GO + OWNER_GATE_REQUIRED (CLASS 3), estado GATED** (nearfull confirmado en vivo desde 2 MONs: pool1 87.28%, osd.0/2 85.6% vs osd.1/3 40–45%, VAR 0.62–1.33, 129 PGs active+clean; handoff a [[ceph-storage-operations-contract]] con orden de ataque por riesgo ascendente, validación, rollback y abort). Todos read-only; declaraciones del hijo: `operaciones_mutantes=0, sesiones_ssh=4, mcp_calls=0, secretos=no`; evidencia incremental en `~/aranea/work/h6-integration-20260919/golden/`.
...
- **Gaps documentales confirmados por el golden (no bloqueantes, asignados a dueño):** (i) sin registro vivo verificable de discos huérfanos — el contrato Ceph cita como candidatos de liberación VMIDs extintos (112 kronos-sqx-deprecado, 162, 170) y la anomalía unused0/unused1 de sqx-hera 123, pero no existe inventario de huérfanos actualizado: primera tarea del Proyecto Ceph propuesto ANTES de cualquier liberación; (ii) estado esperado de `symphony-worker.service` (inactive+disabled en sqx-hera 123) sin documentación del dueño del servicio SQX — el golden correctamente no lo tocó; (iii) catálogo servicio→puerto/health-check no legible en fuentes canónicas (el service_map H0 vive en evidencia externa) — candidato a integración documental por el carril correspondiente. Nota de instrumentación: el hijo declaró `sesiones_ssh=4` (sesiones lógicas por escenario); el transcript registra 8 invocaciones ssh read-only — discrepancia de conteo registrada en el manifiesto, sin efecto sobre el resultado (todas las sondas read-only, 0 mutaciones).
- **G9 continuidad (G11 del mandato):** hijo aislado con SOLO el punto de entrada Agents-OS (AGENTS.md→bootstrap→router aranea) resolvió solicitud nueva representativa ("necesito subir un LXC DEV para un servicio interno") sin contexto oculto ni golden copiados: enrutó a [[provisioning-operator-contract]] + ejecutor EXECUTOR_NOT_DEFINED (ficha propuesta) + N4 NO_GO pool1 + N9 + handoff reproducible emitido (`~/aranea/work/h6-integration-20260919/continuity/fresca_handoff_lxc-dev.md`). Requirió 0 secretos, 0 mutaciones, mcp_calls=0. DOCUMENTATION_GAPs detectados por el hijo y disposición: (1) contratos sin ruta filesystem declarada → CORREGIDO en el mandato (nota de navegación en 07-integration); (2) ubicación/materialización de fichas de proyecto propuestas no especificada → CORREGIDO en el mandato (ídem); (3) gate ARGUS/observabilidad sin procedimiento enlazado vigente (doc legacy 2026-06-28) → deuda preexistente de otro carril, registrada; (4) sin política pública de backup-por-defecto para LXC DEV (vida-sin-backup) → decisión owner [[BACKUP-DR-OWNER-PROJECT]], registrada como deuda; (5) sufijos DNS `*.lab.aranea` vs `*.lab.aranea.cl` — NO es contradicción: H5 documenta ambos y la revalidación en vivo de este carril confirma que ambos resuelven (dashboard.lab.aranea, mt5-kronos.lab.aranea.cl, ca.lab.aranea); (6) fricción menor del toolbelt de búsqueda (no del vault). Ningún gap bloqueante para continuidad.
