...
| CL-14 | ROUTING | Fila o link de un `00-index.md` de dominio wiki activo apuntando a archivo inexistente (se verifican todos los wikilinks de cada fila) | FAIL | 00-RESOURCE-WIKI (una fila por vigente) |
| CL-15 | ROUTING | Fila o link de un `00-index.md` de dominio activo apuntando a página no vigente o archivada (todos los wikilinks de cada fila) | FAIL | 00-RESOURCE-WIKI (el índice deja una sola entrada vigente) |
...
Múltiples candidatos sin match único → hallazgo WARN "ambiguo" con la lista de candidatos. El índice de resolución cubre **corpus vivo + `40-archive/`**: journal, packaging (`30-resources/agents-os/`) y resultados derivados NO son destino canónico (`agents-os.md`: "auditoría o distribución, nunca autoridad vigente"; relation-maintenance: no enlazar sessions/logs) — un link hacia ese material se reporta como no resuelto y, si el destino existe físicamente en la zona excluida, la evidencia lo distingue como "existe en zona no canónica; no es destino canónico" en vez de "inexistente" (CL-11/CL-12; el veredicto no cambia). En el escaneo de cuerpo se excluyen fenced code blocks e inline code spans (Obsidian no renderiza wikilinks dentro de código). Autoridades citadas: `agents-os-hygiene-review/SKILL.md` (Link And Alias Checks) y `graphify-contract.md` (semántica vault-wide).
