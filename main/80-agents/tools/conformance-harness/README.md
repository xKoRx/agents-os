...
- **Pre-flight — RULES-FIDELITY-ANCHORS (corre en toda capa antes de los escenarios):** guarda anti-deriva de la transcripción (D2 de la verificación adversarial): assertion estática de que las citas literales de las autoridades que `rules.py` declara transcribir — registro opcional, matching exacto de área/evidencia de tarea y sentencia fail-closed del paso 6; cold set, warm turn, entity swap y regla superseded de bootstrap; líneas del club del doctor Check 3 — siguen existiendo verbatim en los archivos vigentes. La cobertura del guard es exactamente el texto de decisión listado en `rules.FIDELITY_ANCHORS`, no equivalencia semántica completa. Ancla ausente → FAIL; archivo ausente → FAIL, nunca crash. Correrlo solo: `--scenario RULES-FIDELITY-ANCHORS`.
- **L0 — STATIC (8 escenarios):** parseo/grep determinista de archivos y frontmatter: validador del schema-contract, startup no duplicado, closed club `load_policy: always`, vocabulario de `load_policy`, paridad de las filas transversales del INDEX always-load y catálogo completo federado contra disco, frontera global↔federada, cero secretos en Markdown vivo y pureza de dominio de la memoria activa.
...
El domain gate se carga desde `30-resources/agents/domain-router-registry.md`.
`registry_selftest.py` verifica cardinalidad `0 → DEFAULT`, `1 → router` y
`>1 → fail-closed` con dominios sintéticos, sin modificar el registro real.
...
- **Context baseline (siempre):** bytes/chars/tokens aproximados (chars/4, baseline-only, sin tokenizador de autoridad) del always-load (constitución, perfil, continuidad global, INDEX) y de cada pack realmente registrado. En una instalación DEFAULT el mapa de packs queda vacío; los escenarios scoped a dominios ausentes reportan SKIP explícito. Sale en el JSON y como línea del summary; nunca es criterio de fallo.
...
- Opciones: `--vault-root <path>` (default: autodetección subiendo desde el script hasta la carpeta que contiene `80-agents/agents-os/agents-os.md`; si no la encuentra, error), `--no-live` (omite la parte de configs de máquina de L2, p. ej. fuera de la máquina del owner), `--no-write` (no crea el record bajo `results/`; usado por el Doctor unificado).
...
- `WARN`: riesgo o ambigüedad verificable, no violación demostrada; las ambigüedades declaradas por los audits son WARN por diseño y no degradan el veredicto. WARN heredados: extras de vocabulario `load_policy`; la nota VPN cross-domain sin campo `area` en COLD-ARANEA; áreas no canónicas en memoria de dominio; clasificación de modo ambigua al resolver entidad desde DEFAULT; keyword+UUID ambiguo en el barrido de secretos. La disponibilidad ambiental de herramientas ya no activa dominios y COLD-DEFAULT es PASS.
...
- Determinista y sin red/DB/CI/daemon; cada run escribe un `results/run-<timestamp>.json` con `{run, baseline (git HEAD informativo), levels, scenarios, counts, context_baseline}`, salvo que reciba `--no-write`.
