...
- **Pre-flight — RULES-FIDELITY-ANCHORS (corre en toda capa antes de los escenarios):** guarda anti-deriva de la transcripción (D2 de la verificación adversarial): assertion estática de que las citas literales de las autoridades que `rules.py` declara transcribir — líneas del domain gate del paso 6, cold set (pasos 1-3), warm turn, entity swap y regla superseded de bootstrap SKILL.md; líneas del club del doctor Check 3 — siguen existiendo verbatim en los archivos de autoridad vigentes (el emparejamiento colapsa solo whitespace por los saltos de línea; las palabras deben ser exactas). Cada ancla vive en `rules.FIDELITY_ANCHORS` (`rules.py`) citando su archivo de autoridad. Ancla ausente → FAIL con la cita faltante (transcripción obsoleta: los resultados de los escenarios dependientes del gate no son confiables; en runs full/--layer el gate L0 corta L1/L2); archivo de autoridad ausente → FAIL, nunca crash. Correrlo solo: `--scenario RULES-FIDELITY-ANCHORS`.
